...
is decomposed into an equivalence $\pathsp{\blank}$
...
second line shows the induced maps on the symmetries.
   \[
     ...
     \conncomp{\Set}{\USymG}\\
     (\sh_G \eqto \sh_G) \ar[r,equivr,"\loops\pathsp{\blank}"] & 
     (\princ G \eqto \princ G) \ar[r,"\loops\ev_{\sh_G}"]&
     (\USymG \eqto \USymG)  
     \end{tikzcd}
   \]
By function extensionality, $\princ G \eqto \princ G$ is
equivalent to $\prod_{x:\BG}(\princ G(x) \eqto \princ G(x))$, the type of
fixed points of the $G$-set $(x:\BG)\mapsto(\princ G(x) \eqto \princ G(x))$.
...
a monomorphism.}
However, torsors have more structure than just a value at $\sh_G$.
even though $f:\prod_{x:\BG}(\princ G(x) \eqto \princ G(x))$
is uniquely determined by its value at $\sh_G$, it has an action on
paths in $\USymG$ ...\label{it:ev-is-eq-on-inv}

Coming back to the example of $\SG_3$, we see that $\princ G$ maps symmetries
of $\bn3$ to symmetries of the $6$-element set $\USG_3\jdeq (\bn3\eqto\bn3)$.
