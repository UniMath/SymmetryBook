...
$R_m$ by $0: R_m(\base)$. In \cref{con:psi-alpha-m} we gave
an equivalence $\psi_m : \Tot(R_m) \equivto \Sc$. Hence $R_m$ is
transitive as $\Sc$ is connected. The point of $\Tot(R_m)$ is $(\base,0)$.
...
That means that $p=\Sloop^mk$ for some integer $k$.
...
Consider the  homomorphism $i:\Sigma_2\to\SG_3$ of permutation groups corresponding to sending $A:\BSG_2\defequi \FinSet_2$ to $A+\true:\BSG_3$.
