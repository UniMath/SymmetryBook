...
\begin{definition}\label{def:map-of-Gsets}
  ...
  When $f$ is such a map, we may write $f_z$ for $f(z)$.\footnote{Such a map $f$
    is called a map of families in \cref{def:fiberwise}.}  \index{map!of
...
\end{definition}
By the construction in~\cref{rem:GSet=SetHomG} we identify $G$-sets
...
from~\cref{def:Gset},
and harmonizes with~\cref{rem:GSet=SetHomG}, relating $G$-sets and
...
\begin{xca}\label{xca:HomGsets-ev}
\wip{New: }
