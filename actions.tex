...
\end{definition}
By the construction in~\cref{rem:GSet=absGset} we identify $G$-sets
...
from~\cref{def:Gset},
and harmonizes with~\cref{rem:GSet=absGset}, relating $G$-sets and
...
\begin{lemma}\label{lem:fixpts-are-fixed}
  ...
  Then the evaluation map $\ev : X^{hG} \to X(\sh_G)$ at $\sh_G$, defined by $\ev (f) \defeq f(\sh_G)$,
  \begin{enumerate}
...
\begin{proof}
  ...
  from which the proposition $x=f(\sh_G)$ follows since $x$ is fixed.
\end{proof}
...
\begin{xca}\label{xca:Hom_G(X,Y)-ev}\wip{New: }
Let $G$ be a group and let $X$ and $Y$ be $G$-sets.
Show that evaluation at $\sh_G$ defines an equivalence
beween $\Hom_G(X,Y)$ and the subset of $X(\sh_G)\to Y(\sh_G)$
consisting of the fixed elements for the $G$-set
$z\mapsto(X(z)\to Y(z))$.
\end{xca}
...
In \cref{sec:epis} we will prove such an $f$ to be an \emph{epimorphism},
just as we called $f$ a \emph{monomorphism} in \cref{def:typeofmono} 
when $\USymf$ is injective. The notions of monomorphism and epimorphism 
will be generalized in \cref{def:mono-in-cat,def:epi-in-cat}, respectively.}
...
All arrows are paths, so invertible. The squares marked with $\odot$
commute by the definition of $\USymf$ and of $Z$, plus easy path algebra.
Hence the middle square commutes
if and only if the outer one (the contour) does.}
\end{figure}

\begin{proof}
By \cref{lem:inj-ap}  it suffices to prove that,
for all $H$-sets $X$ and $Y$, the induced map 
$f^*:(X\eqto Y)\to(f^*X\eqto f^*Y)$ is an equivalence.
By function extensionality, the domain of the induced $f^*$ is equivalent 
...
\ie those $p:Z(\sh_H)$ such that $p\cdot X(h) = Y(h)\cdot p$ 
for all $h:\USymH$.\footnote{Equivalently: 
$p(h\cdot_X x)=_{Y(\sh_H)} h\cdot_Y p(x)$ for all $x:X(\sh_H)$ and $h:\USymH$.}
...
are equivalent. In particular, if $\USymf$ is surjective we can use
...
\begin{remark}
  Note that there is a more elementary proof of the result above, that doesn't
  rely on \cref{lem:BGbytorsor} (a very strong result!). All we need to show is
  that $\B\rho_G :\BG \to \BSG_{\USymG}$ is a \covering. But, for any set
  $X:\BSG_{\USymG}$, the fiber of $\B\rho_G$ at $X$ is
  \begin{displaymath}
    \inv {\B\rho_G} (X) \jdeq \sum_{z : \BG}X \eqto (z\eqto\sh_G).
  \end{displaymath}
  Take two elements $(z_1,\phi_1)$ and $(z_2,\phi_2)$ of this fiber. Then an
  identification of type $(z_1,\phi_1) \eqto (z_2,\phi_2)$ is given by a path
  $p: z_1 \eqto z_2$ such that the following triangle commutes:
  \begin{displaymath}
    \begin{tikzcd}[ampersand replacement=\&]
    ...
    \end{tikzcd}
  \end{displaymath}
  The goal is to prove that $(z_1,\phi_1) \eqto (z_2,\phi_2)$ is a proposition,
  and so we can use the connectedness of $\BG$ and suppose that
  $z_1\jdeq\sh_G$. In this case, the commutative triangle above fully determines
  $p$: indeed, looking at the image of $\refl {\sh_G}$ (in the upper right
  corner), we get $p = \inv{\phi_2(\inv{\phi_1}(\refl {\sh_G}))}$.

  However, the proof presented with the theorem's statement is conceptually more
  interesting, and offers a preview into the Yoneda Lemma (see
  \cref{thm:yoneda-lemma}).
\end{remark}

Note that the \cref{lem:allgpsarepermutationgps} yields that $(G,\rho_G)$ is a
monomorphism into $\SG_{\USymG}$. In other words, $G$ is a subgroup of
$\SG_{\USymG}$.  Note that the induced map on symmetries is:
\begin{displaymath}
  \USym\rho_G:\USym G \to (\USym G \eqto \USym G): 
  g \mapsto (x \mapsto x\inv g)
\end{displaymath}
The reader may ask for a more informative underlying group of this subgroup than
$G$ itself. To this end we apply $E$ from \cref{lem:SubG=MonoG} to $(G,\rho_G)$
to obtain a subgroup in the sense of \cref{def:set-of-subgroups}. The underlying
group of $E(G,\rho_G)$ has classifying type
$\sum_{X:\BSG_{\USymG}} \inv{\B\rho_G} (X)$ and shape
$(\USymG,(\sh_G,\id_{\USymG}))$.  As illustrated in the following diagram, the
symmetries of this shape are precisely those permutations $\pi$ of $\USymG$ for
with there exist a $g:\USymG$ such that $\pi$ is right multiplication by
$\inv g$.\footnote{%
  Those who have read Caley's theorem in ordinary abstract group theory might
  find this computation suprising. Indeed, traditionally, Cayley's Theorem
  selects the permutations that are left multiplications by some $g$. We could
  have arranged for $\USym\rho_G$ to select the same permutations here (by
  chosing $\B\rho_G$ to be $\princ G$ instead), but then the triangle of the
  proof does not commute anymore: to recover the commutativity, one must point
  $\ev_{\sh_G}$ by the non-trivial path $\inv{\blank}: \USymG \equivto \USymG$.}
