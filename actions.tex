...
\begin{proof}
  ...
  $\B\rho_G\jdeq\princ G :\BG \to \BSG_{\USymG}$ is a \covering.  Note first
  that $\princ G$ can be factorized as:
  \[
    \begin{tikzcd}[ampersand replacement=\&]
      \BG \ar[r,equivr,"{\pathsp{\blank}^G}"]\ar[dr,"{\princ G}"']
      \& (\typetorsor_G,\princ G) \ar[d,"{\ev_{\sh_G}}"]%\jdeq\bigl((\BG\to\Set)_{(\princ G)},\princ G\bigr)
      \\
      \& \BSG_{\USymG}%\jdeq(\Set_{(\USymG)},\USymG)
    \end{tikzcd}
  \]
  Here, $\pathsp{\blank}^G:\BG\to (\typetorsor_G,\princ G)$ is the map from
  \cref{lem:BGbytorsor}, and
  $\ev_{\sh_G}: (\typetorsor_G,\princ G)\to \conncomp{\Set}{\USymG}$ is the
  evaluation map defined by $\ev_{\sh_G}(X)\defeq X(\sh_G)$. This is a diagram
  of unpointed maps that can be filled through function extensionality: for any
  $x:\BG$, path symmetry gives an equivalence from
  $\pathsp x^G(\sh_G) \jdeq (x \eqto \sh_G)$ to
  $\princ G (x) \jdeq (\sh_G \eqto x)$. In other words, we have an
  identification $\sigma : \ev_{\sh_G} \circ \pathsp{\blank}^G \eqto \princ G$.

  Since $\princ G$ is an equivalence, we are done if we show that, for any
  $S:\Set_{(\USymG)}$, the fiber $\inv{\ev_{\sh_G}}(S)$ is a set. This fiber is
  by definition $\sum_{X:\typetorsor_G}(S\eqto X(\sh_G))$, which is a subtype of
  $\sum_{X:\BG\to\Set}(S\eqto X(\sh_G))$. The latter is the type of pointed maps
  from $\BG$ to $(\Set,S)$ and hence a set by \cref{lem:hom-is-set} (more
  speicifcallu \cref{ft:ptd-decr-h-lev} since $\BG$ is connected and $\Set$ is a
...
\wip{%
  ...
  \begin{proof}
    We need to show that $\B\rho_G\jdeq\princ G :\BG \to \BSG_{\USymG}$ is a
    \covering. For any set $X:\BSG_{\USymG}$, the fiber of $\rho_G$ at $X$ is
    \begin{displaymath}
      \inv {\rho_G} (X) \jdeq \sum_{z : \BG}X \eqto \princ G (z).
    \end{displaymath}
    ...
    \begin{displaymath}
      \begin{tikzcd}[ampersand replacement=\&]
        ...
        \& (\sh_G \eqto z_1) \dar["p\cdot\blank"]
        \\
        \& (\sh_G \eqto z_2)
      \end{tikzcd}
    \end{displaymath}
...
\begin{remark}\label{rem:CayleyOversize}
  ...
  $\princ G: \BG\ptdto\conncomp{\Set}{\USymG}$ classifying the monomorphism
  $\rho_G$ is decomposed as an equivalence $\pathsp{\blank}$ (pointed by
  reflexivity) followed by the evaluation map $\ev_{\sh_G}$ pointed by
  $\sigma_{\sh_G} \jdeq \inv{(\blank)} : \USymG \eqto \USymG$. This is depicted in the
  following diagram, where the second line shows the induced maps on the
  symmetries.
   \[
     \begin{tikzcd}
     \BG \ar[rr,equivr,"\pathsp{\blank}"] && 
     (\typetorsor_G,\princ G) \ar[rr,"\ev_{\sh_G}"]&&
     \conncomp{\Set}{\USymG}\\
     \USymG \ar[rr,equivr,"\pathsp{\blank}"] &&   %\loops?
     (\princ G \eqto \princ G)
     \ar[rr,"\inv{\sigma_{\sh_G}} \circ\ev_{\sh_G}(\blank)\circ \sigma_{\sh_G}"]&&  %\loops?
     (\USymG \eqto \USymG)  
     \end{tikzcd}
   \]
   ...
   its value $h_{\sh_G}$ at $\sh_G$, and thus also by the permuatation
   $\inv{(h_{\sh_G}(\inv\blank))}$.\footnote{%
     This is an alternative way to understand that $\ev_{\sh_G}$, and hence
     $\princ G$, classifies a monomorphism.%
   } %
   The underlying set of $\PP$ is $\PP(\sh_G)\jdeq(\USymG \eqto \USymG)$.
   ...
   $\USymG$? See exercise \cref{xca:PP-fixed-permutations}.

   In other words, $\ev_{\sh_G}$ forgets about the extra structure of $\PP$ and
   sends invariant maps of $\PP$ to fixed permutations of $\USymG$. For example,
   in the case of $\SG_3$, the fixed permutations are the 3 elements of
   $\bn3\eqto\bn3$ given by multiplication by each of the 3 elements of $\bn3$.
\end{remark}

\begin{xca}\label{xca:PP-fixed-permutations}
  Let conditions be as in \cref{rem:CayleyOversize}.  By analyzing transport in
  the type family $\princ G(\blank)$, show that a permutation $\pi$ of $\USymG$
  ...
  this group to \wip{the opposite of} $\abstr(G)$.
\end{xca}
...
\wip{%
  ...
  \begin{proof}
    ...
    $g\cdot\varphi = g\varphi(\inv g \cdot \blank)$. When such a $\varphi$ is
    fixed by the action, then $g\varphi(\inv g \blank) = \varphi$ for all
    $g$. That is $\forall g,g', \varphi(g g') = g\varphi(g')$. In other words,
    $\varphi = \blank \varphi(\refl{\sh_G})$. So, all in all,
    $\USym (\mkgroup(\ev_G,\sigma_{\sh_G})) (h) = x \mapsto
    \inv(h_{\sh_G}(\refl{\sh_G}))x$. When $h \jdeq \princ g$ for some
    $g:\USymG$, we get $h_{\sh_G}(\refl{\sh_G}) = \refl{\sh_G} \inv g$, so the
    selected permutation is indeed the left multiplication $x \mapsto g x$.
  \end{proof}%
}
