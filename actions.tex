...
action type $X_{hG}$ is a set if and only if each $x:X(\sh_G)$ is trivial.
\end{xca}
...
\begin{definition}\label{def:stab-action-on-G}
...
\begin{xca}\label{xca:stab-action-on-G}\MB{New:}
Let $s:(\sh_G,x,!)\eqto(z,y,!)$ be a path in $\BG_x$ and
let $g:\USymG$. Show that $s\cdot_{\tilde G_x} g = sg$, \ie
...
\footnote{\MB{Replaces \cref{old:orbitstab}.}}
  ...
  Recall the $G_x$-set $\tilde G_x$ from \cref{def:stab-action-on-G}.
...
so the one and only orbit is $X(\sh_G)$. 
Then $\fst: (G\cdot x) \to X(\sh_G)$ is an equivalence.
Also, we have that ${\tilde G_x}/G_x$ is a set by \cref{thm:orbitstab}, 
so using \cref{xca:orbit-set-as-quotient} we get an
equivalence from $X(\sh_G)$ to ${\tilde G_x}/G_x$. We denote 
the latter equivalence by $[\blank]_x$ and elaborate it
for all $x':X(\sh_G)$ and $z:\BG$, $y:X(z)$, $h:\sh_G\eqto z$ as
\[
[x']_x(z,y,!,h) \defeq \bigl((\sh_G,x',!,\refl{\sh_G})=(z,y,!,h)\bigr).
\]
\marginnote{%
...
Combining the above with \cref{lem:sum-of-fibers}, we get the
...
$g\cdot_X y = x$.
So we get an equivalence between $\USymG$ and 
...
\begin{tabular}{|c|c|c|} 
 ...
 \hline
\end{tabular}
\caption{Underlying sets of orbits and the stabilizers of their elements.}
\end{marginfigure}
