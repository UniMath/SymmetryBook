...
bit the definition above. Given a subgroup\footnote{%
    ...
    conveniences regarding subgroups.} $(X,x):\Sub(G)$ in the sense above,
...
namely $(\sh_G,x)$. \cref{xca:group-Xx!} above tells us that this choice of
...
$(\sh_G,x)$ in $\Tot(X)$ into these of $\sh_G$ in $\BG$. This explain the
...
\begin{definition}\label{def:underlying-group-of-subgroup}
  ...
  $(X,x) : \Sub(G)$ is
  \[
    \mkgroup \left(\Tot(X),(\sh_G,x)\right).\qedhere
  \]
\end{definition}
...
\end{lemma}
\begin{proof}
  Let $G$ be a group, and let $(X,x)$ and $(X',x')$ be subgroups of $G$. Since
  $X$ is transitive we can use the remark following
  \cref{lem:evisinjwhentransitive} that $\ev_x : (X \eqto X') \to X'(\sh_G)$ is
  injective. In particular, the preimage of $x'$ is thus a proposition. Now, we
  have a chain of equivalences:
  \begin{eqnarray*}  
    \left((X,x)\eqto(X',x')\right)
    & \equivto & \displaystyle\sum_{p:X \eqto X'} x' = \trp p x \\
    & \equivto & \displaystyle\sum_{p:X \eqto X'} x' = \ev_x(p) \\
    & \jdeq & {\ev_x}^{-1} (x'). 
  \end{eqnarray*}
\end{proof}
...
\begin{lemma}\label{lem:SubG=MonoG}
  ...
  $(X,x) : \Sub(G)$ to the element
  \begin{displaymath}
    \bigl(\mkgroup\bigl(\Tot(X),(\sh_G,x)\bigr),\mkgroup(\fst)\bigr) : \typemono(G),
  \end{displaymath}
  where $\fst: \bigl(\Tot(X),(\sh_G,x)\bigr)\ptdto\BG$ is pointed by
...
\begin{proof}
  ...
  we can define a map $E$ defined as follows:%
...
\begin{lemma}\label{lem:E-preserves-symms}
  ...
  \cref{lem:SubG=MonoG}.  For all $(X,x) : \Sub(G)$ and $(H,i) : \typemono(G)$
  such that $(H,i)=F(X,x)$, we have $g \cdot_X x = x$ in $X_{\sh_G}$ if and only
  if $g$ is in the image\footnote{\ie there exists $h:\USymH$ such that
    $g = \USymi(h)$ in $\USymG$.} of $\USymi$.
\end{lemma}
\begin{proof}
Let $(X,x):\Sub(G)$. It suffices to prove the lemma for 
$(H,i)\jdeq F(X,x) : \typemono(G)$.
This means $\BH\jdeq(\Tot(X),(\sh_G,x))$ and $\Bi\jdeq\fst$.
We have to prove: $g \cdot_X x = x$ iff there exists an
$h:(\sh_G,x)\eqto(\sh_G,x)$ such that $g = \USymi(h)\jdeq\loops\fst(h)$.

If $g \cdot_X x = x$, then we can simply take $h$ to the symmetry defined by the
pair $(g,\refl x)$.

For the converse, assume there exists an $h:(\sh_G,x)\eqto(\sh_G,x)$ such that
$g = \USymi(h)$.  Then $h$ is defined by a pair of the form $(g,p)$ for some
$p: \pathover x {X} g x$.\footnote{This path $p$ is in fact equal to
  $\refl{\pt}$ since $X_{\sh_G}$ is a set.} But $\trp [X] p x$ is precisely
$X(g)(x)$ (\ie $g \cdot_X x$).
\end{proof}
...
We say that a subgroup $(X,x):\Sub(G)$ is:
\begin{enumerate}
\item \emph{trivial}\index{trivial subgroup} if the underlying group
  $(\Tot(X),(\sh_G,x))$ is trivial\wip{, \ie when $\Tot(X)$ is contractible};
...
\begin{remark}\label{rem:notationsubgroup}
  A note on classical notation is in order.  If $(X,x)$ is a subgroup
...
\begin{example}
  ...
  then $(H,i):\typemono(G)$ corresponds to the subgroup with $G$-set 
