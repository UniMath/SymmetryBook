...
indentified with the classifying type of the group $G$. In other words, the
...
natural number $m>0$, see \cref{def:mfoldS1cover}).  At the level of the set
...
\begin{displaymath}
  \pathsp{\blank}^G:\BG\equivto\typetorsor_G : z \mapsto \{ y \mapsto (z \eqto y)\}.
\end{displaymath}
Fixing the second argument to $\sh_G$, we obtain a map
...
\end{proof}
\wip{%
  Isn't it a bit overkill to use the equivalence with torsors? I feel the
  following proof is much simpler:
  \begin{proof}
    We need to show that $\B\rho_G :\BG \to \BSG_{\USymG}$ is a \covering. For
    any set $X:\BSG_{\USymG}$, the fiber of $\rho_G$ at $X$ is
    \begin{displaymath}
      \inv {\rho_G} (X) \jdeq \sum_{z : \BG}X \eqto \pathsp z^G (\sh_G).
    \end{displaymath}
    ...
    $p: z_1 \eqto z_2$ and such that commutes the triangle
    \begin{displaymath}
      \begin{tikzcd}[ampersand replacement=\&]
        X \rar["\phi_1"] \drar["\phi_2"swap]
        \& (z_1 \eqto \sh_G) \dar["p\cdot\blank"]
        \\
        \& (z_2 \eqto \sh_G)
      \end{tikzcd}
    \end{displaymath}
    In proving the proposition that such a $p$ is unique, we can use the
    connectedness of $\BG$ and suppose that $z_1\jdeq\sh_G$, in which case we
    can see that $p$ is fully determined by the other data: $p$ must be
    $\phi_2(\inv{\phi_1}(\refl{\sh_G}))$.
  \end{proof}%
}


Note that the above theorem yields that $(G,\rho_G)$ is a monomorphism into
$\SG_{\USymG}$. In other words, $G$ is a subgroup of $\SG_{\USymG}$. Note that
the induced map on symmetries is:
\begin{displaymath}
  \USym\rho_G:\USym G \to (\USym G \eqto \USym G): g \mapsto \{x \mapsto x\inv g\}
\end{displaymath}
