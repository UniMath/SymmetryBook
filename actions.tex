...
\begin{example}\label{ex:S2-acts-on-C3}
  ...
  Within this type we have the pair $(\bn 1 \coprod S,f)$,
  where
  \begin{align*}
    f_s(\inl\triv)     &\defeq \inr s,\\
    f_s(\inr s)        &\defeq \inr{\swap(s)},\\
    f_s(\inr{\swap(s)}) &\defeq \inl\triv.
  \end{align*}
  Then $G(S) \defeq \Aut_{\sum_{X:\Set}S\to X\to X}(\bn1\coprod S,f)$ defines an action
  $\BSG_2 \to \Group$.
  Furthermore, we identify $G(\bool)$ with $\BCG_3$ by mapping
  a shape $(X,f)$ in $\BG(\bool)$ to the $3$-cycle $(X,f_\yes)$
  and identifying the $3$-cycle $(\bn1\coprod\bool,f_\yes)$, for the $f$ defined above,
  with the standard $3$-cycle $(\bn3,\zs)$.
\end{example}
