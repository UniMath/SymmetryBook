...
\MB{New:} Find groups $G,H$, $f:\Hom(G,H)$ and $G$-set $X$ such that
...
\begin{construction}
  \label{lem:inducedtorsor}
   ...
   Can easily be implemented by starting everywhere by:
   Let $\mkgroup f$ be a homomorphism ... If you want to have the
   operation ${}_*$ on homomorphisms, 
   just define $(\mkgroup f)_* \defeq \mkgroup f_*$}}
   \[
     \begin{tikzcd}
...
\begin{remark}
  ...
  sending $[(p,q)]$ to $p\,\phi(q):(\shape_H\eqto y)$.\footnote{%
    \color{red}UB: I'd like to rephrase this in terms of noticing
    when $f_*X$ is a set, or maybe redefining $f_*X$ to always take the
    set truncation. MB: OK now? BTW $[(p,q)]$ needs explanation.}
\end{remark}
