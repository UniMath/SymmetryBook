...
Note that \cref{xca:stab-action-on-G} already
...
\footnote{\MB{Replaces \cref{old:lagrange}.}} 
  Let $G$ be a group and let $(X,x,!):\Sub_G$ be a subgroup of $G$,
  with the stabilizer group $G_x$ as underlying group. 
  Then:
  \begin{enumerate}
  \item The $G_x$-set $\tilde G_x$ from \cref{def:Gx-action-on-G}
  has underlying set $\USymG$ and is free. 
  \item The function $[\blank]_x : X(\sh_G)\to\tilde G_x /G_x$
  from \cref{lem:preLagrange} is an equivalence,
  and all orbits in the set $\tilde G_x /G_x$
  are \emph{merely} equivalent to $\USymG_x$.
  \end{enumerate}
\end{theorem}
