...
Then, we say that a subgroup $(X,\pt,!):\Sub_G$ is:
      ...
      \item \emph{proper}\index{proper subgroup} if $X(\shape_G)$ is not
      contractible.
      \end{enumerate}
...
\begin{definition}\label{def:orbit-stabilizer}
  ...
  \item Define $G\cdot x \defeq \setof{y : X(\shape_G)}{[x] =_{X/G} [y]}$
    to be the \emph{orbit}\index{orbit} of $x$.\qedhere
  \end{enumerate}
\end{definition}
...
as sums of the respective fibers.\footnote{%
  ...
  \MB{Unclear because of "set of orbits". 
  What means $G\cdot x$ (pointed at $x$)?})}
...
\begin{example}\label{ex:pathsptransport}
  ...
  is obtained
  by sending $p:\pathsp y(x)\jdeq (y\eqto x)$ to
...
\begin{proof}
  ...
  (this\footnote{\MB{Props!}} indeed holds when $p\jdeq \refl y$).
...
\begin{proof}
  ...
  on $q:y\eqto z$, we have\footnote{\MB{Duplication? MB confused by some P's.
  We could use $P_\blank$ to indicate that the argument is subscripted,
  $\mkgroup P$ for the iso, adapt 5.5.7 a bit, and use $\ap{P_\blank}$ for the action. Univalence and funext are transparent.}}
  \[
...
\begin{definition}
  ...
  the \emph{induced $H$-set}\index{action!induced}\index{induced action}\footnote{%
    Without set truncation on the right, $f_*X$ may
    not be an $H$-\emph{set}. As an example, let $f$ be the unique
    homomorphism $f: \Hom(\ZZ,\mkgroup(\bn1,0))$. Let $X$ be the constant
    $\ZZ$-set mapping each $z:\Sc$ to $\bn 1$. Then 
    $\sum_{z:\Sc}(0=0)\times \bn 1$ is a circle, so not a set.
    
    
    
    
    \MB{Old:} As an example, consider the homomorphism 
    $f\defeq(\blank\mod 2) : \Hom(\ZZ,\SG_2)$
    ...
    $R_2$ from~\cref{def:RmtoS1}.) Let $X$ be the constant
    $\ZZ$-set mapping each $z:\Sc$ to $\bn 1$. Then 
    ...
    that some construction is not ``exact''. \MB{Points first everywhere?}}
  $f_*X : \BH\to\UU$ by setting, for $y:\BH$,
  \[
    f_*X(y)\defeq\setTrunc{\sum_{z:\BG}(\Bf(z) \eqto y)\times X(z)}.\qedhere
  \]
\end{definition}
\begin{xca}\label{xca:id_*-is-id}
\MB{New:} Give an equivalence from $f_*X$ to $X\inv\Bf$
...
\[
  \eta_y:f_*\pathsp x^G(y) \jdeq \setTrunc{\sum_{z:\BG}(\Bf(z) \eqto y)\times(x \eqto z)}
  \equivto (\Bf(x)\eqto y)\jdeq\pathsp{\Bf(x)}^H(y).
\]
The resulting identity $\bar\eta:f_*\pathsp x^G\eqto \pathsp{\Bf(x)}^H$ shows that for every $G$-torsor $X$ the $H$-set $f_*X$ is an $H$-torsor.
...
gives an identification $f_*\princ G\eqto \princ H$.\footnote{%
\MB{If $X$ is a $G$-set such that $\Trunc{\princ G \eqto X}$, then $f_*X$
is an $H$-set such that $\Trunc{\princ H \eqto f_*X}$ by $\eta_\blank$ for
$x\jdeq\sh_G$ and using $\Bf_\pt$.}}
...
\begin{construction}
  \label{lem:inducedtorsor}
   ...
   and we have an element of the identity type 
   $f_* \pathsp{}^G \eqto \pathsp{}^H \Bf$
   represented by the following diagram:\footnote{%
   \MB{As pointed maps. Ugly: $f_*$ has no $B$.}}
   \[
     \begin{tikzcd}
