...
For \MB{(check decidable set not needed)} any type $A$, 
give an equivalence from $A$ to $\sum_{B:\UU}(A\eqto(B+\true))$.
\end{xca}
...
\begin{lemma}\label{lem:SubG=MonoG}
  ...
  reflexivity, is an equivalence from $\Sub_G$ to $\typemono_G$.
\end{lemma}
