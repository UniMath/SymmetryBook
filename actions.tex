...
and $[\blank]_0$, respectively) with quotient set $X/G$.\footnote{%
  ...
  \[
    ...
    \end{tikzcd}
  \]}
  We can write $X(\sh_G)$ and $X_{hG}$
as sums of the respective fibers, and these fibers can be expressed
intuitively.
...
is equivalent to the underlying set of $X_O$,
$X_O(\sh_G)\jdeq\sum_{x:X(\sh_G)} O(\sh_G,x)$.
In other words, the fiber $\inv{[O]}$ is the underlying set of $X_O$.
