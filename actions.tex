...
\wip{{\em Replace by?}A central notion in this chapter is that of ``$G$-torsor'', introduced in
...
\wip{{\em Delete?}}
  ...
  One of the goals of \cref{sec:Gsetforabstract} below is to prove that the
  types of groups and abstract groups are equivalent.  In doing that, we are
  invited to explore how elements of abstract groups should be thought of as
  symmetries and introduce the notion of a $G$-set.  However, this takes a
  pleasant detour where we have to explore a most important feature of groups:
  they can \emph{act} on things (giving rise to manifestations of symmetries)!

\wip{{\em Replace by?}

  In a group, symmetries of the shape can be thought of as ``transformations''
  that leave the shape overall unchanged. For example, in the circle $\Sc$, the
  loop $\Sloop$ is a non-trivial transformation that leaves the base point
  $\base$ unchanged. But symmetries can also \emph{act} on other mathematical
  entities than their shape. We have seen that phenomenon already in the case of
  the circle. Take for example the $2$-fold cover of $\Sc$: the symmetry
  $\Sloop$ acts on the two elements in the fiber over $\base$ by swapping
  them. The following definition decribes that phenomenon for arbitrary groups.}
...
\begin{definition}\label{def:Gset}
  For $G$ a group, a \emph{$G$-set} is a function\footnote{Recall that, to make
    things more lisible, we write $\BG\to\Set$ instead of the more correct
  ...
  in $\BG$, then the transport function $\trp{X}(p) : X(x)\eqto X(y)$ \wip{(is
    the use of transparent univalaence warranted here? it feels like
    $p \cdot_X \blank$ is better thought as a function anyway)} is also denoted
  by $X(p)$.  We denote $X(p)(a)$ by $p\cdot_X a$.  The operation $\cdot_X$ is
  called the \emph{group action} of $X$.  When $X$ is clear from the context we
  may leave out the subscript $X$.\footnote{%
  ...
  particular, if $g:\USymG$, then $X(g)$ is a permutation \wip{\em (possible
    conflict: ``permutation'' usually means non-identity on a finite subset)} of
  the underlying set $X(\sh_G)$ of $X$.
...
\wip{\em Delete?} If we vary both ends of the
identifications simultaneously, we get another $G$-set:
\wip{{\em Replace by?} More generally,
  $\pathsp y (y)\defeq (y \eqto y) \defeq \USym \mkgroup(\BG_\div,y)$. Varying $y:\BG$,
  we get in this way another $G$-set of importance:} %
\begin{example}\label{def:adjointrep}
  If $G$ is a group, then
  \[
    \Ad_G:\BG\to\wip{\Set}\UU,\qquad\Ad_G(z)\defequi(z\eqto z)
  \]
  is a $G$-set \wip{\em Delete?} (or $G$-type) called
  the \emph{adjoint $G$-set \wip{\em Delete?} (or $G$-type)}.\footnote{%
...
\wip{
  \begin{xca}\label{xca:equivariant-map-totalization}
    ...
    $\sum_{f : \Tot(X) \to \Tot(Y)}\fst = \fst \circ f$.
  \end{xca}
}

\begin{remark}\label{rem:map-of-Gsets}
  Given $G$-sets $X,Y$ and a map\footnote{\wip{Any reason we don't use the word
      {\em equivariant} at all in the book?}} $f$ from $X$ to $Y$, we have
  $f_w(g\cdot_X x) = g\cdot_Y f_z(x)$ for all $z,w:\BG$, $x:X(z)$, $g:z\eqto
  w$. In other words, the diagram on the right commutes:\footnote{\wip{Remove
      the ``='' signs in the square?}}
\[
\begin{tikzcd}
...
\begin{definition}\label{def:Gsubset}
  ...
  $\triv_G\Prop$ \wip{Remove, it has already been recalled above:[}that is constant $\Prop$.\wip{]} 
...
%   \jdeq \bigl(\prod_{z:\BG}(X(z)\to\Prop)\bigr)
  ...
  \[
  X_P(z) \defeq \sum_{x:X(z)}{P(z,x)}\wip{P(z)(x)},\quad\text{for all $z:\BG$}.\qedhere
  \]
...
the group action.\footnote{\wip{Very difficult to read out of the blue: why not
    establish in 2.20 the notations ``$P\subseteq X$'' for $P:\Sub(X)$ and for
    any $x: X$, ``$x\in P$'' for $P(x)$?}}
\end{xca}
...
a set $X(\sh_G)$ and
a homomorphism from $G$ to the permutation group of $X(\sh_G)$. \wip{Conversely, a set $S$ together with a homomorphism $f$ from $G$ to $\SG_S$ can be described as a $G$-set with underlying set $S$ and whose group action $g \cdot \blank$ for $g : \USymG$ coincides with $f(g)(s)$.\footnote{The $G$-set associated to the homomorphism $f$ is the ``delooping'' of $f$, and it will be formally defined in \ref{sec:delooping}}.}
\end{remark}
...
and sets with an action of $G$ on a set.\footnote{\wip{This definition is never referenced by label. Do we really need it?}}
...
\begin{proof}
  ...
  $\fst:\Tot(X)\to\BG$. Recall that $\Tot(X)\defequi\sum_{w:\BG}X(w)$.
  \wip{Delete? (The use of the elimination rule for connectedness is not easy to
    see here since the proposition doesn't start with a universal
    quantification.)} Now, $\Tot(X)$ is connected if and only if there exists a
  $z:\BG$ and an $x:X(z)$ such that for all $w:\BG$ and $y:X(w)$ there exists
  some $g:z\eqto w$ such that $y=g\cdot x$. Since $\BG$ is connected, this is
  equivalent to asserting that there exists some $x:X(\sh_G)$ such that for all
  $y:X(\sh_G)$ there exists some $g:\USymG$ such that $x=g\cdot y$.

  \wip{Replace by: Now,
  $\Tot(X)$ is connected if and only if it isn't empty and for all $z:\BG$ and $x:X(z)$
  and all $w:\BG$ and $y:X(w)$ there exists some $g:z\eqto w$ such
  that $y=g\cdot x$. Since $\BG$ is connected, this is equivalent to asserting
  that $\Tot(X)$ isn't empty and for all $x,y:X(\sh_G)$ there
  exists some $g:\USymG$ such that $y=g\cdot x$.}
\end{proof}
...
\begin{xca} \label{xca:A-is-A-1+1} \wip{Seems a bit out of place (although
    connected to the previous exercise obviously). Maybe better to make it part
    of the previous exercise directly.}%
  For any set $A$ with decidable
  equality, give an equivalence from $A$ to $\sum_{B:\UU}(A\eqto(B+\bn1))$.
\end{xca}
...
group of $(F,[0])$ is
\begin{displaymath}
  \mkgroup\left(\sum_{(X,t):\Cyc_6} X/2, ((\bn6,\zs),[0])\right).
\end{displaymath}
...
\begin{definition}
  ...
  The \emph{type of monomorphisms into $G$}\footnote{%
    ...
    \wip{What about an equivalence between concrete mono and abstract mono? That would be enough.}
  }
...
\wip{Does the rest of the example really belong here? It doesn't have anything
  to do with \cref{exa:fix1subSGn}, and it uses ``proper'' and ``non-trivial'',
  not defined for subgroups.} How can we obtain the other proper, non-trivial
subgroups of $\SG_3$? First of all, one should not expect to find all subgroups
through monomorphisms $j:\SG_2\to\SG_3$, see \cref{xca:C3subSG3}. Using only
$\SG_2$, the two other subgroups can be obtained by varying the pointing path of
...
\begin{lemma}\label{lem:E-preserves-symms}
  ...
  if $g$ is in the image\footnote{\ie there exists $h:\USymH$ such that
...
\item \emph{trivial}\index{trivial subgroup} if the underlying group
  $(\Tot(X),(\sh_G,\pt))$ is trivial\wip{, \ie when $\Tot(X)$ is contractible};
...
\begin{remark}\label{rem:notationsubgroup}
  ...
  $i:H\subseteq G$'', or, if we didn't need the name of $i:\Hom(H,G)$, simply
  ``a subgroup $H\subseteq G$'' or ``a subgroup $H$ of $G$''.
    \end{remark}
...
\begin{example}
  ...
  equality $(\BG, i_1) = F(\princ {G'}\circ \mathrm{proj}_2, \refl{\sh_G'})$ in
...
\wip{
  \begin{xca}\label{xca:lagrange-if-subgr-not-normal}
    ...
    $G\defeq \mkgroup (\Sc \wedge \Sc)$, the free group on two generators
    ...
    the first loop $\Sloop_1$ is mapped to the successor modulo 3
    ...
    interest here is $(X,0):\Sub(G)$.
    \begin{enumerate}
    \item Compute the underlying group of $(X,0)$.
    \item Find a ``choice map'' $f:\prod_{x:\bn 3}\sum_{g:\USymG} (g\cdot x = 0)$
      and apply Lagrange's construction. \qedhere
    \end{enumerate}
  \end{xca}
}
...
any two elements in the total type $\Tot(X) \\sum_{x:\BG} X(z)$.
...
% $(\sum_{z:\BG}\sum_{y:X(z)}\Trunc{(\sh_G,x) \eqto (z,y)},(\sh_G,x,!))$.
    ...
    \wip{Notice that for $(X,\pt):\Sub(G)$, the group $G_\pt$ is the underlying group
...
\wip{Remove until next paragraph, it resays the same thing again.}Thus, for
$O\jdeq[x]$, we have easy identifications of $G\cdot x$ and $X_O(\sh_G)$, as
well as of $\BG_x$ and $(X_O)_{hG}$, using $[x]\jdeq[(\sh_G,x)]_0$.  Applying
the maps in \cref{ft:orbit-fibs} in this particular case, we obtain
\cref{fig:fibs-at-[x]}.
\begin{marginfigure}
