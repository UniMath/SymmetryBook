...
The case of $m=0$ is special, since the iterates of $\Sloop^0$, \ie of
...
the trivial subgroup of the group of integers. There is no
degree $0$ function, but we can pick out the single symmetry $\Sloop^0$
...
Transitivity of $R_m$ is obvious. The point of $\Tot(R_m)$ is $(\base,0)$.
What are the symmetries of this point?
An identification of type $(\base,0)\eqto(\base,0)$ is an
identification $p: \base\eqto\base$ such that $R_m(p)(0)=0$,
that is, $p=\Sloop^{mk}$ for some integer $k$.
...
The point of $\Tot(R)$ is $(\base,0)$ and the only symmetry of
$(\base,0)$ is the reflexivity path since $R(\Sloop^k)(0)=0$ iff $k=0$.
Again, no surprise in view of the results in \cref{sec:symcirc}
identifying $R$ as the universal \covering over $\Sc$.

The following result generalizes the fact that $\Sub_T$ is
as set for any type $T$, see \cref{xca:subtypes-set}. It captures
...
Consider the  homomorphism $i:\Sigma_2\to\SG_3$ of permutation groups,
corresponding to sending $A:\BSG_2\defequi \FinSet_2$ to $A+\true:\BSG_3$.
As pointing path we take the reflexivity path of type $\bn3\eqto(\bn2+\true)$.
In the picture in the margin we have taken $3$ for $\inr{\triv}$.
Two other subsgroups can be obtained by taking $1$ and $2$ for $\inr{\triv}$.
...
\begin{xca}
  ...
  Show that $\Hom(\SG_2,\SG_2)$ is contractible. Hint: use
  \cref{xca:CG2isSG2}. Here comes another interesting solution.
  If $f:\Hom(\SG_2,\SG_2)$, then $\USym f: (\bn2\eqto\bn2)\ptdto
  (\bn2\eqto\bn2)$. There is exactly one such pointed map,
  ...
  identification of type $\id\eqto_{\BSG_2}\Bf$.
...
\end{proof}
  ...
  \begin{example}
    The monomorphism of $\SG_2$ into $\SG_3$ of \cref{ex:SG2inSG3} can be displayed as a subgroup of $\SG_3$ through
    $$X:\FinSet_3\to\Set
    $$
