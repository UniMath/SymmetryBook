...
\begin{definition}
  ...
  $f_*X : \BH\to\UU$ by setting, for $w:\BH$,
  \[
    f_*X(w)\defeq\myTrunc{\sum_{z:\BG}(\Bf(z) \eqto w)\times X(z)}{0}.\qedhere
  \]
\end{definition}
...
image and preimage, respectively. To see this, consider
the special case of $X:\BG\to\Prop$ and $Y:\BH\to\Prop$, that is,
$X$ is a subtype of $\BG$ and $Y$ is a subtype of $\BH$. Also,
make the definition of $f_*X$ ``exact'' by propositional truncation
instead of set truncation. Then $f_*X$ is the predicate
that is true for those $w:\BH$ such that there exists an $z:\BG$
in the fiber of $\Bf$ at $w$ such that $X(z)$ holds, that is, the
image of $X$ under $\Bf$.
Also, $f^*Y$ is the predicate that is true for those $z:\BG$ such
that $Y(\Bf(z))$ holds, that is, the preimage of $Y$ under $\Bf$.
\end{remark}

Note that the type $f_*X(w)$ can also be identified
as the orbit set $(H^w \times X)/G$ of the $G$-set $H^w\times X$,
where $(H^w\times X)(z) \defeq (\Bf(z) \eqto w)\times X(z)$ for $z:\BG$,
and whose underlying set is equivalent to $(\sh_H\eqto w)\times X(\sh_G)$.
...
\begin{xca}\label{xca:adjunction-_!-^*}\MB{New:}
