...
The results in this section culminate in the following theorem.
...
(2) works for the finite case, but also, \eg for $\ZZ$ acting on $\bn3$.
...
to $\USymG_x$ in the proof of point \ref{it:lagrange} of \cref{thm:lagrange},
provided we have a $g_0:\USymG$ such that $g_0\cdot_X y = x$.
Hence we can just take $g_0 \jdeq f(y)$ and we are done.
\end{implementation}

\begin{definition}\label{def:finite-G-set}\MB{Where?}
Let $G$ be a group and $X:\BG\to\Set$ a $G$-set.
We say $X$ is \emph{finite} if the set $X(\sh_G)$ is finite.
\index{finite $G$-set}
...
The goal of this exercise is to state and prove a more traditional
