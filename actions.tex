...
\begin{definition}
%via the definition of X/G as the set truncation of the action type, that an orbit is also a connected component of the action type
...
in this context is that $(g \cdot f(z)) = f(z)$ for any $z:\BG$
and $g:z\eqto z$.}
\index{invariant map type} is
\[
  X^{hG} \defeq \prod_{z:\BG} X(z).
\]
The \emph{set of orbits}\footnote{\MB{Homotopy orbit spaces? Above MBs head.}}
\index{set!of orbits}\index{orbit set} is the set-truncation of the action type,
\[
...
\begin{proof}
  ...
  if and only if $\exists_{g:z\eqto w}(g\cdot x\MB{=}y)$.\footnote{%
  \MB{Also iff $\Trunc{(z,x)\eqto(w,y)}$.}}
  Thus,~\cref{thm:quotient-property} gives the
  desired conclusion.
\end{proof}
Thus, both the underlying set $X(\shape_G)$ and the action type
...
\begin{lemma}\label{lem:G/X-set-of-orbits}\MB{New}
...
as sums of the respective fibers.\footnote{%
  ...
  \[
    \begin{tikzcd}[ampersand replacement=\&,column sep=tiny]
      ...
      \& \& \displaystyle\sum_{o:X/G} (\BG_o)_\div\ar[dl,"\fst"] \\
      \& X/G, \&
    \end{tikzcd}
  \]
  (What means $\BG_o$? And in the text left: $G\cdot x$ 
  \MB{(pointed at $x$)?} MB lost.)}
...
there is another reasonable definition of the invariant maps,
namely the subset
\[
  \setof{x : X(\shape_G)}{\text{$x$ is fixed}}
\]
consisting of thefixed elements.
If we evaluate a invariant map $f : \prod_{z:\BG}X(z)$ at $\shape_G$
...
The following lemma states that eachfixed $x$ uniquely
determines a invariant map.
...
\begin{proof}
  ...
  $x : X(\shape_G)$ is fixed, so $i_x\jdeq\fst:\BG_x \to \BG$
...
withfixed elements of $\USymG \eqto \USymG$. In other words,
$\ev_{\sh_G}$ forgets about the extra structure of $\PP$
and maps invariant maps of $\PP$ tofixed permutations of $\USymG$.
For example, in the case of $\SG_3$, we go in total from permutations
of $\bn3$ tofixed permutations of the $6$-element set $\bn3\eqto\bn3$.

In \cref{xca:PP-fixed-permutations} you are asked to explore
the abstract group offixed permutations of $\USymG$.
\end{remark}
...
Show that thefixed permutations of $\USymG$ form an abstract group
...
action type, but rather consists of all the elements in $X(\shape_G)$ sent to a given element in the orbit set.
...
$x : X(\shape_G)$ is the automorphism group of $[x]$ in the action type
