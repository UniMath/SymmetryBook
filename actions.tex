...
that do not move the point of $X(\sh_G)$. For this to produce a
...
Those that do not move the point $0: R_m(\base)$, that is,
...
The only symmetry that does not move $0$ is $\refl{\base}$,
...
of the other elements of $\bn n$. Later we will see that the subgroups
...
with $\Bi$ sending $A:\BSG_2\defequi \FinSet_2$ to $A+\true:\BSG_3$.
As pointing path we take the path induced by $\id_{\bn3}$.
In the picture in the margin we have taken $\set{1,2,3}$ for $\bn{3}$.
