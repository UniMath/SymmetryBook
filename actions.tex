...
$G$, we'll mostly work in that generality.\footnote{\MB{To margin?}}
\begin{definition}
...
\]
The \emph{set of orbits}\footnote{\MB{To do: $G$-*sub*types.
Where: terminology homotopy orbit spaces?}}
...
\begin{lemma}\label{lem:orbit-equiv}
  ...
  $\exists_{g:\USymG}(g\cdot x\eqto y)$.
\end{lemma}
\begin{proof}
  By definition, $[x]$ is a $G$-subtype of $X$. Clearly,
  $\Trunc{(z,x)\eqto(w,y)}$ iff $\exists_{g:z\eqto w}(g\cdot x\eqto y)$.
  It follows that the underlying $G$-type $X_{[x]}$ is transitive,
  so that the map $[\blank]$ is well-defined.
  
  For proving surjectivity, assume $P:X/G$, that is,
  $P$ is a $G$-subtype of $X$ such that $X_P$ is transitive. 
  We have to show that there exists an $x:X(\sh_G)$ such that $P=[x]$.
  By the connectivity of $\BG$ it suffices to show $P(\sh_G)=[x](\sh_G)$
  for some $x$, and this follows from the transitivity of $X_P$. 
...
map any $o:X/G$ to its preimage $\setof{x : X(\sh_G)}{o =_{X/G} [x]}$.
