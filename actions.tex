...
\begin{definition}\label{def:orbit-stabilizer}
  ...
  \item Define $G\cdot x \defeq \setof{y : X(\shape_G)}{[x] =_{X/G} [y]}$
    ...
    \MB{An \emph{orbit} is the orbit of $x$ for some $x:X(\sh_G)$.}\qedhere
  \end{enumerate}
\end{definition}
...
as sums of the respective fibers.\footnote{%
  ...
  MB: What means $G\cdot x$ \MB{(pointed at $x$)? Also, what is $G\cdot p$?
  Perhaps use $\inv{[\blank]}$ from \cref{lem:G/X-set-of-orbits}?})}
