...
between $\USymG$ and $X(\sh_G) \times \USymG_x$ that we define to be $L_H(f)$.%
\footnote{\label{ft:lagrange-conj} A minor modification of the above
implementation gives an equivalence $L'_H(f)$ between $\USymG$ and
$\sum_{y:X(\sh_G)}\USymG_y$, which does not refer to $x$.
This is sometimes more convenient, \eg in the proof of \cref{lem:burnside}.
Proof: conjugation $g\mapsto\inv f(y) g f(y)$ is an equivalence
...
\begin{tabular}{ccc} \toprule
 ...
 $\scriptstyle 0000$ & $\scriptstyle 0,1,2,3$ \\
 $\scriptstyle 1111$ & $\scriptstyle 0,1,2,3$ \\
 $\scriptstyle 0001,0010,0100,1000$ & $\scriptstyle 0$ \\
 $\scriptstyle 0111,1011,1101,1110$ & $\scriptstyle 0$ \\
 $\scriptstyle 0101,1010$ & $\scriptstyle 0,2$ \\
 $\scriptstyle 1100,0110,0011,1001$ & $\scriptstyle 0$ \\ \bottomrule
\end{tabular}
...
\begin{proof}
 ...
 \footnote{%
  ...
  \]
  Note that the latter type above reflects how
  ...
  We aim to apply Lagrange's \cref{con:lagrange},
  in particular \cref{ft:lagrange-conj}, with subgroups defined by $X_O$ and
  $x : X_O(\sh_G)$, for any orbit $O:X/G$. For this we have to give functions
  $f_x : \prod_{y:X_O(\sh_G)}\sum_{g:\USymG} g\cdot_{X_O} y = x$,
  for every $x:X_O(\sh_G)$.
  ...
  such that $g\cdot_{X_O} y = x$, \ie corresponding to the
  smallest number under the equivalence with the standard finite set.
  Applying \cref{con:lagrange}
  we get and equivalence between $\USymG$ and $\sum_{x : X_O(\sh_G)} \USymG_x$. 
