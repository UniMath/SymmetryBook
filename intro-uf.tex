...
\begin{definition}\label{lem:apd}
...
Recall from \cref{sec:natural-numbers} the definition of addition in $\NN$,
by $n+0\defeq n$ and $n+S(m)\defeq S(n+m)$. We also define
multiplication in $\NN$, setting $m\cdot 0\defeq 0$ and 
...
The functions $+$ and $\cdot$ are commutative, associative and distributive.
The relations $\leq$ and $<$ are transitive and preserved under $+$ and $\cdot$. 
...
Define the type $\min(P,n)$ expressing that $n$ is the smallest
natural number such that $P(n)$:
\[
\min(P,n) \defeq P(n) \times \prod_{m:\NN} (P(m) \to n\leq m)
\]
Then $\min(P,n)$ is also a proposition, and all $n$ such that $\min(P,n)$
are equal. Therefore the type $\sum_{n:\NN} \min(P,n)$
is also a proposition. 

For any function $d(n): P(n)\amalg\neg P(n)$ deciding $P(n)$ for each $n:\NN$, 
we can define a function $\mu_P:\NN\to \bool\times\NN$ that searches
for an $n$ such that $P(n)$. 
We set $\mu_P(0)\defeq(\yes,0)$ if $P(0)$, that is, if 
$d(0)\jdeq\inl{\_}$, and $\mu_P(0)\defeq(\no,0)$ else. 
We set $\mu_P(S(n))\defeq \mu_P(n)$ if $\mu_P(n)\jdeq(\yes,\_)$.
Otherwise, we set $\mu_P(S(n))\defeq(\yes,S(n))$ if $P(S(n))$,
and $\mu_P(S(n))\defeq(\no,S(n))$ else, using $d(S(n))$ to decide.
By design, $f$ `remembers' where it has found the least $n$.
We are now done with the computational part and the rest
is a correctness proof.

By triple induction on $n,k:\NN$ and $d(n): P(n)\amalg\neg P(n)$ one can show
\[
\mu_P(n)=(\yes,k)) \to (P(k)\times (k\leq n)).
\]
Note that there may not be an $n$ such that $P(n)$, but we can prove
\[
P(n)\to \sum_{k:\NN} (\mu_P(n)=(\yes,k)
\]
by induction on $n:\NN$. 
Since we also can show $\mu_P(n)=(\yes,k)\to \mu_P(n+m)=f(n)$,
we can conclude $P(n)\to \min(P,\snd(\mu_P(n)))$.
Since $\sum_{n:\NN} \min(P,n)$ is a proposition, 
...
\min(P) : \prod_{n:\NN}(P(n)\amalg\neg P(n)) \to 
           \Trunc{\sum_{n:\NN} P(n)} \to  \sum_{n:\NN} \min(P,n).
\end{align}
\end{definition}

In the above we have made the use of the witnesses of decidability
explicit. In the interest of readability, we will not continue
to do so. For example, in order to define a function $f:\NN\to\NN$
such that $f(n)$ takes value $1$ for $n=13$, and $0$ everywhere else,
we consider this very definition as sufficient and will not refer
to the decidability of $=$ in $\NN$. This remark applies to all
sets and decidable relations on them. We shall immediately put
this convention to good use in the proof a form of the so-called
...
that $f(n)<k+1$ for all $n<k+2$. Take $P(n)\defeq (f(n)=k)$
and compute $\mu_P(k+1)$ as in \cref{def:Niswellordered}.
If $\mu_P(k+1) = (\_,\no)$, then $f(n)<k$ for all $n<k+1$,
so we are done by IH. If $\mu_P(k+1) = (l,\yes)$, take 
$Q(n)\defeq (f(n)=k)\times (n\neq l)$ and compute $\mu_Q(k+1)$.
If $\mu_Q(k+1) = (m,\yes)$, then $l$ and $m$ fulfill the 
conclusion of the  lemma. If $\mu_Q(k+1) = (\_,\no)$,
then $l$ is the unique preimage of $k$ under $f$.
If $l=k+1$, then again $f(n)<k$ for all $n<k+1$ and
we are done by IH. Otherwise, we swap $l$ and $k+1$,
that is, we define $g(k+1) = f(l) = k$, $g(l) = f(k+1)< k$,
and $g(n)=f(n)$ everywhere else. Then $g$ fulfills the
conditions for IH, so we get $n,m<k+1$ such that $n\neq m$
and $g(n)=g(m)$. Swapping back we get the desired result for $f$.
\end{proof}
...
We define the $k$-fold iteration of $f:X\to X$ inductively 
by $f^0 \defeq \id_X$ and $f^{k+1}\defeq f\circ f^k$. 
If $f$ has an inverse $f^{-1}$ we write
$f^{-n}$ for $(f^{-1})^n$. DO WE NEED THIS EARLIER?
