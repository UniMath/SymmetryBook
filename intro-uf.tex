...
is by explicit definition. Two elements $f,g : \prod_{x:X} Y(x)$
are equal if they are pointwise equal, which can be expressed
using two product types $(\prod,\to)$: 
\[
(\prod_{x:X} f(x) = g(x)) \to f=g
\] 
This axiom is called `function extensionality'.
The opposite direction, $f=g \to \prod_{x:X} f(x) = g(x)$ can be
obtained by substituting $g$ for the rightmost $f$ in
$\prod_{x:X} f(x) = f(x)$. The latter can be obtained
by the explicit definition $h(x)\defeq \refl{x}$.
