...
then we are done by IH. Now let $n_0 = \mu_P(k+1) < k+1$,
so $f(n_0)=k$. If also $f(k+1)=k$ then we are done.
If $f(k+1)<k$, then we define $g$ by $g(n_0)=f(k+1)$
and equal to $f$ everywhere else. Then IH applies
to $g$ and we get  $n,m < k+1$ such that 
$n\neq m$ and $g(n)=g(m)$. If both $n,m$ are different
from $n_0$ then we are done. Otherwise, say $n=n_0$,
then $m\neq n_0$, and we are done by $f(m)=g(m)=g(n)=g(n_0)=f(k+1)$.
\end{proof}
...
The embedding function $i:\NN\to\zet$ is defined by induction,
setting $i(0) = z_0$, $i(S(n)) = pos(n)$.

The successor function $s:\zet\to\zet$ is defined inductively
setting $s(z_0) = pos(0)$, $s(pos(n)) = pos(S(n))$,
$s(neg(0))=z_0$, $s(neg(S(n)))=neg(n)$. By induction on $n:\NN$
one proves $s(i(n))=i(S(n))$, so that one can say that $s$ extends $S$
...
\]
The inverse of $s$ is called the predecessor function $s^{-1}$.

Yet another equivalence is the negation function $-:\zet\to\zet$
inductively defined by setting $-pos(n)=neg(n)$,... 
\end{definition}
