...
and which may be denoted by $T(a)$.  For that reason, we may also say, when introducing a family of types, we may say instead that \emph{for each
element} or \emph{for all elements} $x$ of type $X$ we are given a type $T(x)$.
...
and which will be denoted by $e(a)$.  For that reason, we may also say, when introducing a family of elements, we may say instead that \emph{for each
element} or \emph{for all elements} $x$ of type $X$ we are given an element $e(x)$ of type $T(x)$.
...
with values in $\bool$
requires the Law of the Excluded Middle (\cref{pri:lem}).

In this book we do not always assume the Law of Excluded Middle,
