...
We introduce the following convenient definitions.
\begin{align*}
 1 & \defeq S(0) \\
 2 & \defeq S(1) \\
 n+1 & \defeq S(n)
...
defining $X(m)$ to be $\NN$ for all $m$ and by setting $f(0) \defeq 1$ and setting $f(S(m)) \defeq S(m) \cdot f(m)$.  The latter definition
applies rule P4 with $1$ for $a$ and the function $n \mapsto S(m) \cdot n$ for $g_m$, for all $m$. Application of definition shows, for example,
that $f(2)$ and $2$ are the same by definition, because they both reduce to $S(S(0))$.

The following definition is another example of recursion that will be used later. 

\begin{definition}\label{def:n-fold-iteration}
Let $X$ be a type and $f$ a function from $X$ to $X$. 
We define by induction the $n$-fold iteration of $f$
by setting ....
\end{definition}
...
One well-known equivalence, which is its own inverse, 
is the \emph{negation} function ${-}:\zet\to\zet$
...
The \emph{successor} function $s:\zet\to\zet$ is defined inductively
setting $s(z_0)\defeq pos(0)$, 
