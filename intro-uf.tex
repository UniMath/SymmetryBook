...
In the case where, for each $x:X$, we are only interested in 
$T(x)$ having elements or not, and not in the elements themselves,
we may say that `having elements or not' is invariant under 
substitution of equals.
Once we have seen how we do logic in type theory this can be rephrased
...
$T(x)$ could be $\bool\to S(x)$. Then $f:T(x)$ specifies a pair
of elements of $S(x)$, and $\trp^T e f : T(y)$, specifies a pair
of elements of $S(y)$. We see that the structure `pair of elements of $S(x)$'
is \emph{transported} along the proof $e:x=y$ 
to a `pair of elements of $S(x)$'. In such cases transport will of course
not be ignored.
