...
$p^{n+1}\defeq p^n\ct p$. Negative powers $p^{-n}$ are defined
as $(p^{-1})^n$.
...
We come back to this function in \cref{sec:equivalences}.
...
We come back to \cref{sec:sum-types} characterizing equality in a sum type.  
...
\[
(\_\,,\_)^= : 
(\sum_{i:a_1 = a_2} \trp_{B,i}(b_1)= b_2) \to (a_1,b_1)=(a_2,b_2)
\] 
...
We recall that any $f:A\to B$ induces a family of functions 
$\ap{f}: (a=a') \to (f(a)=f(a'))$ for all $a,a':A$.
In saying `for each $\ap{f}$' we refer to all members of this family. 


\begin{lemma}\label{lem:ap-equivalence}
If $f:A\to B$ is an equivalence, then so is each $\ap{f}$.
\end{lemma}
\begin{proof}
...
Let $x_0,x_1:X$, $q:f(x_0)=f(x_1)$ and recall $\ap{f}: x_0=x_1 \to f(x_0)=f(x_1)$.
Then we have:
\[
((x_0,\refl{f(x_0)})=_{f^{-1}(f(x_0))}(x_1,q))\equiv
\ap{f}^{-1}(q) \defeq \sum_{p:x_0=x_1} q =\ap{f}(p)
\]
\end{lemma}
\begin{proof}
%Observe that the type on the right is the fiber of $\ap{f}$ at $q$.
Put $y_0\defeq f(x_0)$. By \cref{lem:isEq-pair=},
...
\refl{y_0}\ct \ap{f}(p)$. Now the lemma follows.
\end{proof}

\begin{lemma}\label{cor:fib-vs-path}
...
\item If $X$ is inhabited and $Y$ is connected, then $f$ is an equivalence if and only if each $\ap{f}$ is an equivalence;
\item If ?...?, then all fibers of $f$ are sets if and only if each $\ap{f}$ is an injection.
\end{enumerate}
\end{lemma}
\begin{proof}\mbox{}%

(1) Follows directly from \cref{lem:fib-vs-path} since $(x,p)=(x',p')$ is
equivalence to $(x,\refl{x}) = (x',p^{-1}\ct p')$.

(2) By (1) and \cref{cor:inj+connected}.
