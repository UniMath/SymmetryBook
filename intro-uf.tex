...
\index{composition!of paths}\index{concatenation!of paths}
(The notation $q\circ p$ fits particularly well when the paths are
related to functions and the composition of the paths is related to the
composition of the related functions in the same order, e.g., see transport below.)%
...
\section{Excluded middle and propositional resizing}
...
LEM is usually assumed in ordinary mathematics. However, type theory is born
in a constructivist tradition which aims at developing as much mathematics as
possible without assuming LEM altogether. Following this idea, we will
...
\begin{remark}
  We will later see a weaker version of LEM, called the Limited Omniscient
  Principle (\cref{LPO}), that is often enough.
\end{remark}
...
In this book we do not always assume LEM, and there are some more subtleties.
...
\begin{definition}\label{def:predicate}
  ...
  Then call $P$ a \emph{predicate}\index{predicate} on $T$.\footnote{%
...
\end{lemma}
\begin{proof}
  TODO\footnote{May want to go via the equivalence
    $((a,p) =_{f^{-1}(b)} (a',p')) \equiv
    \ap{f}^{-1} (p' \circ \inv p)$,
    where $p : b = f(a)$ and $p' : b = f(a')$,
    so $p' \circ \inv p : f(a) = f(a')$.}
\end{proof}

Given a predicate $P$ on $T$, we can form the type
$\sum_{t:T}P(t)$ which comes with a map
$\fst : \biggl(\sum_{t:T}P(t)\biggr) \to T$.
...
\begin{definition}\label{def:subtype}
  ...
  \[
    \mathrm{Sub}_T \defeq
    \sum_{S:\UU}\sum{f:S\to T}\isinj(f).
  \]
\end{definition}
Given a predicate $P$ on $T$,
...
\end{lemma}
By Hedberg's \cref{thm:hedberg}, it follows in particular that $\NN$ is a set.
