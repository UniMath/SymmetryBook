...
also say $f: \prod_{m:\NN} X(m)$. Phrases with `family' and
`for all' will continue to be used, always referring to functions
...
if $Z(x)$ is a type for every element $x:X$,
and $f:\prod_{x:X} Z(x)$ is a dependent function, then
the function $\apd{f}$ is a dependent function defined similarly:
\[
 \apd{f}: \prod_{e:x=y} \trp_{Z,e}(f(x)) = f(y);~\apd{f}(\refl{x})\defeq\refl{f(x)}
\]
The latter definition is correct since $\trp_{Z,\refl{x}}(f(x)) \jdeq f(x)$. 
\end{definition}
...
$\snd: \prod_{z: \sum _{x:X} Y(x)} Y(\fst z)$,
$\snd(a,b)\defeq b$.
\end{enumerate}
...
with the latter as the \emph{type} that $t(f(x))$
proves contractible. Hence the center of contraction
$\fst(t(f(x))$ is equal to $(x,refl{f(x)})$, and hence
...
This large abuse of notation is compatible with the following
`practical use' of UA: 
\begin{quote}
...
\begin{theorem}\label{thm:sets-inductive-types}
...
This we do by double induction on $n,m:\NN$. In the two (of four) 
cases in which $n\neq m$ we have $\Eq_\NN(n,m)=\bn{0}$ 
and use Ex Falso to define $g_{n,m}: \Eq_\NN(n,m)\to (n=m)$.
In the two other cases we simply set $g_{n,n}(\_)=\refl{n}$.
By induction on $p: n=m$ we get $g_{n,m}(f_{n,m}(p))=p$.
By double induction on $n,m:\NN$ we get that $f_{n,m}(g_{n,m}(x))=x$
for all $x:\Eq_\NN(n,m)$ (two of four cases are by Ex Falso
and the others are identifications in a propositional type).

We are now in position to prove that each $f_{n,m}$ is an equivalence,
that is, that the fiber $f_{n,m}^{-1}(x)$ is contractible for all $x:\Eq_\NN(n,m)$.
This is now easy using $g_{n,m}$. First, we can use $g_{n,m}(x)$
paired with the proof above as the center of contraction.
Second, if $f_{n,m}(p)=x$, then $p= g_{n,m}(f_{n,m}(p))= g_{n,m} (x)$.
Finally, second components of pairs in the fiber have propositional 
type and are therefore equal if in the same type.
\end{proof}
...
\begin{xca}\label{xca:finite-types}\hspace{1in}
  \begin{enumerate}
  \item Denote in full all elements of $\bn{0} $, $\bn{1} $, and $\bn{2}$.
  \item Show (using UA) that $\bn{1} =\true$, $\bn{2} =\bool$.
  \item Show by double induction on $m,n:\NN$ that $n=m$ if $\bn{m}=\bn{n}$.
  \end{enumerate}
\end{xca}
...
by \cref{xca:finite-types}. But $\NN$ is a set by \cref{XX}, 
