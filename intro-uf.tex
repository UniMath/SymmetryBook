...
\section{Equivalences, universes and univalence}
...
all inverse images $f^{-1}(y)$ contain exactly one element.
...
we have an element $t$ satisfying
\[
t: \prod_{y:Y} \iscontr(f^{-1}(y)),
\]
denoted by $\isEq(f)$. If so, we can define an inverse $f^{-1}: Y\to X$
by setting $^{-1}(y) = \fst(t(y))$ and prove that $f^{-1}$ is also
an equivalence. (EXERCISE)
In such a case we define the types
$X$ and $Y$ to be \emph{equivalent}, denoted by $X\equiv Y$. 
More precisely, since there could be more than one equivalence
between two types, we define:
\[
...
The following lemma gives an important example of an equivalence.
...
equivalent types are equal. One way to understand this is
that an equivalence $f : X \to Y$ makes it possible to
transport properties and structure from $X$ to $Y$
(and vice versa using $f^{-1}$). We deliberately use the term transport
here since it is precisely transport in the sense of
\cref{def:transport} along the identification
$X = Y$ stipulated by $f$ (there could be more than one).
...
(HOPEFULLY: one universe suffices for the purpose of this book.)
\end{definition}

REMOVE THIS PARA IF CONSIDERED A DISTRACTION.
...
The answer is: the next universe. Although not done explicitly
in this book, one can set up a hierarchy of universes to achieve
the foundational ideal of `everything has a type'.
...
\trp_{id_\UU, e} : X \to Y
\]
Voevodsky's \emph{univalence axiom} states that $\trp_{id_\UU, e}$
as a function of $e$ is an equivalence from $X = Y$ to $X\equiv Y$.
TBE
\end{definition}
...
In \cref{sec:prop-sets-grpds} we have seen that propositions are
