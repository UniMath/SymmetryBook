...
%of the sum $m+n$. NOT SO (left versus right recursion)
...
One possible answer is: $\sum_{X:\UU}(\zet=X)$.
Another possibility is $\sum_{X:\UU}(X=X)$.
The following exercise shows that these two candidates
do not have the property that $X=X$ is equivalent to
$(\zet,s) = (\zet,s)$ for all $X=(\zet,s)$.
It is for this reason that in \cref{sec:gsets}
yet another choice will be made.
