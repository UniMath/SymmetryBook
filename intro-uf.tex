...
Such an expression will be called a \emph{family of elements of $Y$} \index{family} parametrized by the elements of $X$.
...
Such an expression will be called a \emph{family of types} \index{family} parametrized by the elements of $X$.
...
variable}.\footnote{Students of calculus are familiar with the concept of dummy variable and are accustomed to using identities such as
$\int_a^b f(t)\,dt = \int_a^b f(x)\,dx$.}\index{dummy variable} This signifies that the name of the variable no longer matters, i.e., that $x
\mapsto e(x)$ and $t \mapsto e(t)$ may regarded as identical.
...
\item\label{P4} suppose we are given:
  (1) a family of types $X(m)$ depending on a variable $m$ of type $\NN$;
  (2) an element $a$ of $X(0)$; and
  (3) a family of functions $g_m : X(m) \to X(\Succ(m))$.
...
by the two declarations $f(0) \defeq a$ and $f(\Succ(m)) \defeq g_m(f(m))$, which cover both ways of introducing elements of
...
We may now define addition and multiplication of natural numbers by induction on $\NN$.  In both cases, the family of types $X(m)$ is defined by setting
...
We may also define the factorial function $\fact : \NN \to \NN$ by induction on $\NN$, setting $\fact(0) \defeq 1$ and setting
...
We finish this section with the definition of iteration by induction on $\NN$.  Let $X$ be a type, and suppose we have a function $e : X \to X$.  We
