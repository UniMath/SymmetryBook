...
A good way to state the recursion is that, for any proposition $P$,
precomposition with $\trunc{\_}$ is an equivalence
\[
(T \to P) \quad \to \quad (\Trunc{T} \to P).
\]
\end{definition}

One may ask about induction. Truncation is special in that
induction follows from the above recursion principle.
Proving this is a good illustration of how to use recursion.

\begin{lemma}\label{lem:prop-trunc-elim}
Let $P: \Trunc{T}\to\UU$ be such that $P(x)$ is a proposition for all $x:\Trunc{T}$.
If $g(t): P(\trunc{t})$ for all $t:T$, then there is $f: \prod_{x:\Trunc{T}} P(x)$
with $f(\trunc{t})\jdeq g(t)$ for all $t:T$.
\end{lemma}
\begin{proof}
By \cref{lem:prop-utils}(\ref{prop-utils-pi}), 
$\prod_{x:\Trunc{T}} P(x)$ is a proposition. We will define
a map $h: \Trunc{T}\to\prod_{x:\Trunc{T}} P(x)$. 
Then we set $f(x)\defeq h(x,x)$ and we are done.
By recursion it suffices to define $k(t,x): P(x)$ 
for $t:T$ and $x:\Trunc{T}$. Let $t:T$ and $x:\Trunc{T}$,
then $\trunc{t}: \Trunc{T}$ and $p: \trunc{t}=x$ for some $p$.
By assumption we have $g(t): P(\trunc{t})$, and we can transport
$g(t)$ along $p$ in the family $P$. Thus we
set $k(t,x)\defeq \trp_{P,p} g(\trunc{t}) : P(x)$.
\end{proof}
...
we get an element $a$ in $A$, so $f(a):B$. By $\prod_{x,y:B} \Trunc{x=y}$
...
$(\sum_{a:A}P(a), \fst)$.
Equivalence now follows by \cref{lem:weq-iso}.
\end{lemma}
...
$\Prop\defeq \sum_{X:\UU}\isprop(X)$ from \cref{sec:subuniverses}.
...
The above argument can be refined for each of the subuniverses
from \cref{sec:subuniverses}, and one can prove the following
