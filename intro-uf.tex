...
of type theory that has not been very visible until know, namely
...
we can define a function $f:\NN\to \bool\times\NN$ that searches
for an $n$ such that $P(n)$. 
We set $f(0)\defeq(\yes,0)$ if $P(0)$, that is, if 
$d(0)\jdeq\inl{\_}$, and $f(0)\defeq(\no,0)$ else. 
We set $f(S(n))\defeq f(n)$ if $f(n)\jdeq(\yes,\_)$.
Otherwise, we set $f(S(n))\defeq(\yes,S(n))$ if $P(S(n))$,
...
By triple induction on $n,k:\NN,~d(n): P(n)\amalg\neg P(n)$ one can show
\[
(P(n)\times (f(n)=(\yes,k))) \to (P(k)\times (k\leq n)).
\]
Note that there may not be an $n$ such that $P(n)$, but we can prove
\[
P(n)\to \sum_{k:\NN} (f(n)=(\yes,k))
\]
by induction on $n:\NN$. 
Since we also can show $f(n)=(\yes,k)\to f(n+m)=f(n)$,
we can conclude $P(n)\to \min(P,\snd(f(n)))$.
Since $\snd(f(n))$ is unique, we obtain the desired function

\[
\min(P) : \prod_{n:\NN}(P(n)\amalg\neg P(n)) \to 
           \Trunc{\sum_{n:\NN} P(n)} \to  \sum_{n:\NN} \min(P,n).
\]
COULD BE SIMPLIFIED A BIT IF $P : \NN\to\bn 2$.
\end{definition}
