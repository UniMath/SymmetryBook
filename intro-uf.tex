...
we can define
\[
\min(P,d) : \sum_{n:\NN} P(n) \to  \sum_{n:\NN} \min(P,n)
\]
by a bounded search. There are many ways to implement this,
we sketch one. Define 
\[
f:\NN\to \bool\times\prod_{n:\NN} (P(n)\amalg\neg P(n))
\] 
setting $f(0)\defeq(\yes,(0,d(0)))$ if $P(0)$, that is, if 
$d(0)\jdeq\inl{\_}$, and $f(0)\defeq(\no,(0,d(0)))$ else. 
Set $f(S(n))\defeq f(n)$ if $f(n)\jdeq(\yes,(\_,\_))$.
Otherwise, set $f(S(n))\defeq(\yes,(S(n),d(S(n))))$ if $P(S(n))$,
and $f(S(n))\defeq(\no,(S(n),d(S(n))))$ else, using $d(S(n))$ decide.
By design, $f$ `remembers' where it has found the least $n$ and the
proof that $P(n)$.
Now we can define $\min(P,d,n,p)\defeq f(n)$ having all
the desired properties but one: 


Now we can define for each $p: \Trunc{\sum_{n:\NN} P(n)}$ the function 
\[
\min_{P,d}(p) \defeq \fst(\min(P,d,p))
\]
which returns the smallest natural number $n$ such that $P(n)$ if
