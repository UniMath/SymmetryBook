...
\[
(\_\,,\_)^= : (\sum_{i:a_1 = a_2} \trp_{Y,i}(b_1)= b_2) \to (a_1,b_1)=(a_2,b_2),
\]
...
\begin{xca}\label{xca:isEq-pair=}
Show that the function $(\_\,,\_)^=$ from \cref{sec:sum-types} is an equivalence.
\end{xca}
...
Now $t(x)\equiv (\yes=x)$ by \cref{lem:weq-iso}.  
Since each type $t(x)$ is a proposition (see below
...
in \cref{sec:prop-trunc-logic} how to work with disjunction and 
...
proposition. The recursion principle is: 
for any proposition $P$, any function $g: T\to P$
yields a function $f:\Trunc{T}\to P$ 
satisfying $f(\trunc{t})\jdeq g(t)$ for all $t:T$.\footnote{%
Note that $f(\trunc{t})$ respects equality since $P$ is a proposition.
For the same reason we need not specify how $f$ acts on the identifications.}
A good way to state the recursion principle is that, 
...
One may ask about induction. Induction follows from the above recursion principle.
Proving this is a good illustration of logical reasoning in type theory.

\begin{lemma}\label{lem:prop-trunc-elim}
Let $P: \Trunc{T}\to\UU$ be a predicate such that $P(\trunc{t})$ for all $t:T$. 
Then we have $P(x)$ for all $x:\Trunc{T}$. 
\end{lemma}
\begin{proof}
The assumption amounts to having a function $g: \prod_{t:T} P(\trunc{t})$.
Note that $\sum_{x:\Trunc{T}} P(x)$ is a proposition. 
We will first define a map $h: \Trunc{T}\to\sum_{x:\Trunc{T}} P(x)$. 
By recursion it suffices to define $k(t) \defeq (\trunc{t},g(t))$ for all $t:T$.
Now let $x:\Trunc{T}$ and consider $\fst(h(x)): \Trunc{T}$
and $\snd(h(x)): P(\fst(h(x)))$. Since $\Trunc{T}$ is a
proposition, $\fst(h(x)) = x$ and we get $P(x)$ by substitution.
\end{proof}

Now we are ready to define logical $\vee$ and $\exists$, by
\[
(P \vee Q) \defeq \Trunc{P \amalg Q} \quad\text{and}
\quad (\exists x:T\,P(x)) \defeq \Trunc{\sum_{x:T}P(x)}.
\]

For example, for all $x:\Trunc{T}$ there exists $t:T$ such that $x=\trunc{t}$
follows immediately from \cref{lem:prop-trunc-elim}.
...
\end{xca}
% [\footnote{This tries to say that, nonfunctorially, a space is the disjoint union of its components.  Don't think we need it so it can probably be thrown away.}Assuming the appropriate form of axiom of choice \footnote{someone insert our choice}, prove that the path components define a family $A\to\UU$ factoring as $A\to||A||$ making sense of the equation ``$||A=\sum_{[a]:||A||}A_{[a]}||$''.] MB OUTCOMMENT
...
\begin{proof}
If $(x_1,p_1),(x_2,p_2) : \sum_{x:T} P(x)$, then by \cref{xca:isEq-pair=}
...
We formulate them in such that they amount to their
...
A function $f:A\to B$ is an \emph{insurjection}, or \emph{injective}, 
...
Being contractible is a proposition, so by \cref{lem:prop-trunc-elim}
we can drop the truncation in $\Trunc{\sum_{a:A} b=f(a)}$.
Now apply injectivity.
\end{proof}

\begin{corollary}\label{cor:inj+connected}
Let $A,B$ be types such that $\Trunc{A}$ and $\prod_{x,y:B} \Trunc{x=y}$.
Then any injection $f: A\to B$ is an equivalence. This holds
in particular if both $A$ and $B$ are connected. 
\end{corollary}
\begin{proof}
By \cref{lem:inj+surj} it suffices to show that $f$ is surjective.
This is a propostion, so by \cref{lem:prop-trunc-elim} and $\Trunc{A}$
...
\[
\preim: \quad\sum_{B:\UU}(B\to A)\quad\to\quad (A\to\UU) 
\] 
...
\item $(A\to\Set) \equiv \sum_{B:\UU}\sum_{f:B\to A}
