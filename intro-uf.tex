...
$m\cdot(n+1)\defeq (m\cdot n) + m$. Elementary properties of addition and
multiplication can be proved in type theory in the usual way.
We freely use them, sometimes even in definitions, leaving most of the
proofs to the reader.
\begin{definition}
\label{def:orderonN}
  Let $n,m:\NN$. We say that $n$ is less than or equal to $m$, and write $n\leq m$, if there is a $k:\NN$ such that $n+k=m$. This $k$ is unique and we say 
that $n$ is less than $m$, and write $n<m$, if this $k$ is not $0$. 
\end{definition}

We will prove the following lemma without using the 
Law of the Excluded Middle (LEM).
\begin{lemma}\label{lem:dec-eq+order-N}
The relations $=$, $\leq$ and $<$ on $\NN$ are decidable.
\end{lemma}
\begin{proof}
A useful function is a the \emph{predecessor} defined by
$p(0)\defeq 0$ and $p(S(n))\defeq n$, from which we get
that the successor function is injective. 
The lemma can now be proved by double induction.
\end{proof}

\begin{xca}\label{xca:eq+order-N}
Try your luck in type thoery by proving any of the following.
The relations $\leq$ and $<$ on $\NN$ are transitive and preserved under addition. 
We have $m\leq n$ if and only if $m<n$ or $m=n$ (so $\leq$ is reflexive).
Furthermore, $m\leq n$ and $n\leq m$ if and only if $m=n$,
and not $m<n$ and $n<m$.
\end{xca}
