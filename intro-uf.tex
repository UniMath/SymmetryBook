...
\begin{definition}\label{def:pathsoverpaths}
  ...
  allowing us to set $$\pathoverdisplay y Y {\refl x} {y'} \defeq (y = y').$$
\end{definition}
...
two pairs to be equal.  In this case, the following lemma gives the desired characterization.

\begin{lemma}\label{lem:isEq-pair=}
  Suppose we are given a type $X$, a family of types $Y(x)$ parametrized by the elements $x$ of $X$.
  For any elements $(x,y)$ and $(x',y')$ of $\sum _{x:X} Y(x)$,
  there is an equivalence of type $$\left ( (x,y) = (x',y') \right ) \weq \sum_{p:x=x'} \pathover y Y p {y'}.$$
\end{lemma}

Refer to \cref{def:pathsoverpaths} for the meaning of the type $\pathover y Y p {y'}$.
...
\begin{proof}
  ...
  $$ \Phi : \left ( \sum_{p:x=x'} \pathover y Y p {y'} \right ) \to ((x,y) = (x',y')). $$
  It may be defined by $$ \Phi (p,q) \defeq \apap g p q, $$
  or it may be defined directly by induction by setting $$ \Phi ( \refl x, \refl y ) \defeq \refl {(x,y)}. $$
  ...
  Applying \cref{lem:weq-iso}, we see that $\Psi$ and $\Psi$ are inverse equivalences, thereby obtaining the desired equivalence.
\end{proof}
...
One may verify that $\cast(p)(x) = \trp_{\id_\UU, p} (x)$, for $x : X$, by induction on $p$.
Here $id_\UU$ is the identity function $id_\UU$ on $\UU$, viewed as a family of types on $\UU$. 
As a corollary, one sees that $\trp_{\id_\UU, p}$, previously defined as a function, is an equivalence.
...
\begin{definition}\label{def:merely}
  If {\em A} is an English adjectival phrase encoded by a type $T$, then {\em merely A} is the English adjective encoded by the type $\Trunc A$.
\end{definition}

For example, a type is inhabited if and only if it {\em merely has an element}, and two elements of a connected type are {\em merely equal}.
