...
In {\em univalent mathematics}, types are used for everything.  There are ways to form new types from old ones to provide everything we need to
write mathematics.
...
In the following sections we will expose various other elementary sorts of types and elementary ways to make new types from old ones.
...
Here is an example of defining a function by recursion using induction for $\NN$.  We define the factorial function $f : \NN \to \NN$ by setting
$f(0) \defeq 1$ and setting $f(S(m)) \defeq (m+1) \cdot f(m)$.  One can infer that the function $g_m$ of rule (P4) is $n \mapsto (m+1) \cdot n$.
...
of $b=a$ from an element $e$ of $a=b$, for any $b$ and $e$.  By induction (letting $P(b,e)$ be $b=a$ in rule E3 above), it suffices to produce
an element of $a=a$; we choose $\refl(a)$ to achieve that.
...
One frequent use of elements of identity types is in {\em substitution}.  Let $X$ be a type, and let $P(x)$ be a family of types depending on a
...
consider the case where $b$ is $a$ and $e$ is $\refl(a)$ and to provide a function $P(a) \to P(a)$.  To achieve that, we provide the identity
function.
...
$\emptyset$, it suffices to consider no special cases (!).  Hence, every statement about an arbitrary element of $\emptyset$ can be proven.  As
...
there will be a type $\sum _{x:X} Y(x)$ whose elements are all pairs $(a,b)$, where $a:X$ and $b:Y(a)$.  Sums may be implemented by an inductive
definition, and they behave as one would expect.
...
parameter $x$ of type $X$, then there will be a type $\prod _{x:X} Y(x)$ whose elements serve as {\em families} of elements $b:Y(a)$, one for
each $a:X$.  A family is much like a function, but without a single codomain.  (FILL IN SOME PROPERTIES.)
...
products, or as a special case of sums.  (FILL IN SOME PROPERTIES.)
...
by $\prod_{a:X} \prod_{b:X} (a=b)$.  We shall call such a type a {\em proposition}, and its elements will be called {\em proofs}.
