...
Recall from \cref{sec:natural-numbers} the definition of addition on $\NN$,
...
The relations $\leq$ and $<$ are transitive, and $\leq$ is
preserved under $+$ and $\cdot$. 
...
we can define a function $\mu_P:\NN\to\NN$ that,
given input $n$ searches for a $k<n$ such that $P(k)$.
If $k<n$ with $P(k)$ exists, $\mu_P$ returns the least such $k$,
otherwise $\mu_P(n)=n$.
...
we can conclude $P(n)\to P_{\min}(\snd(\mu_P(n)))$.
