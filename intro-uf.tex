...
parameter $x:X$.  Suppose $x,y:X$ and $e:x=y$.  Then there is a function of type $T(x) \to T(y)$. We define one specific such function by induction on $e:x=y$.
\begin{definition}\label{def:transport} The function
\[ 
{\trp}^T_{X,x,y} : x=y \to T(x) \to T(y)
\]
is defined by path induction putting ${\trp}^T_{X,x,x}(\refl{x})\defeq \id_{T(x)}$.
\end{definition} 
