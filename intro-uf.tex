...
Here is an example of defining a function on $\NN$ by recursion.
We use addition and multiplication of natural numbers, which will be defined
in \cref{sec:more-on-N}.  Then we define the factorial 
function $\fact : \NN \to \NN$ by setting $X(m)$ to be $\NN$ for all $m$,
by setting $\fact(0) \defeq 1$, and by setting $\fact(S(m)) \defeq (m+1) \cdot \fact(m)$.
This definition applies rule P4 with $1$ for $a$,
and with the function $n \mapsto (m+1) \cdot n$ for $g_m$, for all $m$. 
Application of the definitions shows, for example,
that $\fact(2) \jdeq 2$, because both sides expand to $S(S(0))$.
...
by setting $e^0 \defeq \id_X$ and $e^{S(n)}\defeq e\circ e^n$.
\end{definition}
...
Conversely, a basic logical principle called {\em function extensionality}, which we adopt,
...
Let $X$ be a type, and let $Y(x),Z(x)$ be families of types indexed 
by a parameter $x:X$. A map $f$ of type $\prod_{x:X}(Y(x)\to Z(x))$
can be viewed as a family of maps $f(x): Y(x)\to Z(x)$ and is called a 
\emph{fiberwise} map. Then the \emph{totaliser} of $f$ is defined as
\[
...
Let conditions be as in \cref{def:fiberwise}. Let $x:X$ and $z:Z(x)$.
Then the functions $g$ and $h$ defined below are inverse equivalences
between $f(x)^{-1}(z)$ and $\tot(f)^{-1}(x,z)$.

Define $g: f(x)^{-1}(z) \to \tot(f)^{-1}(x,z)$ as follows.
For any $y:Y(x)$ and $p: z=f(x)(y)$, define $g(y,p)$ by induction on $p$. 
This induction instantiates $z$ with $f(x)(y)$ and $p$ with $\refl{f(x)(y)}$.
Thus it suffices to set $g(y,\refl{f(x)(y)}) \defeq ((x,y),\refl{f(x)(y)})$,
where the right-hand side has the right type $\tot(f)^{-1}(x,f(x)(y))$ since
$\tot(f)(x,y)\jdeq (x,f(x)(y))$.

We define $h_t: \tot(f)^{-1}(t) \to f(\fst(t))^{-1}(\snd(t))$ for
all $t:\prod_{x:X} Z(x)$, then $h$ is obtained as $h_{(x,z)}$.
It suffices to define $h_t((x,y),q)$ for any $y:Y(x)$ and $q: t= \tot(f)(x,y)$.
Induction on $q$ instantiates $t$ with $\tot(f)(x,y) \jdeq (x,f(x)(y))$ and 
we can simply set $h_t$ to take the value $(y,\refl{f(x)(y)})$ 
of type $f(x)^{-1}(f(x)(y))$ in this case.
\end{lemma}
...
\begin{corollary}\label{cor:fiberwise}
bla
\end{corollary}
...
\begin{definition}\label{def:idtoeq}
  For types $X$ and $Y$ in $\UU$ and a path $p : X = Y$, we define an equivalence $\cast_{X,Y} (p) : X\equiv Y$ by induction on $Y$ and $p$, setting
...
inverse function of $\cast_{X,Y}$ to $f$, if no confusion will result.  Thus the identity $\cast (\ua (f)) = f$ holds.

In certain contexts, we will want to mix paths and equivalences freely, so, if it will not cause confusion, we may abbreviate $\ua(f)$ to $f$
for $f : X \equiv Y$, and we may abbreviate $\cast(p)$ to $p$ for $p : Y = Z$.  Thus we will be able to write such expressions as $p \cdot f : X
= Z$ and $p \circ f : X \equiv Z$.
