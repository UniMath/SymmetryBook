...
an identity $f=g$ can be produced from a family of identities of type $f(x) = g(x)$ indexed by the elements $x$ of $X$.
...
By an easy induction on $p$ we have the following relation with transport.

\begin{definition}\label{lem:pathsover-transport}

Let conditions be as in \cref{def:pathsoverpaths}.
Then $\pathover y Y p {y'}$ by induction on $x'$ and $p$, thereby reducing to the case
  where $x'$ is $x$ and $p$ is $\refl x$,
  rendering $y$ and $y'$ of the same type,
  allowing us to set $$\left( \pathoverdisplay y Y {\refl x} {y'} \right) \defeq (y = y').$$
\end{definition}
...
We may also write $\ua (f)$ more briefly as $\etop f$.  Thus there are identities $\etop {\ptoe p} = p$ and $\ptoe {\etop f} = f$.
...
Let $A$ be a connected type, and suppose for every (or for some) $a:A$ that $a=a$ is a proposition.
...
\section{How to deloop a group morphism}
