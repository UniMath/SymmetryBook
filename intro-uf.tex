...
parameter $x:X$.  Suppose $x,y:X$ and $e:x=y$.  Then there is a function of type $T(x) \to T(y)$. We define one specific such function by path
induction, by taking its value to be the identity function on $T(x)$ in the case of $\refl{x}:x=x$.
...
\end{definition} 
The function thus defined may be called \emph{the transport function in the type family $T$ along the path $e$}, or, less verbosely, {\em
  transport}.  We may also simplify the notation to just $\trp_e$.

When the types $T(x)$ may have more than one element, we may regard an element of $T(x)$ as providing additional {\em structure} on $x$.  In
that case, we will refer to the transport function $T(x) \to T(y)$ as {\em transport of structure} from $x$ to $y$.  By contrast, when the types
$T(x)$ may have at most one element, we may regard an element of $T(x)$ as providing a proof of a property of $x$.  In that case, the transport
function $T(x) \to T(y)$ provides a way to establish a claim about $y$ from a claim about $x$, so we will refer to it as {\em substitution}.  In
...
Product types are not inductive, there are no constructors and
