...
If $Y(x)$ is a family of types parametrized by the elements $x$ of a type $X$, and $a$ and $a'$ are elements of type $X$, then after identifying
$a$ with $a'$ it is possible to ``identify'' an element of $Y(a)$ with an element of $Y(a')$.  That is the idea of the following definition.
...
\item The types $\emptytype$, $\true$, $\bool$, and $\NN$ are elements of every universe $\UU$.
\item If $X : \UU$ and $a,b:X$, then $(a = _X b) : \UU$.
\item Given a type $X:\UU$ and a family of types $Y(x):\UU$ parametrized by variable $x$ of type $X$, the sum $\sum_{x:X} Y(x)$ and the product
