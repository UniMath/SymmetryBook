...
\begin{exercise}
  Let $X$ be a type and let $a,b,c,d:X$ be elements.  Establish the following identities.
  \begin{enumerate}
  \item For $p:a=b$, show that $p \ct \refl b = p$.
  \item For $p:a=b$, show that $\refl a \ct p = p$.
  \item For $p:a=b$, $q:b=c$, and $r:c=d$, show that $(p \ct q) \ct r = p \ct (q \ct r)$.
  \item For $p:a=b$, show that $p^{-1} \ct p = \refl b$.
  \item For $p:a=b$, show that $p \ct p^{-1} = \refl a$.
  \item For $p:a=b$, show that $(p^{-1})^{-1} = p$.
  \end{enumerate}
\end{exercise}
...
\begin{definition}\label{def:subtype}
  ...
  \[
    \mathrm{Sub}_T \defeq
    \sum_{S:\UU}\sum_{f:S\to T}\isinj(f).\qedhere
  \]
\end{definition}
