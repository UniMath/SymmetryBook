...
\item\label{prop_utils_product} $\prod_{a:A} P(a)$ is a proposition;
...
If $p,q : A\to P$, then $p=q$ is proved by firstr observing that $p$ and $q$
...
special case of (\ref{prop_utils_product}).
...
\item\label{level_n_utils_product} $\prod_{a:A} Z(a)$ is a type of level at most $n$;
