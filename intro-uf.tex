...
\begin{xca}\label{xca:fiber-of-pr1}
  Suppose $X$ is a type and $Y(x)$ is a family of types indexed by a parameter $x$ of type $X$.
  Consider the projection map $\fst : \sum_x Y(x)$ defined above, and suppose $x : X$.
  Show that the map $j : Y(x) \to \fst^{-1}(x)$ defined by $j(y) \defeq ((x,y),\refl x)$ is an equivalence.
  (Hint: use \cref{lem:weq-iso}.)
\end{xca}
