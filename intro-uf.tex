...
\begin{proof}
  ...
  \]
  and for the other cases we set, say, $T(0, S(m), p) \defeq T(S(n), 0
  ,p) \defeq \bn{0}$.  Next we prove $T(n,m,p)$ for all $n,m$, and $p$
  by induction on $p$, leaving us with $T(n,n,\refl{n})$ for all $n :
  \NN$, which we in turn prove by distinguishing cases on $n$.  Both
  the case for $0$ and for $S(n)$ hold by reflexivity, where in the
  successor case we use $\refl{n}$ for $q$ and note that $\ap S
  \refl{n} \jdeq \refl{S(n)}$.

  We can now prove $p = \refl{n}$ for all $p : n = n$ by induction
  on~$n$.  In the base case this is simply $T(0,0,p)$.  And in case $n
  \jdeq S(m)$ we get from $T(S(m),S(m),p)$ that $p = \ap{S} q$ for
  some $q : m = m$.  By induction hypothesis we have $e : q =
  \refl{m}$ and thus also
  \[
    p = \ap{S} q = \ap{S} \refl{m} \jdeq \refl{S (m)}
  \]
