...
\begin{lemma}\label{lem:isEq-pair=}
  ...
  there is an equivalence of type $$\left ( (x,y) = (x',y') \right ) \weq \sum_{p:x=x'} \pathoverdisplay y Y p {y'}.$$
\end{lemma}
...
\begin{proof}
  ...
  $$ \Phi : \left ( \sum_{p:x=x'} \pathoverdisplay y Y p {y'} \right ) \to ((x,y) = (x',y')). $$
...
\begin{proof}
  ...
  By \cref{lem:UNKNOWN} it suffices to establish an equivalence of type 
  ...
  The two sides are equal by definition, so the identity equivalence provides what we need.  
\end{proof}
