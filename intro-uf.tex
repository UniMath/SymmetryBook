...
The following exercise identifies the type of paths over with the type 
of paths using transport.

\begin{xca}\label{def:pathover-trp}\label{xca:pathover-trp}
In the context above, define by induction on $p$ an element of
$\left(\pathover y Y p {y'}\right) = \left( \trp{Y,p}(y)=y'\right)$.
\end{xca}
...
setting $f(a,\refl{a},b) \defeq q$ for all $b:B(a,\refl{a})$.
...
and let $(x_1,p_1),(x_2,p_2) : \sum_{x:T} P(x)$.
...
say with center $c_q$. Mapping $q$ to $\pathpair q c_q$ defines an inverse of $\ap{\fst}$.
Clearly $\ap{\fst}\pathpair q c_q = q$. Also ...
...
\begin{xca}\label{xca:component-same-loops}
...
Lemma \cite[7.3.12]{hottbook} give an equivalence from 
$\trunc{t}_0 = \trunc{t'}_0$ to $t=t'$ for all $t,t:T$.}
A good way to state this recursion principle is that, 
