...
the plane'', or ``consider a line $L$ of the plane''.  After that introduction, one may say that the \emph{type} of $P$ is \emph{a point of the
...
$X$, we may produce an identification of type $f(x) = g(x)$.  This can be proved by induction, as follows.
...
\begin{definition}\label{def:ptw}
  ...
  \]
  setting ${\ptw}_{f,f}(\refl{f}) \defeq x \mapsto \refl{f(x)}$.
  \footnote{ Alternatively, we can consider for any $x:X$ the evaluation function $\ev_x : \big(\prod_{x:X} Y(x)\bigr) \to Y(x)$, defined by
    $\ev_x(f) \defeq f(x)$.  Then one could define ${\ptw}_{f,g}(p,x) \defeq \ap{\ev_x}(p)$.  The two functions are not equal by definition.}
\end{definition}
