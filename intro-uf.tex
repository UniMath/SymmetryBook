...
One expresses the declaration that an \emph{element} $a$ is of \emph{type} $X$ by writing $a:X$.
\footnote{The corresponding notation in mathematics based on \emph{set theory} is $a \in X$.}
\index{element}
\glossary(2:){${:}$}{element judgment}
Using that notation, each variable is introduced along with a declaration of the type of thing it will refer to, and
