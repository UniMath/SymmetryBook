...
for any predicate $P:\Trunc{T}\to\UU$, 
...
We are now ready to define logical $\vee$ and $\exists$, by
\[
...
follows immediately from \cref{def:prop-trunc}.
...
\begin{xca}\label{xca:inj-via-ap-f}
Prove that $f$ above is an injection if and only if
...
Combining \cref{xca:inj-via-ap-f} with \cref{cor:inj+connected}
we have the following converse of \cref{lem:ap-equivalence}
for connected types.%\footnote{
%By \cref{xca:component-connected} we may even take $a'\jdeq a$.}

\begin{lemma}\label{lem:ap-eq-connected-eq}
Let $A,B$ be connected types, $f:A\to B$, and assume
$\ap{f}: (a=a') \to (f(a)=f(a'))$ is an equivalence for all $a,a':A$.
