...
For convenience, we may abbreviate $\apap g p q$ to $g(q)$.
...
\begin{lemma}\label{lem:isEq-pair=}
  ...
  there is an equivalence of type $$(x,y) = (x',y') \weq \sum_{p:x=x'} \pathover y Y p {y'}.$$
\end{lemma}
...
\begin{definition}\label{def:idtoeq}
  For types $X$ and $Y$ in $\UU$ and a path $p : X = Y$, we define an equivalence $\idtoeq_{X,Y} (p) : X\equiv Y$ by induction on $Y$ and $p$, setting
  $\idtoeq_{X,Y} (\refl X) \defeq \id_X : X \equiv X$.  The result is a function \[ \idtoeq_{X,Y} : (X = Y) \to (X\equiv Y). \]
\end{definition}
  
In expressions such as $\idtoeq_{X,Y}(p)$ we may abbreviate $\idtoeq_{X,Y}$ to $\idtoeq$ if no confusion will result.

One may verify that $\idtoeq(p)(x) = \trp_{\id_\UU, p} (x)$, for $x : X$, by induction on $p$.
...
$\idtoeq_{X,Y}$ above is an equivalence for all $X,Y:\UU$.
Formally, we postulate the existence of an element \[ \ua_{X,Y} : \isEq(\idtoeq_{X,Y}).\]
\end{definition}

For an equivalence $f: X\equiv Y$, we will adopt the notation $\ua(f) : X = Y $ to denote $\idtoeq_{X,Y}^{-1}(f)$, the result of applying the
inverse function of $\idtoeq_{X,Y}$ to $f$, if no confusion will result.  Thus the identity $\idtoeq (\ua (f)) = f$ holds.
...
\begin{xca}
  If $P$ is a proposition, then show, for any $p, q: P$, that $p=q$ is contractible.
\end{xca}
