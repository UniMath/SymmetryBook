...
Clearly, $f(g(y)) \eqto y$ by unfolding all definitions.
...
\item a constructor identifying $x \eqto y$  for all $x,y:\Trunc{T}$.
\end{enumerate}
...
from $x:\Trunc{T}$ we may deduce $\exists_{t:T} x \eqto \trunc{t}$.
...
\begin{definition}\label{def:non-empty}
  ...
  if we have an element of $\Trunc{A}$.\footnote{%
    ...
    $\neg(A \eqto \emptytype)$, which is equivalent to
    $\neg\neg A$.}
\end{definition}

\begin{definition}\label{def:connected}
  Let $A$ be a type. If $a:A$, then the subtype $A_{(a)}\defeq \sum_{x:A}\Trunc{a \eqto x}$
  is called the \emph{(connected) component} of $a$ in $A$.
  We say that elements $x,y$ of $A$ are \emph{in the same component}
  if we have an element of $\Trunc{x \eqto y}$.
...
that is, $\Trunc{\sum_{a:A} b \eqto f(a)}$.\footnote{%
...
\begin{definition}\label{def:decidable-set}\index{decidable set}
  A type $A$ is called a \emph{decidable set} if the equality relation $x \eqto y$
