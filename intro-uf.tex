...
Equality of two elements $(x,y),(x',y')$ of $\sum _{x:X} Y(x)$ is inductively defined in \cref{sec:identity-types}, as for any other type, but
...
\begin{lemma}\label{lem:isEq-pair=}
  ...
  Then there is an equivalence of type $$(x,y) = (x',y') \weq \sum_{p:x=x'} \pathover y Y p {y'}.$$
\end{lemma}

\begin{proof}
  Consider the function $$g : \prod_{x:X} ( Y(x) \to \sum _{x:X} Y(x) ) $$ defined by $$ g(x)(y) \defeq (x,y). $$
  Given elements $x,x':X$, $y:Y(x)$, and $y':Y(x')$, application of $g$ to paths over paths, defined in \cref{def:applfun2}, yields a function
