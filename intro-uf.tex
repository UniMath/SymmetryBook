...
Such an expression will be called a \emph{family of elements of $Y$} parametrized by the elements of $X$.
...
It gives a family of real numbers parametrized by the natural numbers (also called a \emph{sequence} of real numbers).
...
Such an expression will be called a \emph{family of types} parametrized by the elements of $X$.
...
Here's an example: we let $n$ be a natural number and $T$ be the type of regular $n$-sided polygons in the plane.  It gives a family
of types parametrized by the natural numbers.  One of the members of the family is the type of all triangles in the plane.
...
Here is some concise notation for constructing a function without naming it.
Suppose we wish to construct a function of type $X \to Y$.  For that purpose, it suffices to consider
an element $x:X$ and to construct an expression $e:Y$, all of whose components have been already been given a meaning;
here the variable $x$ is regarded also as having already been given a meaning.  Given that, the notation $x \mapsto e$ will denote the function
that sends each $x$ to $e$.  To apply it to a particular element $a:X$, one replaces each instance of $x$ in $e$ by $a$, as usual.

To provide another example of making new functions in the usual way, we first introduce notation for making and for using \emph{definitions}.\index{definition}
...
that $x$ and $y$ are \emph{the same by definition}.
...
Good motivation for this notation for equality is provided by a remark made by Robert Recorde in 1557:
...
Think of a congruence of a triangle with itself.  
...
The notation $q\circ p$ for path composition fits our intution particularly well when the paths are related to functions and the composition of
...
The transport functions behave as expected: $\trp {e\circ e'} = \trp e \circ \trp {e'}$.
In words: transport along the composition $e\circ e'$ is the composition of the two
transport functions (to be proved by induction on $e$).
