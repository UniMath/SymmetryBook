...
and $ns(h,p)$ is called a \emph{naturality square}.%\footnote{%
...
Lemma \cite[7.3.12]{hottbook} gives an equivalence from 
...
In this section we fix a universe $\UU$. Often we run into types that are not
in $\UU$ directly, but are nevertheless equivalent to types in $\UU$.
\begin{definition}\label{def:ess-loc-small}
  We say that a type $A$ is \emph{essentially $\UU$-small} if there
  (purely) is a small type $X:\UU$ and an equivalence $A \equiv
...
To show that a type is locally $\UU$-small we have to give a reflexive relation
$\mathrm{Eq}_A : A \to A \to \UU$ inducing equivalences
$(x=_Ay) \equiv \mathrm{Eq}_A\,x\,y$ by path induction.

\begin{xca}
  Show that $\UU$ is locally $\UU$-small, and investigate
  the closure properties of locally $\UU$-small types.
...
\begin{principle}
  ...
  {\bf Propositional Resizing} \index{Propositional resizing}
  For any pair of nested universes $\UU:\UU'$, the injection
  $\Prop_\UU \to \Prop_{\UU'}$ is an equivalence.
\end{principle}
\begin{remark}
  %ARE WE ALWAYS ASSUMING IMPREDICATIVITY? IF SO, WHERE DO WE SAY THIS?
...
\begin{principle}
  ...
  {\bf The Replacement Principle} \index{Replacement principle}
  For any map $f : A \to B$
...
The replacement principle can be proved using the join construction of the image,
cf.~\cite{Rijke-Join}.
