...
One expresses the statement that an \emph{element} $a$ is of \emph{type} $X$ by writing $a:X$.\index{element}\glossary(1a){${:}$}{element judgment}
...
\glossary(2a){$\protect\to$}{function type}
...
\glossary(1c){$\jdeq$}{definitional equality}
...
\glossary(2b){$\mapsto$}{``maps to'', function definition}
...
\glossary(0b){$\circ$}{function composition}
...
\item\label{E2} for any type $X$ and for any element $a$ of it, there is an element $\refl a$ of type $a =_X a$
  ...
  \glossary(refl){$\protect\refl{a}$}{reflexivity}
...
\glossary(p1){$p^{-1}$}{reverse identification, path inverse}
\end{definition}
...
\glossary(pq){$p\protect\ct q=q\cdot p=qp=q\circ p$}{path concatenation}
...
\begin{definition}\label{def:transport} The function
  ...
  \glossary(trp){$\protect\trp[T]{e}$}{transport function}
\end{definition} 
...
\glossary(ap){$\protect\ap{f}$}{action on paths}
\end{definition}
...
\begin{definition}\label{def:pathsoverpaths}
  ...
  \glossary(1e){$=^Y_p$}{path-over type}
...
\glossary(ptoe){$\protect\ptoe p$}{cast along a path between types}
which we also use to denote only the function from $X$ to $Y$.\footnote{%
    ...
    an element $x$ of $X$ is cast in the role of a $Y$
    as directed by the path $p : X = Y$.
...
\glossary(ftop){$\protect\etop f$}{$\protect\ua(f)$}
Thus there are identities $\etop {\ptoe p} = p$ and $\ptoe {\etop f} = f$. Also, $\overline{\id_X}=\refl{X}$
