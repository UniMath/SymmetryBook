...
\begin{enumerate}[label=(E\arabic*),ref=(E\arabic*)]\label{rules-for-equality}
  ...
  \item\label{E4} suppose we are given:
    \begin{enumerate}
    \item a type $X$ and an element $a:X$;
    \item a sequence of variables $b,e,s_1:S_1(b,e),\dots,s_n:S_n(b,e)$ consisting of a variable $b$ of type $X$,
      a variable $e$ of type $a =_X b$, and perhaps some further variables $s_i$,
      where the type $S_i$ of $s_i$ may depend on the variables to the left of it in the sequence; and
    \item a family of types $Y$ parametrized by variables $$s_1:S_1(a,\refl a),\dots,s_n:S_n(a,\refl a).$$
    \end{enumerate}
    Then from those data we are provided with a family of types $Z$ parametrized by the variables $b,e,s_1:S_1(b,e),\dots,s_n:S_n(b,e)$.
    Moreover, $Z(a,\refl a,\dots) \jdeq Y$.
\end{enumerate}

The type $a =_X b$ may be called an \emph{equality type}, an \emph{identity type}, or simply an \emph{equation})\index{equation|see{identity type}}.
...
We will refer to rules \ref{E3} and \ref{E4} as ``induction for equality''.  To signal that we wish to apply one of them,
e may announce that we argue \emph{by induction on $e$}.
...
We will see our first example of the use of rule \ref{E4} in \cref{def:pathsoverpaths}.
...
\begin{definition}\label{def:pathsoverpaths}
  ...
  We proceed by induction on $a'$ and $p$ (i.e., by application of rule \ref{E4}), which reduces us to the case where $a'$ is $a$ and $p$ is $\refl a$,
