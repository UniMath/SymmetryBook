...
parameter $x$ of type $X$. Let $x,y$ be of type $X$ and let $i$ be of type $x=y$.
Then there is a function of type $T(x) \to T(y)$. 
We define one specific such function as follows.
\begin{definition}\label{def:transport} 
For $x,y: X$, define by identification induction the function
\[ 
{\trp}^T_{x,y} : x=y \to T(x) \to T(y)
\]
setting ${\trp}^T_{X,x,x}(\refl{x})\defeq \id_{T(x)}$.
\end{definition} 
