...
\begin{enumerate}[label=(E\arabic*),ref=(E\arabic*)]\label{rules-for-equality}
  \item\label{E1}
    for any type $X$ and for any elements $a$ and $b$ of it, there is a type $a \eqto b$;%
...
The type $a \eqto b$ may be called an \emph{equality type} or an \emph{identity type}.
...
In certain circumstances, to be discussed later, we will refer to an element $i$ of $a \eqto b$ as a \emph{proof}\/\footnote{Here we override
  ...
  element of $a \eqto b$.} that $a$ is equal to $b$.%
\index{proof}
...
The chief novelty of univalent mathematics is that the basic logical notion of equality, as implemented by the types $a \eqto b$, is carefully
...
We will refer to rule \ref{E3} as ``induction for equality''.  To signal that we wish to apply it, we may announce that we argue \emph{by induction on $e$}.
...
Intuitively, the induction principle for equality amounts to saying that the element $\refl a$ ``generates'' the system of types $a \eqto b$, as $b$
...
\begin{xca}\label{xca:path-groupoid-laws}
  ...
  \begin{enumerate}
    \item For $p:a \eqto b$, construct an element of type $p \ct \refl b \eqto p$.
    \item For $p:a \eqto b$, construct an element of type $\refl a \ct p \eqto p$.
    \item For $p:a \eqto b$, $q:b \eqto c$, and $r:c \eqto d$, construct an element of type $(p \ct q) \ct r \eqto p \ct (q \ct r)$.
    \item For $p:a \eqto b$, construct an element of type $p^{-1} \ct p \eqto \refl b$.
    \item For $p:a \eqto b$, construct an element of type $p \ct p^{-1} \eqto \refl a$.
    \item For $p:a \eqto b$, construct an element of type $(p^{-1})^{-1} \eqto p$.
    \qedhere
  \end{enumerate}
\end{xca}
...
An element $q : \pathover y Y p {y'}$ is called a \emph{path} from $y$ to $y'$ \emph{over} $p$.\index{path over}\footnote{\label{ft:path-over-pic}%
...
identification
$\po_p: \left(\pathover y Y p {y'}\right) = \left( \trp[Y]{p}(y)=y'\right)$ in $\UU$,
...
\begin{definition}
  ...
  \[
  (X\equiv Y) \defeq \sum_{f:X\to Y} \isEq(f).\qedhere
  \]
\end{definition}
...
\begin{lemma}\label{lem:weq-iso}
  Let $X,Y$ be types. For each equivelence $f: X\to Y$, we have a function $g: Y\to X$ such that for all $x:X$ we have $g(f(x))=x$ and for all
  $y:Y$ we have $f(g(y))=y$. Conversely, if we have such a function $g$, then $f$ is an equivalence.
\end{lemma}
\begin{proof}
...
\begin{lemma}\label{lem:eqandcovofconntypes}
  ...
  for all $a,a':A$.\footnote{%
    ...
    if and only if we have the implication $(f(a) = f(a')) \to (a = a')$,
...
if and only if $\ap{f}: (a=a) \to (f(a)=f(a))$ is an equivalence.
\end{enumerate}
\end{corollary}
\begin{proof}
(1) For $n=-2$ this is \cref{lem:eqandcovofconntypes}
...
\begin{theorem}\label{thm:unique-fact-0image}
  Show that the $0$-image of $f : A \to B$ induces a factorization $f = i\circ p$
  \[
    \begin{tikzcd}
      A \ar[rr,"f"]\ar[dr,"p"'] & & B \\
      & \im(f)\ar[ur,"i"'] &
