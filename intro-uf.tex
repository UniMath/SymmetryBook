...
We put \cref{lem:equivalence-via-inverse} immediately to good use.
...
By \cref{lem:equivalence-via-inverse} if follows that $f$
is an equivalence.
\end{proof}
...
to obtain a simpler, equivalent type. 
...
\begin{lemma}\label{lem:transport-in-Id-type}
...
\begin{xca}\label{xca:transport-in-Id-type}
Prove the following special cases of \cref{lem:transport-in-Id-type},
where $Y\jdeq X$ and $a,b$ in $X$:
\begin{enumerate}
\item $\trp_{x\mapsto a=b,\,e} (i) = i$;
\item $\trp_{x\mapsto a=x,\,e} (i) = i*e$;
\item $\trp_{x\mapsto x=b,\,e} (i) = e^{-1}*i$;
\item $\trp_{x\mapsto x=x,\,e} (i) = e^{-1}*i*e$ (also called \emph{conjugation}).
\end{enumerate}
\end{xca}

There is also a dependent version of \cref{lem:transport-in-Id-type},
...
by computing $\mu_P(N+1)$ as in \cref{def:Niswellordered}.
...
Another application of \cref{def:Niswellordered} is a
...
Now set $k\defeq\mu_P(n)$ as in \cref{def:Niswellordered}.
...
object classifier stuff

\section{Subtypes and Decidability --- TBD}
\label{sec:subtype}
