...
\end{remark}
((we're going to need the following since it is going to reduce the burden of showing that $S^1$ is equivalent to the alternative model unto showing that it induces an equivalence $\zet\to\zet$.  The argument for a general group and the torsors over its abstract group is VERY similar: proofread))
\begin{lemma}
  Let $f:X\to Y$ be a function, $x_0:X$ and assume $Y$ is connected.  Then $f$ is an equivalence if and only if the induced function from $x_0=x_0$ to $f(x_0)=f(x_0)$ is.
\end{lemma}
\begin{proof}
  We want to show that all fibers of $f$ are contractible, but since $Y$ is connected it is enough to consider the fiber $f^{-1}(y_0)\defequi\sum_{x:X}y_0=f(x)$ over $y_0\defequi f(x_0)$.  
