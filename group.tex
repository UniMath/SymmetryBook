...
\begin{definition}\label{def:group-symmetries}
  ...
  We adopt the notation $\G$ for the type $\loops \BG$ of symmetries in $G$;
...
\begin{example}\label{ex:cyclicgroups}
  ...
  (equivalently, of the ``$m$-fold \covering'' over the circle, $(\Sc,\dg{m})$)
  ...
  $0,1,\dots,m-1$ of $\bn n$,
  and under this correspondence composition of symmetries correspond to
  addition modulo $m$,
  because
  This group is called the \emph{cyclic group of order $m$},
  $\CG_m \defeq \Aut_\Cyc((\bn m,\zs))$.\footnote{%
