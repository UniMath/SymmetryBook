...
\begin{example}
  ...
  Let us spell out the data needed to give an identity between two group homomorphisms $f,f':\Hom(G,H)$.  We clearly must have a
...
$$
so that -- since $BG$ is connected and $Bf_\div(z)=Bf'_\div(z)$ is a set -- $J(z)$ is determined by $J(\pt_G)$ which again is determined by $p_f$ and $p_{f'}$.  This shows that $f=f'$ is a proposition and that $\Hom(G,H)$ is a set.
...
% {\color{blue}DELETE
% \subsection{Actions on preimage}
% \label{sec:actonfiber}
% Let $G$ be a group and let $X:BG\to\Set$ be a $G$ set.
% Assume given an element $x_0:X(\pt_G)$ and that $\sum_{z:BG}X(z)$ is connected.  Let $\pt_H\defequi (\pt_H,x_0)$ and define $BH_\div\defequi\sum_{z:BG}X(z)$, so that the first projection is an inclusion $i:BH\to BG$ of a subgroup.

% % \begin{lemma}
% %   \label{lem:conjactiontrivontotalspace}
% %   Let $f:\Hom(H,G)$ be a group homomorphism.  Then the composite
% %   $$\xymatrix{BH\ar[r]^f& BG\ar[r]^{\coker(G)}&\Set}
% %   $$
% % a trivial $H$-set; ``the induced $H$-action on $\coker(f)$ is trivial''.
% % \end{lemma}
% % \begin{proof}
% %   If $g:\pt_G=\pt_G$, then the action on $\coker(f)$ is given by ``$\preinv g$'': considered as an equivalence it sends an element $(x,p)$ (where $x:BH$ and $p:\pt_G=f(x)$) to $(x,p\, g^{-1})$.
% %   If $h:\pt_H=\pt_H$ we must construct an element in $(x,p)=(x,p\,f(h)^{-1})$.


% % the induced identity $\preinv f(h):\coker(f)=\coker(f)$ is $\refl{}$.  Considered as an equivalence, $\preinv g$ sends $(x,p)$ to $(x,p\,g^{-1})$
% % \end{proof}



% Let $f:(X,x_0)\to (Y,y_0)$ be a pointed map (where $p_f:y_0=f(x_0)$ is part of the data).  Consider the preimage
% $$\preim_f:Y\to\UU$$
% given by $\preim_f(y)=f^{-1}(y)$.
% Letting $\mathcal G$ be the abstract group given by the identity type $y_0=y_0$ we get that $f^{-1}(y_0)$ is an $\mathcal G$-type via the action $(\y_0=\y_0)\times f^{-1}(y_0)\to f^{-1}(y_0)$ sending $(g,(x,q))$ to $(x,q\, g^{-1})$ (where $x:X$, $q:y_0=f(x)$ and $g:y_0=y_0$). 

% More concretely, if $(Y,y_0)$ is a group,\ie $Y$ is $BG_\div\defequi\concr\mathcal G$ pointed at $\pt_G\defequi y_0$, then $\preim_f:BG\to\UU$ is a $G$-type.
...
\end{proof}
Not only is the type of subgroups  of $G$ a set, it is in a natural way (the value at $\pt_G$ of) a $G$-set which we denote by the same name
$$%\typesubgroup_G:BG\to\Set,\qquad 
\typesubgroup_G(x)\defequi \sum_{H:\typegroup}\sum_{f:\Hom(H,G)}\isset(Bf^{-1}(x)).$$
\begin{definition}
...
(COMEBACK 190408 FIX UP THIS REMARK)
  It is worthwhile to study this action a bit further: given an identity $g:\pt_H=\pt_H$ and a subgroup $(H,i_H,!)$ of $G$, to what subgroup ``$(gHg^{-1},i_{gHg^{-1}})$'' is $(H,i_H,!)$ taken to by (the equivalence corresponding to) $g$?  

First consider the equivalence $\tilde g:(Bi_H)^{-1}(\pt_H)\we (Bi_H)^{-1}(\pt_H)$ induced by $g$: the image $\tilde g(\pt_H,p_{Bi_H})$ is a pair consisting of a point which we call $\pt_{gHg^{-1}}:BH$ and an identity $p_{Bi_{gHg^{-1}}}:Bi_H(\pt_{gHg^{-1}})=\pt_G$.

In sum, $(gHg^{-1},i_{gHg^{-1}},!)$ is our name for the subgroup with 
$$B(gHg^{-1})_\div\defequi BH_\div$$ pointed at $\pt_{gHg^{-1}}$ with $i_{gHg^{-1}}:\Hom(gHg^{-1},G)$ given by $Bi_{gHg^{-1}}\defequi Bi_H:BH_\div\to BG_\div$ and $p_{Bi_{gHg^{-1}}}:Bi_H(\pt_{gHg^{-1}})=\pt_G$.
...
We say that the homomorphism $f$ is \emph{surjective}\index{surjective! group homomorphism} if $(Bf)^{-1}(\pt_{G'})$ is connected (which is equivalent to $\coker f$ being contractible).
...
A subgroup is said to be \emph{normal}\index{normal} if it is the kernel of a homomorphism.


The \emph{image}\index{image} of $f$ is the subgroup of $G'$ given by $B\image f\defequi \sum_{z:BG'}\coker f(z)$.  The induced homomorphism $\tilde f:\Hom(G,\image f)$ is given by sending $x:BG$ to $B\tilde f(x)\defequi (Bf(x),|x,\refl{Bf(x)}|)$. 
\end{definition}
% \begin{remark}
...
\begin{definition}
  \label{def:pullback}
  Let $f:B\to D$ and $g:C\to D$ be two maps with common target.  The \emph{pullback}\index{pullback} of $f$ and $g$ is the type 
...
\begin{definition}
  ...
  Let $f:\Hom(H,G)$ and $f':\Hom(H',G)$ be two homomorphisms with common target.  The \emph{pullback}\index{pullback!of groups} $H\times_GH'$ is the group obtained as the (pointed) component of $\pt_{H\times_GH'}\defequi(\pt_H,\pt_{H'},p_{f'}p_f^{-1})$ of the pullback $BH\times_{BG}BH'$ (where $p_f:\pt_G=f(\pt_H)$ is the name we chose for the data displaying $f$ as a pointed map).
If $f$ and $g$ are injective homomorphisms, then the pullback is called the \emph{intersection}\index{intersection! of subgroups} and if the context is clear denoted simply $H\cap H'$.
\end{definition}
\begin{xca}
