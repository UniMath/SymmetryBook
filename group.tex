...
$$\princ G:BG\to\UU,\qquad\princ G(z)\defequi(\pt_G=z)$$ is a $G$-set (or $G$-type) called the \emph{principal $G$-torsor}.  We've seen this family before in the guise of the ``universal covering $P_{\pt_G}$'' of \cref{def:universalcover}!  
...
There is nothing sacred about starting the equality in $\pt_G$: if $y:BG$, then $P_y:BG\to\UU$ is also a $G$-set (type) and if $q:y=y'$, then the preferred equality between $P_y$ and $P_{y'}$ sends $p:y=z$ to $pq^{-1}:y'=z$.  As a matter of fact, \cref{lem:BGbytorsor} will identitfy $BG$ and the type of $G$-torsors via the map sending $y:BG$ to $P_y$ using the full transport structure of the identity type $P_yz\defequi(y=z)$.
...
Conversely, if $X$ is any type \emph{and} we have a homomorphism $G\to\Sigma_X$ (in other words, a pointed map $BG\to B\Sigma_{X(\pt_G)}%\defequi(\UU_{(X(\pt_G))},X(\pt_G)
$), then the composite
...
$$T:BG\to\typetorsor_G,\qquad T(z)\defequi P_z\defequi(x\mapsto(z=_{BG}x))$$
is an equivalence.
...
For $q:y=z$,  the transport $T(q):P_y=P_z$ is given by  sending $p:y=x$ to
...
Conversely, by the induction principle for the identity type, %$T(y)(x)\defequi (y=x)y=x$,  
a function $f:P_y\to P_z$ is given by its value $f(\refl y):P_zy\defequi (z=y)$ and in general, for $p:y\to x$ we have by transport that $f(p)$ is the composite $p\,f(\refl y)$.  In other words the map $T:(y=z)\to(P_z=P_y)$ is an equivalence.
...
$$\alpha_G:\Hom(G,\Sigma_{\pi_G=\pt_G})$$  
...
Useful notation:\footnote{move somewhere. Also, 190218, there are many things that I have not yet had time to improve in the remainder of the chapter}
...
\end{align*}
((In notation used elsewhere, ``$\preinv(q)(p)=P_q(p)$, $\post(p)(q)=P_x(p)(q)$ and $\adjoint(q)(p)=\Ad(p)(q)$'', but this may be confusing since some of these are seen as secondary (\eg in\footnote{why doesn't xspace work?} $P_x$ the $x$ has been largely fixed before) and some primary))
...
\begin{enumerate}
\item forming abstract group of a group and 
...
The \emph{principal $G$-torsor} $\princ G^\abstr$ is the $G$-set consisting of the underlying set $\mathrm{under}(G)$ together with the homomorphism $G\to\abstr(\Sigma_{\mathrm{under}(G)})$ given by $g\mapsto \mathrm{ua}(\cdot g^{-1})$ ((explain)).

The type of \emph{$G$-torsors} is
$$\typetorsor_G^\abstr\defequi\sum_{S:\Set_G^\abstr}||\princ G=S||.$$
\end{definition}
\begin{example}
...
\begin{example}\label{ex:qG}
  If $G$, then for any $x:BG$ the set $\pt_G=x$ has a natural structure of an $\abstr(G)$-torsor by means of 
...
\begin{definition}
  If $G$ is an abstract group, then the \emph{concrete group $\concr(G)$ associated with $G$} is the group (given by the pointed connected groupoid) $(\typetorsor_G^\abstr,\princ G)$.
\end{definition}
...
by $$
q_G(z)=((\pt_G=z),\preinv%:(\pt_G=\pt_G)\to ((\pt_G=z)=(\pt_G=z))
,!).$$
%(the $!$ expresses that $\preinv:(\pt_G=\pt_G)\to ((\pt_G=z)=(\pt_G=z))$ underlies a homomorphism of abstract groups $\abstr(G)\to\Sigma_{\pt_G=z}$, \ie that $\preinv(\refl{\pt_G})=\refl{\pt_G=z}$ and that $\preinv(q_0\,q_1)(p)=p(q_0q_1)^{-1}=pq_1^{-1}q_0^{-1}=\preinv(q_0)(\preinv(q_1)(p))$ (here you see why it was important to include the inverse!) and that the $\abstr(G)$-set $(\pt_G=z,\preinv)$ is in the component of $\princ {\abstr(G)}$).
\end{definition}

\begin{lemma}
  \label{lem:Groupsareidentitytypes}Let $G$ be an abstract group.  
...
\begin{xca}
  ...
  Let $G=(S,e,\mu,\iota)$ be an abstract group.  Then that underlying set of $\abstr(\concr(G))$ is $\princ G^\abstr=_{\typetorsor^\abstr_G}\princ G^\abstr$.  
...
$$r_G:S\to  \sum_{p:S=S}\prod_{q,s:S}(p(s\,q^{-1})=p(s)\,q^{-1}),\qquad r_G(u)\defequi(u\cdot\,,!)
$$  
is an equivalence of sets, which we by univalence turn into an equality.  The abstract group structure of $\abstr(\concr(G))$ is given by it being the symmetries of $\princ G^\abstr$, or translated to $\sum_{p:S=S}\prod_{q,s:S}(p(s\,q^{-1})=p(s)\,q^{-1})$ that of $S=S$ of the first projection.  
Hence our identity $r_G$ underlies an identity of abstract groups
$$r_G:G=_{\typegroup^\abstr}\abstr(\concr(G)).$$
\end{xca}

We use this this to show that every abstract group encodes the symmetries of something essentially unique.
\begin{theorem}
  \label{lem:Groupsareidentitytypes}Let $G$ be an abstract group.  
Then
