...
To the extent that it is needed, show the characterizing properties of kernels, images and cokernels.  % In particular, a subgroup is normal iff it is a fixed point under the conjugate action.  

 Also show counting results for the finite group part. 
\end{lemma}
...
$\mathrm{ev}_{X_{\pt_G}}:(X_{\pt_G}=X_{\pt_G})\to X_{\pt_G}(\pt_G)$ which we in \cref{lem:conistrans} proved was an equivalence since $X_{\pt_G}$ is transitive.  Evoking univalence we get an identification of subgroups between the kernel and $(\mathrm{ass}(N),i_N,!)$.
\end{proof}
