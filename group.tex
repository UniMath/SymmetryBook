...
\subsection{The concrete vs. abstract homomorphisms via torsors.}
...
is the $G$-set of homomrophisms from $H$ to $G$.
...
$$\sum_{H:\typegroup}\sum_{\phi:\Hom^\abstr(\abstr(H),\abstr(G))}\isprop(\phi^{-1}(e_G))$$ with action $g\cdot(H,\phi,!)\defequi(H,c^g\,\phi,!)$ for $g:\abstr(G)$, where  $c^g:\abstr(G)=\abstr(G)$ is conjugation as defined in \cref{ex:conjhomo}.
\end{lemma}
...
A subgroup is said to be \emph{normal}\index{normal} if it is the kernel of a surjective homomorphism.
\end{definition}

\begin{definition}
\label{def:image}\label{def:surjective}
The \emph{image}\index{image} of $f$ is the subgroup of $G'$ given by $B\image f\defequi \sum_{z:BG'}\coker f(z)$ and the first projection $B\image f\to BG'$ (plus the fact that $\coker f(z)$ is a set).  
...
\begin{lemma}
  \label{lem:kerandcoker}\footnote{((WRITE)) \color{blue}THE REST OF THE CHAPTER IS IN FLUX AND I DID NOT GET TO FIX ALL I WANTED TO FIX BEFORE I HAD TO QUIT FOR NOW.  IT CONTAINS KNOWN NONSENSE}
...
Show the characterizing properties of kernels, images and cokernels.  In particular, a subgroup is normal iff it is a fixed point under the conjugate action.  

 Also show counting results for the finite group part.  Need notation for order/cardinality.
\end{lemma}
...
\begin{xca}
  If $\mathcal G$ is an abstract group and $\mathcal H$ and $\mathcal K$ are abstract subgroups  Then the intersection $\mathcal H\cap\mathcal K$ is the abstract subgroup of $\mathcal G$ with underlying set \footnote{COMEBACK/define}.
\end{xca}
\begin{lemma}
...
%  \item The 
and induced homomorphism $H/N\cap H\to G'$ is injective
  % \item If $H$ and $G'$ are finite with coprime cardinalities, then $H$ is a subgroup of $N$.
%  \end{enumerate}
  \begin{proof}
Let $i:\Hom(H,G)$ be the inclusion, and express concretely $BN$ as $\sum_{y:BG}\pt_{G'}=Bfy$
%    \begin{enumerate}
%    \item 
Writing out the definition of the pullback and using that for each $x:BH$ the type $\sum_{y:BG}y=Bix$ is contractible, we get an equivalence between $BN\times_{BG}BH$ and the preimage of $\pt_{G'}$ of the composite $B(fi):BH\to BG'$
$$B(fi)^{-1}(\pt_{G'})\defequi\sum_{x:BH}\pt_{G'}=B(fi)x.$$  
 By definition, the intersection $N\cap H$ is a certain pointed component of the pullback, and under the equivalence with the preimage, the intersection corresponds to the component of $(\pt_H,Bf(p_i)\,p_f):\sum_{x:BH}\pt_{G'}=B(fi)x$.  Since (by definition of the composite of pointed maps) $p_{fi}\defequi Bf(p_i)\,p_f$ we get that the intersection $N\cap H$ is identified with the kernel of the composite $fi:\Hom(H,G')$.
%    \item 
...
{\color{blue}\tiny Don't actually seem to need at present; hence put on hold
\begin{lemma}
  \label{lem:iso2}
...
\end{lemma}
\begin{proof}
  Consider the $H$ action of $H$ on $G$, \ie the $H$-set $\tilde G:BH\to\Set$ with $\tilde G(x)\defequi(\pt_G=Bi(x))$.  Note that composing with the structure identity $p_i:\pt_G=Bi(\pt_H)$ gives an equivalence $\tilde G(\pt_H)\equiv (\pt_G=\pt_G)$.
  Lagrange's \cref{thm:lagrange} says that the action of $H$ is free \footnote{\cref{thm:lagrange} doesn't say this at present: fix it} (and so all orbits are equivalent to the $H$-set $\tilde H(x)=(\pt_H=x$) the equivalence $X(\pt_H)\simeq\sum_{x:\tilde G/H}\mathcal O_x$ gives that $G/H\defequi \tilde G/H$ and $H$ are finite (as sets) and that $|G|=|G/H|\cdot|H|$.\footnote{somewhere: prove that if $A$ is a finite set and $B(a)$ is a family of finite sets indexed over $a:A$, then $\sum_{a:A}B(a)$ is a finite set of cardinality $\sum_{i:\bn n}|B(f(i))|$ for any $f:\bn n=A$, hence if $m=|B(a)|$ for all $a$ then $|\sum_AB(a)|=n\cdot m$.}


Finally, if $|H|=|G|$, then $|G/N|=1$ and so the set $G/N$ is contractible.  Since we were considering a subgroup, the preimage $Bi^{-1}(\pt)$ is equivalent to the set $G/H$.\end{proof}
    ...
    \end{corollary}
    \begin{proof}
      By Lagrange's counting \cref{lem:Lagrangeascounting} a subgroup of $C_p$ has order dividing $p=|C_p|$, \ie either $1$ or $p$.
    \end{proof}
...
\end{corollary}
\begin{proof}
  By \cref{lem:whatSylow2needs} the induced homomorphism from $H/N\cap H$ to $G'$ is injective, and so Lagrange's counting \cref{lem:Lagrangeascounting} gives that the cardinality of $H/N\cap H$ divides the cardinality of $G'$.  

Since $|H|$ and $|G'|$ are coprime this implies that $|H|$ and $|H/N\cap H|=|H|/|N\cap H|$ are coprime, which implies that $|N\cap H|=|H|$.  Since $N\cap H$ is a subgroup of $H$ this implies that $N\cap H=H$, or in other words, that $H$ is a subgroup of $N$ ((elaborate)).
\end{proof}
...
\begin{definition}
  ...
  Let $p^n$ be the largest power of $p$ which divides the order of $G$.  A subgroup of $G$ of order $p^n$ is called a \emph{$p$-Sylow subgroup}\index{Sylow subgroup} of $G$.
\end{definition}
\begin{lemma}
...
{\color{blue}(WRITE OUT AND MOVE TO WHEN NORMAL IS DEFINED)


\begin{definition}
  \label{def:normalizer}
{\color{blue}\footnote{TO BE MOVED TO AFTER the chapter on symmetry (need Burnside -  a $G$-set splits into orbits - etc) has been covered.  
...
If $(K,i_K,!)$ is another subgroup containing a conjugate of $(H,i_H,!)$, we say that $(H,i_H,!)$ is \emph{subconjugate} to $(K,i_K,!)$.}%end color
\end{definition}
...
((finish by writing that normals are kernels))
...
\end{lemma}
\begin{proof}
  Remember that $N_G(H)$ is the stablizer subgroup of $(H,i_H,!)$ under the conjugation action, so we need to show that if $h:\pt_H=\pt_H$, then there is an identity between $(H,i_H,!)$ and $c^{i_H(h)}(H,i_H,!)$.   
