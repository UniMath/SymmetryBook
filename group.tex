...
\begin{proof}%
  ...
  % Suppose one has a equivalence $f:(x=x) \to (y=y)$ where $x$ and $y$
  % are the designated points of simply connected 2-types $X$ and
  % $Y$. Then $\Phi:X\to Y$ such that $\Trunc{\loopspace\Phi = f}$?
  % Consequence if this is true: if $f$ is an equivalence then
  % $\loopspace \Phi$ also and then $\Phi$ is an equivalence. For all
  % $x':X$, one can consider the type of all those $y':Y$ and those
  % elements $f':(x=x') \to (y=y')$ such that taking any $p:x=x'$, one
  % gets that $f'$ is $f$ skewed by $p$: more precisely there is a
  % $2$-path $\alpha_p:f'(p \blank) = f'(p)f(\blank)$. We can try to
  % prove that this is contractible for every $x':X$, which by
  % connectedness of $X$ boils down to the contractibility at $x$ only.
  % \begin{align*}
  %   T(x') &\weq \sum_{y':Y}\sum_{f':(x=x')\to(y=y')}
  %           \sum_{\alpha:\prod_{p:x=x'} f'(p\blank) = f'(p)f(\blank)}
  %           \\ &\prod_{p,p':x=x'}\prod_{\chi:p=p'}
  %           \alpha_{p'}(f'(\chi)\cdot f(\blank)) \alpha_p = f'(\chi\cdot\blank)
  %   \\
  %   T(x)  &\weq \sum_{y':Y}\sum_{f':\setTrunc{(x=x)\to(y=y')}}
  %           f'(\blank)=f'(\settrunc{\refl x})\settrunc{f(\blank)} \\
  %         &\weq \sum_{y':Y}\setTrunc{(y=y')}
  % \end{align*}
  % \begin{align*}
  %   &\hphantom{\weq} \sum_{y':Y}\sum_{f':(x=x)\to(y=y')}
  %     \prod_{p:x=x}\prod_{\ell:x=x}\Trunc{f'(p\ell)=f'(p)f(\ell)}\\
  %   &\weq \sum_{y':Y}\sum_{f':(x=x)\to(y=y')}
  %     \prod_{p:x=x}\prod_{\ell:x=x}\Trunc{f'(\ell)=f'(\refl x)f(\ell)}\\
  %   &\weq \sum_{y':Y}\sum_{f':(x=x)\to(y=y')}
  %     (x=x) \to \prod_{\ell:x=x}\Trunc{f'(\ell)=f'(\refl x)f(\ell)}\\
  %   &\weq \sum_{y':Y}\sum_{f':(x=x)\to(y=y')} \prod_{\ell:x=x}\Trunc{f'(\ell)=f'(\refl x)f(\ell)}\\
  % \end{align*}
  % Two elements $(y', f')$ and $(y'',f'')$ in the last type. Their identity type is equivalent to
  % $\sum_{q:y'=y''}q\cdot f'(\blank) = f''(\blank)$ and we can prove there is an inhabitant
  
  
  % Let $T : \BB {(\loopspace {(A,a)})} \to \UU$ be the type family define by
  % \begin{displaymath}
  %   T(X,\varphi) \defequi
  %   \sum_{a':A}    \left(
  %     \sum_{\alpha:\setTrunc{(a'=a')=X}} \left(
  %       \prod_{p:a=a'}
  %       \varphi = \alpha \cdot \settrunc{p\cdot \blank \cdot \inv p}
  %     \right)
  %   \right)
  % \end{displaymath}
  % Let us prove that $T(X,\varphi)$ is contractible for any
  % $(X,\varphi):\BB(\loopspace (A,a))$. By connectedness of
  % $\BB(\loopspace (A,a))$, we only have to prove the contractibility of
  % $T(a=a,\settrunc{\refl{a=a}})$. However,
  % \begin{displaymath}
  %   T(a=a,\settrunc{\refl{a=a}}) \jdeq
  %   \sum_{a':A} \left(
  %     \sum_{\alpha:\setTrunc{(a'=a')=(a=a)}} \left(
  %       \prod_{p:a=a'}
  %       \alpha = \settrunc{\inv p\cdot \blank \cdot p}
  %     \right)
  %   \right)
  % \end{displaymath}
  % To be able to
  ...
  contractible.
  % Indeed, for any $p:a=a$, we shall prove the
  % proposition
  ...
  path $p:a=a'$ and a path $q:\varphi = (x\mapsto p x)$. We have a
  good candidate for $p$, namely $\varphi(\refl a):a=a'$. However we
  don't have quite $q$, only an element of
  $\Trunc{\varphi=(x\mapsto px)}$. But, for any $a'$ there is a function
  \begin{displaymath}
    \ev : ((a=a)\weq (a=a')) \to (a=a'), \phi \mapsto \phi(\refl a) 
  \end{displaymath}
  The proposition $\isEq(\ev)$ is true because is true for
  $a'\jdeq a$ and $A$ is connected.
\end{proof}
...
\begin{enumerate}
\item \emph{monomorphism}\index{monomorphism|of groups} if for any group $F$  postcomposition by $f$ is an injection from $\Hom(F,G)$ to $\Hom(F,H)$
\item \emph{epimorphism}\index{epimorphism|of groups} if for any group $I$  precomposition by $f$ is an injection from $\Hom(G,I)$ to $\Hom(H,I)$.
\end{enumerate}
\end{definition}
