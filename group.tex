...
The following example expresses that $\ZZ$ is a ``free group 
with one generator''. %(DISCUSS) added quotations so that we do not need to discuss, but rather say that the example illustartes the (undefined) concept
...
(I write a short experimental tirade on injections to be moved to the intro which hopefully helps understand what an injection is.  It will mean that we can shorten the monomorphism discussion a _lot_.  I'm by no means finished and it is safest to ignore this just for now).
...
Note that a term $b:B$ can be interpreted as a function $c_b:\bn 1\to B$, and it turns out that we can replace $\bn 1$ by any type $A$ in the definition of an injection:
...
\end{lemma}
\begin{proof}
  Deducing from $x:\prod_{b,b':B}\sum_{\sigma:\prod_{f(b)=f(b')}b=b'}f(\sigma(p))=p$ that $f$ is an injection is easy: if $c:C$ and $(b,p),(b',p'):f^{-1}(c)$, then $p'p^{-1}:f(b)=f(b')$ and the first projection of $x(b,b')$ as applied to $p'p^{-1}$ is an element $q:b=b'$ so that $f(q)=p'p^{-1}$, \ie a term in $(b,p)=(b',p')$.
  ...
  Then $(b,b')\mapsto (y_1(b,b'),y_2(b,b')):\prod_{b,b':B}\Sigma_{\sigma:\prod_{f(b)=f(b')}b=b'}f(\sigma(p))=p$.
...
In other words, an injection is a function $f:B\to C$ such that for all types $A$ and all functions $g,h:A\to B$ such that $fg=fh:A\to C$ 
