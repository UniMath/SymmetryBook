...
\begin{remark}\label{rem:inverses-as-property}
  ...
  Property \ref{axiom:mere-inverse} contains the quantifier ``there exists''.  The translation of \ref{axiom:mere-inverse} into type theory
  would use with $\exists$, ``there merely exists'', as defined in \cref{sec:prop-trunc}.  Under this translation, property \ref{axiom:mere-inverse} does
...
\end{remark}
\begin{remark}
  If $\mathcal G=(S,e,\mu,\iota)$ and $\mathcal G'=(S',e',\mu',\iota')$ are abstract groups, an element of the identity type
  $\mathcal G=\mathcal G'$ consists of quite a lot of information.  First and foremost, we need an identity $p:S=S'$ of sets, but from there on the
  ...
  to ``$e'=p(e)$ and $\mu'(p(s),p(t))=p(\mu(s,t))$''.  A convenient way of obtaining such an identity is to apply univalence
  to an equivalence $f: S -> S'$ that preserves the group structure.  (Such a function will be called an abstract \emph{isomorphism}.)
\end{remark}
\begin{xca}
  \label{xca:conj}
