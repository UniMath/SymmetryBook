...
$$\preinv(p\, q)(r)=r(p\,q)^{-1}=r\,q^{-1}p^{-1}=\preinv(p)\preinv(q)(r)$$  -- demonstrating why we chose $\preinv$: without the inverse this would have gone badly wrong).  That this $\abstr(G)-set$ is an $\abstr(G)$-torsor then follows since $BG$ is connected (any $\pt_G=x$ will serve as a proof of $(\pt_G=x,\preinv,!)=\princ^\abstr_{\abstr(G)}$).
\end{example}
...
Conversely,  $p_G:G\to\abstr(\concr(G))$ is given by \footnote{(write out).}

\begin{lemma}
  \label{lem:Groupsareidentitytypes}Let $G$ be an abstract group.  
For all groups $G$, the $q_G:G\to\concr(\abstr(G))$ 
is a pointed\footnote{defined where?} equivalence.
For all abstract groups the $p_G:G\to\abstr(\concr(G))$ is an equivalence.\footnote{not defined.  Fix and write out}
Hence
