...
\begin{example}\label{ex:cyclicgroups}
  ...
  \glossary(C){$\protect\CG_m$}{cyclic group of degree $m$,
      \cref{ex:cyclicgroups}}\index{cyclic group}%
  ...
  \[
    \begin{tikzcd}
      \CG_m \ar[r,eql] & \Aut_{\sum_{X:\Set}(X\to X)}(\bn m,\zs) \ar[d,eql] &
      \\
      & \Aut_{\SetBundle(\Sc)}(\Sc,\dg{m}) \ar[r,eql] & \Aut_{\Sc\to\Set}(R_m),
    \end{tikzcd}
  \]
...
\begin{definition}\label{def:loops-map}
  ...
  we define a function $\loops k : \loops X \to \loops Y$ by setting\footnote{%
    ...
    $\ap{f}(p)$ by $f(p)$.}
  \[
    \loops k(p) \defeq \inv{k_\pt} \cdot \ap{k_\div}(p) \cdot k_\pt
    \text{,\quad for all $p: \pt_X \eqto \pt_X$.}
  \]
  Later we will need that $\loops k$ is a pointed map and define 
  $(\loops k)_\pt: \refl{\pt_Y}\eqto \inv{k_\pt}\cdot k_\pt$ by
  the inverse law of path algebra, \cref{xca:path-groupoid-laws}.
  \glossary(924Omega){$\protect\loops k$}%
  {loop map of pointed map, \cref{def:loops-map}}
\end{definition}

\begin{remark}\label{rem:loops-map}
  If $k : X \ptdto Y$ has the reflexivity path $\refl{\pt_Y}$ as its
...
\item If $G$ and $H$ are groups, the projections $\BG\gets \BG\times \BH\to \BH$ and inclusions $\BG\to\BG\times\BH\gets\BH$
  ...
  $G\to G\times H\gets H$.
\item\label{it:mod_m}
  In \cref{ex:cyclicgroups} we gave an example of an isomorphism, 
  namely one from the cyclic group $\CG_m$ to $\ZZ/m\ZZ$,
  and in~\cref{ex:Cm} we looked at $R_m : \B\ZZ \ptdto \BSG_m$,
  which, pointed by $\refl{\bn m}$,
  classifies a homomorphism $\ZZ \to \SG_m$. This homomorphism
  factors through $\CG_m$ (equivalently through $\ZZ/m\ZZ$)
  via a homomorphism $\mathrm{mod}_m : \ZZ \to \CG_m$ classified by 
  $\B\mathrm{mod}_m:\B\ZZ \ptdto \BCG_m$ sending $\base$ to $(\bn m,\zs)$
  and $\Sloop$ to $\zs$,\footnote{%
  The function $\zs$ here is the successor function modulo $m$.} 
  and pointed by reflexivity.
  The homomorphism $\CG_m \to \SG_m$ forgets the
  cycle structure.\qedhere
  \end{enumerate}
\end{example}
...
\begin{example}
  ...
  \]
  The domain of this equivalence classifies $\Hom(\ZZ,G)$.
...
\[
  \begin{tikzcd}
    \Hom(\ZZ,G) \arrow[r, equivr, "\ev"] \arrow[d, "f\circ{\blank}"'] &
    \USymG \arrow[d, "\USymf"]\\
    \Hom(\ZZ,H) \arrow[r, equivr, "\ev"] & \USymH,
  \end{tikzcd}
\]
where the horizontal maps evaluate
the classifying maps of the homomorphisms from $\ZZ$ at the loop
$\Sloop : \USym\ZZ$.
\end{lemma}
...
\begin{remark}\label{rem:first-abs-hom}
...
\begin{remark}\label{rem:F2-first-look}
...
\begin{remark}\label{rem:inf-dihedral-frieze}
  ...
  In fact, we already saw one manifestation of this
