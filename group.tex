...
\begin{example}\label{ex:groups}
  ...
  Recall that $\bn{1} \eqto \bn{1}$ is contractible.
  Hence $\Aut_{\Set}(\bn{1})$ is a group called the 
  \emph{trivial group}. \index{trivial group}
  The trivial group can also be identified with
  $\Aut_{\bn{1}}(0)$, where $0$ is the sole constructor of $\bn{1}$.
  Unlike $\Set$, the type $\bn{1}$ is connected (in fact
  contractible), so we can also identify the trivial group with
  $\mkgroup (\bn{1},0)$. Similarly for $\mkgroup (\true,\triv)$ and
  $\Aut_{\true}(\triv)$, the automorphism group of the trivial element
  in the type $\true$.
  \item\label{ex:permgroup}
    ...
    Again, we can identify the group $\SG_n$ also with
    $\Aut_\FinSet(\bn{n})$, with $\Aut_{\FinSet_n}(\bn n)$
    (by \cref{rem:symmetriesofnonconnectedgroupoids}),  
    or even $\Aut_{\UU}(\bn n)$ (by \cref{rem:autinfgp}).
...
\begin{xca}
  ...
  Show that $\Sigma_\false$ is a trivial group.
