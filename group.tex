...
\begin{example}\label{ex:groups}
  ...
  \item More generally, if $S$ is a set, is there a pointed connected groupoid $(A,a)$ so that $a\eqto_Aa$ models all the ``permutations'' $S\eqto_{\Set}S$ of $S$?
...
\item In \cref{ex:cyclicgroups} we gave an example of an isomorphism, namely one from the cyclic group $\CG_m$ to $\ZZ/m\ZZ$.\qedhere
  \end{enumerate}
\end{example}
...
for all finite sets $A$.\footnote{%
  It is prudent to add the decoration signifying which finite set $A$
  is considered as reference, since the classifying map depends on it:
  We can take $\Bsgn^A_\div(B) \defeq (\Bsgn_\div(B) \eqto \Bsgn_\div(A))$
  with the pointing given by the identity at $\Bsgn(A)$.}
...
\begin{remark}
  ...
  whereas the sign itself does not.\marginnote{%
    \begin{tikzpicture}
      \draw (1,2) .. controls (1,1.5) and (0,1.5) .. (0,1)
      .. controls (0,.5) and (2,.5) .. (2,0);
      \draw (0,2) -- (1,0);
      \node at (2.5,1) {$\rightsquigarrow$};
      \draw (4,2) -- (5,0);
      \draw (3,2) -- (4,0);
    \end{tikzpicture}}
  ...
  (and thus the sign) is preserved.
\end{remark}
