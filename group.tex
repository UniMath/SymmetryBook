...
\footnote{\color{blue}THE REST OF THE CHAPTER IS IN FLUX AND I DID NOT GET TO FIX ALL I WANTED TO FIX BEFORE I HAD TO QUIT FOR NOW.  IT CONTAINS KNOWN NONSENSE}
...
Now, as we've seen, that   $f:(\pt_H=\pt_H)\to(\pt_G=\pt_G)$ is an injection (preimages are propositions) is equivalent to the preimages of $Bf:BH\to BG$ being sets.  Hence we get the following neat formulation.
    \begin{definition}
      ...
      \end{enumerate}
    \end{definition}
A note on notation is in order.  If $(H,i,!)$ is a subgroup of a group $G$ tradition often relax the burden of notation; we may write ``a subgroup $i:H\subseteq G$'', or, if we don't need the name of $i:\Hom(H,G)$ in what follows, simply ``a subgroup $H\subseteq G$'' or ``a subgroup $H$ of $G$''.
\begin{lemma}
...
$(H,i_H,!)=_{\typesubgroup_G}(H,i_H,!)$ is equivalent to the type of homomorphisms $f:\Hom(H,H)$ which are such that $!:i_H^==i_H^=f^=$ such that $f^=$ is an equivalence (as we see in a moment this last requirement is redundant).  
Now, since $(H,i_K,!)$ is a subgroup, $i_H^=$ is an injection of sets, which forces $!:f^==\refl{\pt_H=\pt_H}$, which forces $f$ to be (identical to) the identity homomorphism. 
\end{proof}
...
where 
$$\Hom(H,G)(y)\defequi\sum_{F:BH_\div\to BG_\div}(y=F(\pt_H))$$ as in \cref{ex:HomHGasGset}.  
