...
\begin{remark}
  When considering the identity type $a=_Aa$, the elements $x:A$ with $x$ unequal to $a$ are not relevant, and we are free to consider only \emph{connected} $A$, \ie where $x=_Aa$ is always inhabited.  Also, our preference for $a=_Aa$ to be a set indicates that we should consider only the connected types $A$ that are \emph{groupoids}.
\end{remark}
...
\begin{example}
  ...
  \begin{enumerate}
  \item Recall that the set $\true$ has the single element $\triv$. Then $\aut_{\true}(\triv)$ is a group called the \emph{trivial group}.
  \item More generally, if $S$ is a set, is there a pointed connected groupoid $(A,a)$ so that $a=_Aa$ models all the ``permutations'' $S=_{\Set}S$ of $S$?  The only thing wrong about $(\Set,S)$ (apart from $\Set$ being large\footnote{how do we deal with that?}) is that $\Set$ is not connected.  So we \emph{pick
 the component of $\Set$ containing $S$}!\footnote{it's so simple -- so very simple -- that only a child can do it!}  To be precise, the component of $S$ is
$$A\defequi\sum_{X\in\Set}||S=X||.$$  
The connected groupoid $\sum_{X\in\Set}||S=X||$ is pointed at $S$ (and the fact that $S=S$ is nonempty since $\refl S:S=S$).    Then 
$$(S=_AS)=(S=_{\Set}S)$$ 
...
So, the \emph{group of permutations of $S$} is defined to be $\BSigma_{S}=(A,S)$.
  \end{enumerate}
\end{example}
\begin{xca}
  \begin{enumerate}
  \item 
  Prove that the the trivial group $\aut_{\true}(\triv)$ is equivalent to the group $\BSigma_{\true}$ of permutations of $\true$.
\item Display a nontrivial element in the group $\BSigma_{\bool}$ of permutations of $\bool$.
\item \footnote{really want this exercise in a previous chapter} Define a type family $\mathbf 0$, $\mathbf 1$, $\mathbf 2$,\dots inductively on $\NN$ by declaring that the value at $0:\NN$ is $\mathbf 0\defequi\emptyset$ and the value at a successor $s(n):\NN$ is $\mathbf n\coprod\true$.  
