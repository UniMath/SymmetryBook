...
\section{Groups concrete or abstract -- two sides of the same coin}
...
\label{def:abstrGtorsors}
  If $\agp G$ is an abstract group, a $\agp G$-set is a set $S$ together with a homomorphism
$\agp G\to\abstr(\Sigma_S)$
from $\agp G$ to the (abstract) permutation group of $S$:
$$Set_{\agp G}^\abstr\defequi \sum_{S:\Set}\Hom_\abstr({\agp G},\abstr(\Sigma_S)).$$

The \emph{principal ${\agp G}$-torsor} $\princ {\agp G}^\abstr$ is the ${\agp G}$-set consisting of the underlying set $\mathrm{under}({\agp G})$ together with the homomorphism ${\agp G}\to\abstr(\Sigma_{\mathrm{under}({\agp G})})$ given by $g\mapsto \mathrm{ua}(\cdot g^{-1})$ ((explain)).
...
\begin{xca}
  ...
  Let ${\agp G}=(S,e,\mu,\iota)$ be an abstract group.  Then that underlying set of $\abstr(\concr({\agp G}))$ is $\princ {\agp G}^\abstr=_{\typetorsor^\abstr_{\agp G}}\princ {\agp G}^\abstr$.  
...
$$  
is an equivalence of sets, which we by univalence turn into an equality.  The abstract group structure of $\abstr(\concr({\agp G}))$ is given by it being the symmetries of $\princ {\agp G}^\abstr$, or translated to $\sum_{p:S=S}\prod_{q,s:S}(p(s\,q^{-1})=p(s)\,q^{-1})$ that of $S=S$ of the first projection.  
