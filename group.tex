...
and on the way discover that if $(t,Q):F$, then $t$ is an equivalence, so that $F\simeq \sum_{g:C=C}-^mg=_{C\to C}-^m$ actually {\bf is} the type we are interested in.
...
%For brevity, let $a\defequi -^mc$ and $b\defequi-^mt$ be shorter names for the two functions in $S^1\to C$ that are going to be equal.   
...
((this does the hardest part (4), but the other cases still need to be written))
    \end{enumerate}
  \end{proof}
