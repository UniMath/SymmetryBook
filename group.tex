...
\begin{lemma}\label{lem:pathsptransportiseq}
  For  $y,z:BG$ we make the induced map  (\ie transport) of identity types
$$\pathsp{}:(y=z)\to (\pathsp y=\pathsp z)
$$
is an equivalence.
\end{lemma}
...
Univalence then gives an identity of groups (or $\infty$-groups) from $G=(BG,\pt_G)$ to $(\typetorsor_G,\princ G)$.\footnote{in the appropriate universe}.
\end{theorem}

\begin{proof}
  Since both $\typetorsor_G$ and $BG$ are connected, it suffices by \cref{lem:eqandcovofconntypes} to show that the induced map
$$T\pathsp{}:(y=z)\to(\pathsp y= \pathsp z)
$$
