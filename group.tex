...
\begin{example}\label{ex:cyclicgroups}
  ...
  By using univalence on the equivalences of~\cref{thm:coveringsofS1perms} and composing the resulting identification, we have a composite identification
  \[
    \CG_m \eqto \Aut_{\sum_{X:\Set}(X\to X)}(\bn m,\zs)
    \eqto \Aut_{\SetBundle(\Sc)}(\Sc,\dg{m})
    \eqto \Aut_{\Sc\to\Set}(R_m),
  \]
