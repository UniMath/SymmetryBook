...
\begin{definition}\label{def:typegroup}
  ...
  of the type of pointed connected groupoids $\UUscone$, defined as follows.
  \[
    \typegroup \defequi \Copy(\UUscone)
  \]
  A \emph{group} is an element of $\typegroup$.
  For $X : \UUscone$, we adopt the notation $\mkgroup X$ as a synonym for the
  group $\inc X : \typegroup$ provided by the constructor associated with $\Copy$.
\end{definition}

\begin{definition}\label{def:classifying-type}
  For $G : \typegroup$, we adopt the notation $\clf G$ for the pointed type $\out G : \UUscone$
  provided by the destructor associated with $\Copy$; we will refer to it as the \emph{classifying type} of $G$.
  Morever, the elements of $\clf G$ will be referred to as the \emph{shapes of $G$},
  and we define the \emph{designated shape of $G$} by setting $\p_G\defequi \pt_{\clf G}$.
\end{definition}

\begin{definition}\label{def:group-symmetries}% or elements
  Let $G$ be a group.
  We regard every group as a group of symmetries, and thus we refer to the elements of $\loops \clf G$ as the
  \emph{symmetries in $G$}; they are the symmetries of the designated shape $\p_G$ of $G$.
  (Notice the careful distinction between the phrases ``\emph{symmetries in}'' and ``\emph{symmetries of}''.)
  We adopt the notation $\USym G$ for the type $\loops \clf G$ of symmetries in $G$; it is a set.
\end{definition}
...
\begin{remark}\label{rem:aut}
  ...
  the type $\USym G$ is at least as important as $\clf G$ --- the copying forces us to use
  the notation $\clf G$, preventing a glib identification of $G$ with its classifying type
  and preventing us from regarding $G$ as having \emph{elements}.
  ...
  you know also that $X$ is the classifying type for a group, namely $G\defeq\mkgroup X$.
\end{remark}
