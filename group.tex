...
\begin{example}\label{ex:groups}
  ...
  Recall that $\bn{1} \eqto \bn{1}$ has exactly one element:
  $\refl{\bn{1}}$. Hence $\Aut_{\Set}(\bn{1})$ is a group called the 
  \emph{trivial group}. \index{trivial group}
  Of course, the trivial group can also be identified with
  $\mkgroup (\bn{1},0)$ and $\Aut_{\bn{1}}(0)$, and with
