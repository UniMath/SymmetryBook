...
make the abstract group $\abstr(G)$ from $\USymG$ and the abstract group $\abstr(H)$ from $\USymH$.
...
\begin{definition}\label{def:grouphomomorphism}
  ...
  and we call $\Bf$ \emph{classifying map}\index{classifying map} of the homomorphism.
\end{definition}
...
\begin{remark}\label{rem:loops-map}
  ...
  of pointedness, then we have an identification $\loops f(p) \eqto \ap{f_\div}$.
\end{remark}
...
\item In \cref{ex:cyclicgroups} we gave an example of an isomorphism, namely one from the cyclic group $\CG_m$ to $\ZZ/m\ZZ$,
  ...
  factoring through $\ZZ/m\ZZ$ (and $\CG_m$).\qedhere
  \end{enumerate}
\end{example}
