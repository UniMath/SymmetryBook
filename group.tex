...
% In \cref{xca:component-same-loops} you were asked to show that if $A$ is a type and $a:A$, then 
  As a consequence of \cref{lem:subtype-eq-=},\marginnote{%
    ...
    \end{tikzpicture}}
  the inclusion of the component $A_{(a)}\defequi\sum_{x:A}\Trunc{a=x}$ into $A$
...
% If $g:\USym G$, then $\ap{\Bf_\div}(g)$ is an element of $\Bf_\div(\sh_G)=\Bf_\div(\sh_G)$, while we want something in $\USym H$.  However, this is not an obstacle since conjugation by $p_f: \Bf_\div(\sh_G)=\pt_H$ gives rise to 
% $$\mathrm{ad}_{p_f}:(\Bf_\div(\sh_G)=\Bf_\div(\sh_G))\weq\USym H$$ (with $\mathrm{ad}_{p_f}(\refl{\Bf_\div(\sh_G)})=\refl{\pt_H}$, as discussed in \cref{sec:heavy-transport}) and so 
...
and likewise with a subscript $H$, we have the following:
  ...
  \item the proposition $f^\abstr(e_G)= e_H$ holds through the
    ...
    \end{displaymath}
% Since $\ap{\Bf}(e_G)\oldequiv \ap{\Bf}f(\refl{\sh_G})\oldequiv\refl{f(\sh_G)}$ and $e_H\oldequiv\refl{\pt_H}$, 
% $$f(e_G)\oldequiv\mathrm{ad}_p(\ap{\Bf}(e_G))\oldequiv\mathrm{ad}_p(\refl{f(\sh_G)})\oldequiv \refl{\pt_H}\oldequiv e_H$$
...
over the component $\Set_{(X(\sh_G))}\defequi\sum_{Y:\Set}\Trunc{X(\sh_G)=Y}$ 
which contains the point $X(\sh_G)$.  
Since $B\Sigma_{X(\sh_G)}\defequi(\Set_{(X(\sh_G))},X(\sh_G))$ the $G$-set $X$ can, 
...
\begin{lemma}
  ...
  $\Set_{\USym G}$ of $\Set$ (c.f.~\cref{remark:GsetsareGsets})
...
% \footnote{OLD: $$f_*X(y)\defequi(\pt_H=y)\times_{\USym G}X(\sh_G)$$ to be the set quotient of $(\pt_H=y)\times X(\sh_G)$ by the relation $(p,x)\sim(p\, f(q)^{-1},X(q)x)$ for all $q:\sh_G=pt_G$.
...
% $$\eta:(\pt_H=y)\times \USym G\to (\pt_H=y),\quad \eta(p,q)=p\,f(q)$$
...
%$$\phi_*X(z)\defequi(\pt_H=z)\times_{\USym G}X(\sh_G). \footnote{here I need set-quotients: $(\pt_H=z)\times_{\USym G}X(\sh_G)$ is the coequalizer of the two maps $(\pt_H=z)\times X(\sh_G)\gets (\pt_H=z)\times{\USym G}\times X(\sh_G)$ sending $(p,q,x)$ to $(p\,f(q),x)$ and $(p,X(q) x)$. I use $[p,x]$ to denote an element of this type}$$
...
%$$(\pt_H=z)\times_{\USym G}\USym G\to (\pt_H=z)$$ 
%sending $[p,q]:(\pt_H=z)\times_{\USym G}\USym G$ to $p\,\phi(q):(\pt_H=z)$.
Let 
...
Since $\princ G(\sh_G)\defequi \USym G$ this defines a pointed function $\rho_G:\BG\to_* \BSigma_{\USym G}\defequi(\Set_{\USym G},\USym G)$, \ie a homomorphism from $G$ to the permutation group
...
\begin{proof}
  ...
  % equivalent to
  % the type of pointed \coverings over $\BH$ with total space $\BG$, so
  We need to show that $\rho_G:\BG\to\Set_{\USym G}$ is a
  \covering.
  Under the identity
...
% Now, by transport along the identity $p_f:\pt_{G'}=f(\sh_G)$ and the equivalence between $G'$-sets and $\abstr(G')$-sets, an identity $F':X=Y$ of $G'$-sets is uniquely pinned down by an identity $F'_{f(\sh_G)}:X(f(\sh_G)=Y(f(\sh_G))$ together with the proposition that for all $g':f(\sh_G)=f(\sh_G)$ the diagram $$\xymatrix{X(f(\sh_G))\ar@{=}[r]^{X_{g'}}\ar@{=}[d]_{F'_{f(\sh_G)}}&
%   X(f(\sh_G))\ar@{=}[d]_{F'_{f(\sh_G)}}\\
%   Y(f(\sh_G))\ar@{=}[r]^{Y_{g'}}&Y(f(\sh_G))}
% $$
% commutes.  Likewise, an identity $F:f^*X=f^*Y$ is given by exactly the same data, except that the diagram is only required to commute for $g'=f(g)$ for all $g:\USym G$.  But when $f:\USym G\to(\pt_{G'}=\pt_{G'})$ these requirements coincide.
...
% $$\sum_{f:\Hom(H,H')}\isEq(f^=:\USym H\to(\pt_{H'}=\pt_{H'})).$$  
...
% % In this interpretation, $(H,F,p,!):\typesubgroup_G(\sh_G)$ represents a subgroup of $G$ (so that $p:\sh_G=F(\pt_H)$).  An identity $g:\USym G$ acts on $\typesubgroup_G(\sh_G)$ by sending $(H,F,p,!)$ to $(H,F,p\,g^{-1},!)$.
...
% action of $g:\USym G$ on a subgroup $(H,F,p,!):\typesubgroup_G(\sh_G)$ (where $p:x=F(\pt_H)$) is simply $(H,F,p\,g^{-1},!)$, which does not seem much like conjugation.  
...
% $$\pt_{\ker f}\defequi(\sh_G,p_f):(\Bf)^{-1}(\pt_{G'})$$ (where $p_f:\pt_{G'}=\Bf(\sh_G)$ is the part of $f$ claiming it is a pointed map). 
...
% The first projection $\Bker f\to_* \BG$ displays the kernel $\ker f$ as a subgroup of $G$ (\emph{sub}group since the preimages are equivalent to the sets $\sum_{p:\pt_{G'}=\Bf(z)}\Trunc{\pt_{\ker f}=(z,p)}$).  
...
% The \emph{image}\index{image} of $f$ is the subgroup of $G'$ given by $B\image f\defequi \sum_{z:\BG'}\coker f(z)$ pointed at $\pt_{\image f}\defequi (\pt_{G'},|\sh_G,p_f|)$ together with the first projection $B\image f\to \BG'$ (plus the fact that $\coker f(z)$ is a set).  
...
% $f^=:\USym G\to(\pt_{G'}=\pt_{G'})$ is a surjection
%     \end{enumerate}
%   \item the following are equivalent
%     \begin{enumerate}
%     \item $f$ is an injective homomorphism (\ie the induced map of sets 
% $f^=:\USym G\to(\pt_{G'}\to\pt_{G'})$
% is an injection)
...
% $$
% (where $H$ is a group, $F:BH_\div\to BG_\div$ is a map and $p_F:y=F(\pt_H)$ an identity in $BG$; and $F^{-1}:BG\to\Set$ is $G$-set given by the preimages of $F$ and $(\pt_H,p_F):F^{-1}(y)\defequi \sum_{x:BH}y=F(x)$ is the base point).  If $y$ is $\sh_G$ we follow our earlier convention of dropping it from the notation.
...
% Observe that by using the identification of the basepoint $Q^f_{f(\sh_G)}$ of $BG/\ker'f$ with $Q^f_{\pt_{G'}}$ given by $p_f:\pt_{G'}=f(\sh_G)$ we have defined a homomorphism $Q^f:\Hom(G',G/\ker'f$) such that 
...
% % $\preinv(p_f):(\pt_{G'}=f(z))=(f(\sh_G)=f(z))$ and 
% % $$\preinv:(f(\sh_G)=f(z))=
...
% $$\pt_{H\times_GH'}\defequi(\pt_H,\pt_{H'},p_{f'}p_f^{-1})$$ of the pullback $BH\times_{BG}BH'$ (where $p_f:\sh_G=f(\pt_H)$ is the name we chose for the data displaying $f$ as a pointed map, so that $p_{f'}p_f^{-1}:f(\pt_H)=f'(\pt_{H'})$).
...
% Recall that if $X:BG\to\Set$ is a $G$-set, then the set of fixed points is the set $\prod_{v:BG}X(v)$, which is a subset of $X(\sh_G)$ via the evaluation map.  If a homomorphism from a group $H$ to $G$ is given by $F:BH_\div\to BG_\div$ and $p_F:\sh_G=F(\pt_H)$, then precomposition (``restriction of scalars'') by $F$ gives an $H$-set 
...
% This can be somewhat confusing when it comes to fixed points: the fixed points of $F^*X$ are given by $\prod_{v:BH}XF(v)$ which evaluates nicely to $XF(\pt_H)$, but in order to considered  these as elements in $X(\sh_G)$ we need to apply $X(p_F^{-1}):X(F(\pt_H))=X(\sh_G)$.  

% Consequently, we'll say that $x:X(\sh_G)$ is an \emph{$H$-fixed point} if there is an $f:\prod_{v:BH}XF(v)$ such that $x=X(p_F^{-1})f(\pt_H)$.



% \begin{lemma}
%   \label{lem:thereisaconjugate}
%   Let $G$ be a group, $X:BG\to Set$ a $G$-set, $x:X(\sh_G)$, $g:\USym G$ and $H=(H,F,p,!):\typesubgroup_G$ a subgroup of $G$ ($F:BH_\div\to BG_\div$ and $p:\sh_G=F(\pt_H)$).  
...
% Recall the equivalence between the set $\typesubgroup_G$ of subgroups and the set $\typesubgroup_G'$ of pointed transitive $G$-sets: The pair $(X,\pt_X)$ where $X:BG\to\Set$ is a transitive $G$-set and $\pt_X:X(\sh_G)$ corresponds to the subgroup $(H,i_H,!)$ defined by  $BH_\div\defequi\sum_{y:BG}X(y)$ pointed at $\pt_H\defequi (\sh_G,\pt_X)$ together with the first projection $Bi_H:BH\to_*BG$.  Conversely, if $(H,i_H,!):\typesubgroup_G$, then the corresponding transitive $G$-set is $G/H\defequi\coker i_H$ pointed at $|\pt_H,p_{i_H}|:\coker i_H(\sh_G)\defequi\Trunc{\sum_{x:BH}\sh_G=BiH(x)}_0$.  
...
% \begin{definition}
%   % Let $G$ be a group and consider the subgroup represented by the transitive $G$-set $X:BG\to\Set$ together with the point $\pt_X:X(\sh_G)$ % (so that $BH_\div\defequi\sum_{y:BG}X(y)$ pointed at $\pt_H\defequi (\sh_G,\pt_X)$ together with the first projection $Bi_H:BH\to_*BG$ defines an element in $(H,i_H,!):\typesubgroup_G$)
%   % .
