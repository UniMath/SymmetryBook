...
\begin{lemma} Let $f:B\to C$ be a function between sets.
  The following propositions are equivalent
  \begin{enumerate}
  \item $f$ is an injection, \ie $\prod_{c:C}\prod_{(b,p),(b',p'):\Sigma_{b'':B}c=f(b'')}(b,p)=(b',p')$
  \item $\prod_{b,b':B}{\prod_{f(b)=f(b')}b=b'}$
  \item $\prod_{A:\UU}\prod_{g,h:A\to B}\sigma:\prod_{fg=fh}g=h$.
  \end{enumerate}
\end{lemma}
...
In other words, an injection between sets is a function $f:B\to C$ such that for all types $A$ and all functions $g,h:A\to B$ such that $fg=fh:A\to C$ 
