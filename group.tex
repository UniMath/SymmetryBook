...
ALTERNATIVELY:
If $(S,j):B_C_p\defequi\sum_{S:\UU}\sum_{j:X=X}||(S,j)=\zet/p||$, consider the type 
$$X(S,j)\defequi\sum_{g:S\to (\pt_G=\pt_G)}\prod_{a:S=\bn p}(g_{a^{-1}(1)}\cdot\dots g_{a^{-1}(p)}=e_G.$$ 
\end{proof}
