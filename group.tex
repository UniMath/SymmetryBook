...
\begin{example}
 \label{ex:listofCcovers}
  ...
  \item If $m:\NN$ is positive the \emph{degree $m$ function} $$-^m:C\to C$$ is given by $-^m((X,f))=(\sqrt[m]X,\sqrt[m]f)$, where $$\sqrt[m]X=\bn m\times X$$ and $\sqrt[m]f(i,x)=(i+1,x)$ for $i<m-1$ and $\sqrt[m]f(m-1,x)=(1,f(x))$.  We prove that $-^m((X,f))$ is in the component of $(\zet,s)$ (which was a requirement for elements in $C$).  Since we know that $(X,f)$ is in the component of $(\zet,s)$ it is enough consider the element $(p,!):(\sqrt[m]{\zet},\sqrt[m]s)=(zet,s)$ explicitly given by univalence through $p((i,n))=i+mn$ ((check that it gives an equivalence)).

If $(p,!):(X,f)=_C(X',f')$, then $$(\sqrt[m](p),!):((\sqrt[m]X,\sqrt[m]f)=(\sqrt[m]X',\sqrt[m]f'))$$ is given by $\sqrt[m](p)(i,x)=(i,p(x))$ (and the fact $!:pf=f'p$).


 Note that the equivalence $c:S^1\to C$.
  \end{enumerate}
\end{example}
\begin{xca}
...
\begin{figure}
  ...
  \begin{tikzpicture}
    ...
    \draw [decorate,decoration={brace,amplitude=10pt}]
    (1,-1) -- (-1,-1) node [black,midway,yshift=-15pt] {\footnotesize $\bn{n}$};
  \end{tikzpicture}
