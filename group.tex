...
\begin{definition}
  ...
  If $f:\Hom(G,G')$ is a homomorphism, consider the groupoid 
$$F(f)\defequi\sum_{z:BG}(\pt_{G'}=f(z)).$$\footnote{where we, as usual, have written ``$f(z)$'' instead of ``$Bf_\div(z)$'' and $p_f:\pt_{G'}=f(\pt_G)$ is part of the data of which $f$ consists} 
The \emph{cokernel}\index{cokernel} of $f$ is the set of components $\coker f\defequi||F(f)||_0$ of $F(f)$.\footnote{set trunctation}  If $f:\Hom(G,G')$ is clear from the context and displays $G$ as a subgroup of $G'$, we often write $G'/G$ for the cokernel of $f$.  

We say that the homomorphism $f$ is \emph{surjective}\index{surjective! group homomorphism} if $F(f)$ is connected (which is equivalent to $\coker f$ being contractible).
...
The first projection $B\ker f\to_* BG$ displays the kernel $\ker f$ as a subgroup of $G$ (since the preimages are equivalent to the sets $\sum_{z:BG}\sum_{p:\pt_{G'}=f(z)}||\pt_{\ker f}=(z,p)||$).  A subgroup is said to be \emph{normal}\index{normal} if it is the kernel of a homomorphism.


The \emph{image}\index{image} of $f$ is the subgroup $\image f\defequi \sum_{x':BG'}||Bf^{-1}(x')||_0$ of $G'$.  
\end{definition}
\begin{definition}
  \label{def:pullback}
...
together with the two projections $\prod(f,g)\to B$ and $\prod(f,g)\to C$ sending $(b,c,p)$ to $b:B$ or $c:C$.  If $f$ and $g$ are clear from the context, we may write $B\times_DC$ instead of $\prod(f,g)$ and summarize the situation by the diagram
...
\begin{xca}
  ...
  \item The sum of the cardinalities of $A$ and $B$ is equal to the sum of the cardinalities of $A\cup B$ and $A\cap B$.
  \end{enumerate}
.  
...
\begin{xca}
  Prove that $f:\Hom(G,G')$ is a surjective group homomorphism if and only if the induced map of sets $f:^=\pt_G=\pt_G\to\pt_{G'}=\pt_{G'}$ is a surjection.
\end{xca}
\begin{xca}
...
\begin{lemma}
  ...
  Show the characterizing properties of kernels, images and cokernels.  
\end{lemma}
