...
\begin{remark}\label{def:grouphomomaxioms}
  ...
  \[
    \begin{tikzcd}
      \Bf_\div(\shape_G)\ar[r,eqtor,"\ap{\Bf_\div}(g)"] &
      \Bf_\div(\shape_G) \\
      \shape_H\ar[r,dashed,"\USymf(g)",eqtor]\ar[u,eqtor,"p_f"] &
      \shape_H\ar[u,eqtor,"p_f"].
    \end{tikzcd}
  \]
  With the shorthand
  \[
  ...
  $\begin{tikzcd}[cramped,sep=small]
    \shape_G\ar[r,eqtor,"g'"] & \shape_G\ar[r,eqtor,"g"] & \shape_G
  \end{tikzcd}$)
...
To do this, we need to assign to every $n$-element set $A$ a $2$-element $\Bsgn(A)$.
\begin{marginfigure}
  \tikzset{->-/.style={
      decoration={
        markings,
...
We get this $2$-element set as quotient of the set of all possible ways of ordering two different elements of $A$, where two different such
choices are deemed the same if they differ in an \emph{even} number of pairs.
A way of ordering all possible $2$-element subsets of $A$ is equivalently
a way of directing the complete graph on $A$.
\Cref{fig:sign-orderings-3} shows all $8$ ways of directing the complete graph on $3$ elements divided into the $2$ resulting equivalence classes.

To see that this really defines an equivalence relation, it helps to generalize a bit.
Thus, fix a nonempty finite set $E$, and let $P : E \to \BSG_2$ be a family of $2$-element sets parametrized by $E$.
\begin{definition}
  The parity relation $\sim$ on $\prod_{e:E}P(e)$ relates functions that disagree in an even number of points. That $f~g$ holds if and only the
...
\begin{lemma}
  ...
  and the quotient is a $2$-element set.
\end{lemma}
\begin{proof}
  ...
  Hence, the Venn diagram of these sets have the simplified form shown in the margin,\marginnote{%
    \begin{tikzpicture}
  ...
  We now find $F_{12} = E'_{13} \cup E'_{23}$ (disjoint union), and similarly for $F_{13}$ and $F_{23}$.
  ...
  Finally, to show the proposition that the quotient is a $2$-element set,
...
\begin{xca}
  ...
  to the product of its values.
\end{xca}

\begin{definition}\label{def:sgn}
  The \emph{sign homomorphism} $\sgn : \Hom(\SG_n,\SG_2)$ is defined
  via the pointed map $\Bsgn : \BSG_n \ptdto \BSG_2$,
  where $\Bsgn(A) \defeq \B\mu_{E(A)}(P)$, where $E(A)$ is the set of $2$-element subsets of $A$,
  ...
  The \emph{sign} of a permutation $\sigma : \USG_n$ is the value
  $\Usgn(\sigma) : \USG_2$.
\end{definition}
...
\begin{lemma}\label{lem:sign-properties}
  \begin{enumerate}
  \item The sign of a transposition is $-1$.
  \item The sign of a $k$-cycle is $(-1)^{k-1}$.
  \item The identity permutation can only be expressed as a product
    of an even number of transpositions.
  \end{enumerate}
\end{lemma}
\begin{proof}
  We only need to prove the first part, as the others follow directly.
