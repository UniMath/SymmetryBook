...
{\color{blue}I need this and must put it somewhere.  Current proof somewhat unsatisfactory.
\begin{lemma}
  If $f:\Hom(G,G')$ is surjective, then $f^*:(G'\text{-}\Set)\to(G\text{-}\Set)$ is ``fully faithful'' in the sense that if $X,Y$ are $G'$-sets, then
$$f^*:(X=Y)\to(f^*X=f^*Y)
$$
is an equivalence.
\end{lemma}
\begin{proof}
  Evaluation at $\pt_G$  yields an injective map $\mathrm{ev}_{\pt_G}:(f^*X=f^*Y)\to(X(f(\pt_G)=Y(f(\pt_G)))$ and the composite $\mathrm{ev}_{\pt_G}f^*=\mathrm{ev}_{f(\pt_G)}:(X=Y)\to(X(f(\pt_G)=Y(f(\pt_G)))$ is the likewise injective, so $f^*:(X=Y)\to(f^*X=f^*Y)$ is injective.  Now, by the equivalence between $G'$-sets and $\abstr(G')$-sets; $F:X=Y$ is in the preimage of $a:X(f(\pt_G)=Y(f(\pt_G))$ if and only if $a=F_{f(\pt_G)}$ and for all $g':f(\pt_G)=f(\pt_G)$ the diagram
...
\begin{lemma}
  ...
  Let $N:\typenormal'_G$ be a normal subgroup' with $N(y)\oldequiv (X_y,\pt_y,!)$ for $y:BG$.  Then for any $y,z:BG$ the evaluation map
$$\mathrm{ev}_{yz}:(X_y=X_z)\to X_z(y),\qquad \mathrm{ev}_{yz}(f)=f_y(\pt_y)$$
is an equivalence.
\end{lemma}
\begin{proof}
To see this we need to do induction independently on $y:BG$ and $z:BG$ in $X_y(z)$ at the same time as we observe that it suffices (since $BG$ is connected) to show that $\mathrm{ev}_{yy}$ is an equivalence.

Induction on the index gives rise to the map $X:(y=z)\to(X_y=X_z)$ ($X_{\refl y}\defequi\refl{X_y}$) and the composite 
...
On the other hand, in  \cref{lem:evisinjwhentransitive} we used the transitivity of $X_y$ to deduce that $\mathrm{ev}_{yy}$ was injective.  Consequently $\mathrm{ev}_{yy}$ is an equivalence.  
\end{proof}
\begin{definition}
...
pointed in $(\pt_G,\refl{X_{\pt_G}})$ and with $i_{\ker f}:B\ker q_N\to_*BG$ given by the first projection.  Consider the map
$$\mathrm{ev}:B\ker q_N\to B\mathrm{ass}(N)_\div,\qquad \mathrm{ev}(z,f)\defequi(z,f(\pt_z)),$$
which is (strictly) pointed since $\mathrm{ev}(\pt_G,\refl{X_{\pt_G}})\defequi (\pt_G,\pt_{\pt_G})$.  

We claim that $\mathrm{ev}$ is an equivalence.  Since $B\mathrm{ass}(N)$ is connected it is enough to show that the preimage of $(\pt_G,\pt_{\pt_G})$ is contractible.  
