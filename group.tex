...
\end{definition}
Note that under this definition, the empty type $\mathbf 0$ is not connected, and neither is $\mathbf n$ if $n>1$. However, $\mathbf 1$ is connected.
...
$$c:\mathbf 2\to(A\to\mathbf 2)$$ defined by $c(x)(a)=x$ for $x:\mathbf 2$ and $a:A$ 
%to the constant function with value $x$ 
is an equivalence. 
%$||A||%\sum{a,b:A}||a=b||=\mathbf 1$.  
\end{lemma}
  \begin{proof}\footnote{this proof is classical.  Do you object?}    
    Assume $A$ is connected.  Let $a:A$ be so that for every $b:A$, then $||a=_Ab||$ is true.  If $f:A\to\mathbf 2$, then for every $b:A$ we get that $||f(a)=f(b)||$, but since $\mathbf 2$ is a set, we get that $f(a)=f(b)$.  Hence, any function $f$ is constant and so $c$ is an equivalence.

Conversely, assume that $c$ is an equivalence.  Then $A$ is nonempty.  Let $a:A$ and let $f:A\to \mathbf 2=\bool$ be defined by $f(b)\defequi||a=b||$.  Since $c$ is an equivalence and $f(a)$ is true, we get that $f(b)$ is true for all $b$.   
  \end{proof}

 As a matter of fact, if $A$ has an element $a$, then $A$ is connected if and only if for all $b:A$ the identity type $a=_Ab$ is inhabited, essentially because if $f\colon A\to\mathbf 2$ an equality $a=_Ab$ forces $f(a)=_{\mathbf 2}f(b)$.  So, a connected type is one that can't be ``torn apart''.  
\begin{definition}
...
\subsubsection{The univalence axiom}\label{sec:univax}\footnote{The proof of \cref{lem:prop-utils} (prev. chapter) refers to univalence without it being defined}
...
\begin{xca}\label{xca:C2}
   Prove that $\mathbf 2=\mathbf 2$ has exactly two elements, $\refl{\mathbf 2}$ and $\twist$ (where $\twist$ is given by univalence from the function $\mathbf 2\to\mathbf 2$ exchanging the two elements of $\mathbf 2$), and that $\trans{}_{\mathbf 2,\mathbf 2,\mathbf 2}(\twist)(\twist)=\refl{\mathrm 2}$.
\end{xca}
...
\begin{definition}
  \label{def:circle}
The circle $S^1\colon\mathcal U$ ((write and point it at $\base$))
\end{definition}
The following Lemma displays the prehaps most fundamental property of the circle: a function from the circle picks out an element and a symmetry of that element:
\begin{lemma}\label{lem:freeloopspace}
  If $A$ is a type, the function $$(S^1\to A)\to \sum_{a:A}(a=_Aa)$$
given by sending $g$ to $(g(\base),g(\Sloop))$ is an equivalence.
\end{lemma}
\begin{proof}
  ((write)) 6.2.9
\end{proof}
...
\end{lemma}
\begin{proof}
  We need to prove that the function $\mathbf 2\to(S^1\to\mathbf 2)$ of \cref{def:connected} is an equivalence.  In view of \cref{lem:freeloopspace} it is enough to show that the composite
$$\mathbf 2\to \sum_{a:\mathbf 2}(a=_{\mathbf 2}a)$$
sending $a:\mathbf 2$ to $(a,\refl a)$ is an equivalence, which is true since $a=_{\mathbf 2}a$ is a proposition.
\end{proof}
...
$$P,R:S^1\to\mathcal U.$$ The first is the family defined by $P(z)\defequi(z=\base)$ for $z:S^1$ and the second is defined by the induction priciple for the circle by  % $E(\Sloop)(p)=\trans{}_{z,\base,\base}(p)(\Sloop):(z=\base)$
...
\end{example}
\begin{example}
  Recall the finite set $\mathbf 2=\bool:\fin_2$ from \cref{def:finiteset}, containing two elements.   According to \cref{xca:C2}, $\mathbf 2=\mathbf 2$ has exactly two elements, $\refl{\mathrm 2}$ and $\twist$, and doing $\twist$ twice gives you back $\refl{\mathbf 2}$.  We see that this is exactly all the symmtries you'd expect to have in a two point set: you can let everything be ($\refl{\mathbf 2}$) or you could swap the two elements ($\twist$); and if you swap twice everything is let be.  The type $\fin_2$ (of ``finite sets with two elements'') is our embodyment of these symmetries.  

Observe that (by the definition of $S^1$) there is an interesting function $S^1\to\fin_2$, sending $\base:S^1$ to $\mathbf 2:\fin_2$ and $\Sloop$ to $\twist$.
\end{example}
