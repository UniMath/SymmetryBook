...
\begin{example}
  ...
  In the course of our discussion, we will see that the following represent all the connected coverings of the circle.
  \begin{enumerate}
  \item The trivial connected cover  $$c_\base:\bn 1\to S^1$$ sends the unique element of $\bn 1$ to $\base$ is a covering corresponding to the trivial symmetry of $\base$ (\ie only $\refl\base$).  
...
\begin{example}
 \label{ex:listofCcovers}
  ...
  \begin{enumerate}
  \item  The trivial connected cover is still represented by the constant function
    $$c_{(\zet,s)}:\bn 1\to C$$ sends the unique element of $\bn 1$ to $(\zet,s):C$.
  \item Assume that $m:\NN$ is positive.  If $X$ is a type and $f:X\to X$ we define the \emph{ $m$th root}
$$\sqrt[m]X=\bn m\times X$$ and 
$$\sqrt[m]f(i,x)=
\begin{cases}
  (i+1,x)& \text{for $i<m-1$ and}\\(1,f(x))& \text{for i=m-1}:
\end{cases}$$ most of the time $\sqrt[m]f$ increases the element $\bn m$, but when that is no longer possible (one $m$th of the time; hence the term $m$th root) it uses $f$ on $X$, see \cref{fig:root}:
\begin{figure}
  \centering
  \begin{tikzpicture}
    \node (A) at (2,1) {$\sqrt[m]X$};
    \foreach \y in {0,1,2}
...
Note that the function $p:\sqrt[m]{\zet}\to\zet$ given by $p(i,n)=i+mn$ is an equivalence and that $p\sqrt[m]s(i,n)=i+1+mn=sp(i,n)$.  Consequently $(\sqrt[m]{\zet},\sqrt[m]s):\sum_{X:\Set}X=_\UU X$ is in the component of $(\zet,s)$, \ie $(\sqrt[m]{\zet},\sqrt[m]s):C$. % In consequence, if $(X,f):C$ (\ie, $(X,f):\sum_{X:\Set}X=_\UU X$ is in the component of $(\zet,s)$) then also $(\sqrt[m]X,\sqrt[m]f):C$, giving rise to the 
Ultimately, we have defined the
 \emph{degree $m$ function} $$-^m:C\to C,\qquad -^m((X,f))=(\sqrt[m]X,\sqrt[m]f).$$
...
If $(p,!):(X,f)=_C(X',f')$, then $$(p,!):((\sqrt[m]X,\sqrt[m]f)=(\sqrt[m]X',\sqrt[m]f'))$$ is given by $p(i,x)=(i,p(x))$ (and the fact $!:pf=f'p$).  In particular, letting $p\defequi s:\zet=\zet$, we get that $s(i,n)=(i,sn)=(\sqrt[m]s)^m(i,n)$.



 Note that the equivalence $c:S^1\to C$.\footnote{comeback 190102}
  \end{enumerate}
\end{example}
\begin{xca}
...
\begin{remark}
  ...
  Following up our interpretation of the covering $f:A\to S^1$ with contractible $A$ as the exponential function sending a real number $x$ to the point $e^{2\pi i x}$ on the complex unit circle, the analogue of our degree $m$ function is the $m$th power of complex numbers restricted to the unit circle.
  ((picture!!))
\end{remark}
...
\footnote{fill in the rest of the argument!}
\begin{theorem}
