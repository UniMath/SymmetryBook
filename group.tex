...
\eg as the set of $m$th roots of unity in the complex plane, as shown in~\cref{fig:m-cycle-roots}.
\begin{marginfigure}
  \begin{tikzpicture}
...
\end{marginfigure}
Any cyclic permutation is in particular a permutation of $m$-element set underlying the cycle.
...
\emph{all} permutations of the set $\bn m$ with $m$ elements.
The projection map,
whose fiber at $X : \BSG_m$ is the set $\sum_{f:X\to X}\Trunc{(X,f)=(\bn m,\zs)}$,
captures $\CG_m$ as a ``subgroup'' of the permutations, namely the cyclic ones,
corresponding to the fact that the shapes of $\CG_m$ are those of $\SG_m$
...
\[
  R_m : \B\ZZ \to \BSG_m.
\]
Set truncation of \cref{def:set-truncation} provides us with a tool for capturing only the symmetries of $\FinSet_m$ hit by $R_m$:
the (in language to come) subgroup of the permutation group generated by the cyclic permutations $\zs$ is the group
\[
...
\marginnote{More precisely, but using language not yet established: $\CG_m$ is isomorphic to $\ZZ/m\ZZ$, which is the ``quotient group'' (\cf \cref{def:normalquotient}) of $\ZZ$ by the ``kernel'' (\cf \cref{def:kernel}) induced by $R_m$, whereas $\CG'_m$ is exactly the corresponding ``image'' (\cf \cref{sec:image}). Thanks to \cref{lem:kerandcoker} these are isomorphic groups. However, in our special case we can give a proof using only language we know.}
\end{example}

\begin{construction}
  \label{lem:CmisisotoZ/m}
  We defined a pointed equivalence $g : \BCG'_m \ptdto \BCG_m$,
  commuting definitionally with the projection maps to $\BSG_m$.\marginnote{%
    $\begin{tikzcd}[column sep=tiny,ampersand replacement=\&]
      \B\ZZ \ar[rr,dashed]\ar[drrr,bend right,"R_m"'] \& \phantom{1} \&
      \BCG'_m \ar[rr,"g"]\ar[dr,"\prj"'] \& \& \BCG_m\ar[dl,"\prj"] \\
      \& \& \& \BSG_m \&
    \end{tikzcd}$}
\end{construction}

\begin{implementation}{lem:CmisisotoZ/m}
  We'll define $g$ as the totalization (\cref{def:fiberwise})
  of a family of maps
  \[
    f_X : \sum_{z:\Sc}(X = R_m(z)) \to \sum_{f:{X\to X}}\Trunc{(X,f)=(\bn m,\zs)},
  \]
  parametrized by an $m$-element set $X$ (recall $\BSG_m \jdeq \FinSet_m$).
  Note that the domain of $f_X$ is exactly the fiber $R_m^{-1}(X)$.

  Let
  $f_X(z,v) \defequi (v^{-1}\cy m(\Sloop_z)v,!)$
