...
\begin{remark}
  ...
  A $G$-set $X$ is often presented by focusing on the \emph{underlying set} $X(\pt_G)$  and providing it with a structure relating it to $G$ determining the entire function $X\colon BG\to\Set$.

More precisely, since $BG$ is connected, a $G$-set $X\colon BG\to\Set$ factors over the component $\Set_{(X(\pt_G))}\defequi\sum_{Y:\Set}\Trunc{Y=X(\pt_G)}$ which contains the point $X(\pt_G)$.  Since $B\Sigma_{X(\pt_G)}\defequi(\Set_{(X(\pt_G))},X(\pt_G))$ the $G$-set $X$ can, without loss of information, be considered as a homomorphism 
$$G\to\Sigma_{X(\pt_G)}% \defequi(\Set_{(X(\pt_G))},X(\pt_G))
$$ from $G$ to the permutation group $\Sigma_{X(\pt_G)}$ of $X(\pt_G)$.

Conversely, if $X$ is any set \emph{and} we have a homomorphism $G\to\Sigma_X$ (in other words, a pointed map $BG\to B\Sigma_{X}%\defequi(\Set_{(X)},X(\pt_G)
$), then the composite
$$BG\to \Set_{(X)}\to \Set$$
is a $G$-set with $X$ exactly the value of $\pt_G$.
...
\xymatrix{\pt_G\ar@{=}[r]^{\refl{\pt_G}}_\to\ar@{=}[d]_q^\downarrow&\pt_G\ar@{=}[d]^{\princ G(p)(q)}_\downarrow\\
\pt_G\ar@{=}[r]^p_\to&\pt_G}$$
A third $G$-set with underlying type $\pt_G=\pt_G$ is $\mathrm{triv}_G(\pt_G=\pt_G)$.
\end{remark}
