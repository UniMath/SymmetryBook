...
The \emph{principal ${\agp G}$-torsor} $\princ {\agp G}^\abstr$ is the ${\agp G}$-set consisting of the underlying set $\mathrm{under}({\agp G})\defequi S$ together with the homomorphism ${\agp G}\to\abstr(\Sigma_{S})$ with undelying function of sets $S\mapsto (S=S)$ given by sending $g:S$ to $\mapsto \mathrm{ua}(s\mapsto s\cdot g^{-1})$.
...
\footnote{TO BE MOVED TO AFTER Burnside's (\ie a $G$-set splits into orbits) has been covered.}
