...
\end{align*}
((In notation used elsewhere, ``$\preinv(q)(p)=P_q(p)$, $\post(p)(q)=P_x(p)(q)$ and $\adjoint(q)(p)=\Ad(p)(q)$'', but this may be confusing since some of these are seen as secondary (\eg in $P_x$ the $x$ has been largely fixed before) and some primary))
