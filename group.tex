...
\end{lemma}
\begin{proof}
  \footnote{((WRITE))\color{blue}
To the extent that it is needed, also show the characterizing properties of kernels, images and cokernels. Some done in the next section. % In particular, a subgroup is normal iff it is a fixed point under the conjugate action.  
...
\end{definition}
Since the families are equivalent we use $\typesubgroup_G$ or $\typesubgroup_G'$ interchangeably.  There is, however, a little explanation needed in order to see that the type of normal subgroups is equivalent to $\typenormal'_G$.
...
\end{lemma}
\begin{proof}
  Let $G/N$ be the group defined as the component of the groupoid of $G$-sets containing and pointed in $X_{\pt_G}$.  Let $f:\Hom(G,G/N)$ be the homomorphism defined by $Bf(z)=X_z$.  Consider the kernel of $f$,
$$B\ker f_\div\defequi\sum_{z:BG}(X_z=X_{\pt_G})$$
pointed in $(\pt_G,\refl{X_{\pt_G}})$ and with $i_{\ker f}:B\ker f\to_*BG$ given by the first projection.  Consider the map
$$\mathrm{ev}:B\ker f_\div\to B\mathrm{ass}(N)_\div,\qquad \mathrm{ev}(z,f)\defequi(z,f(\pt_z)),$$
which is (strictly) pointed since $\mathrm{ev}(\pt_G,\refl{X_{\pt_G}})\defequi (\pt_G,\pt_{\pt_G})$.  We claim that $\mathrm{ev}$ is an equivalence.  Since $B\mathrm{ass}(N)$ is connected it is enough to sow that the preimage of $(\pt_G,\pt_{\pt_G})$ is contractible.  Since $\mathrm{ev}$ agrees with the projections to $BG$, the preimage is equivalent to $\sum_{f:X_{\pt_G}=X_{\pt_G}}f(\pt_{\pt_G})=\pt_{\pt_G}$, the preimage of the evaluation map 
$\mathrm{ev}_{X_{\pt_G}}:(X_{\pt_G}=X_{\pt_G})\to X_{\pt_G}(\pt_G)$ which we in \cref{lem:conistrans} proved was an equivalence since $X_{\pt_G}$ is transitive. Evoking univalence we get an identification of subgroups between the kernel of $f$ and $(\mathrm{ass}(N),i_N,!)$.
\end{proof}

There are many valuable constructions to be extracted from this proof.
\begin{definition}
...
$$ Our $f:\Hom(G,G/N)$ is the composite of the equivalence $\pathsp{}^G:BG\we\typetorsor_G$ of \cref{lem:BGbytorsor} and the quotient map $-/N$.
\end{remark}
