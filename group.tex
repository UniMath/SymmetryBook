...
\end{lemma}
\begin{proof}
  Since the circle is connected, for any $z,w:S^1$ the statement that $z=_{S^1}w$ is a set equivalent to $z=_{S^1}\base$ being a set, so it is enough to prove the last statement.  Define two type families 
...
\end{example}
\begin{example}
  Recall the finite set $\mathbf 2=\bool:\fin_2$ from \cref{def:finiteset}, containing two elements.   According to \cref{xca:C2}, $\mathbf 2=\mathbf 2$ has exactly two elements, $\refl{\mathrm 2}$ and $\twist$, and $\twist^2=\refl{\mathbf 2}$.
\end{example}
