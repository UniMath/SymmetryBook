...
\MB{This type is called the \emph{classifying type} of the group.}
...
However, a $G$-torsor corresponds to \emph{a} universal \covering
(\MB{problem: u.c. are now pointed}), 
and the important step is to consider the \emph{type} of these.
...
\footnote{%
  ...
  smooth or continuous symmetries of $a$ in $A$
...
different animals.
We will return to this example after we have defined torsors.\MB{Do we?}
\end{remark}
...
\begin{definition}\label{def:typegroup}
  ...
  of the type of pointed connected groupoids $\UUscone$,\marginnote{%
  ...
  In this way we avoid awkward terminology as ``the group of the integers
  is the circle''. The symbol $\mkgroup$ has been inspired by $\loops$
