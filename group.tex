...
\begin{definition}
  \label{def:covering}
  A function $f:A\to B$ between two types is a \emph{covering} if for all $b:B$ the preimage $f^{-1}(b)$ is a nonempty set.  We say that the covering is \emph{decidable} if the preimages are decidable sets.
\end{definition}
...
$$\sum_{A:\UU}\sum_{f:A\to B}||\prod_{b:B}\isset(f^{-1}(b))\times f^{-1}(b)||.$$ 
...
\begin{theorem}
  ...
  The type of decidable coverings of the circle by connected types is equivalent to $\NN$.
\end{theorem}
...
If $0<m$, then the injection $g:(\pt_A=\pt_A)\to\zet$ hits all multiples of $m$.  In particular, if $m=1$, then the injection hits \emph{all} integers, so $g:(\pt_A=\pt_A)\to\zet$ is an equivalence.  Again, by \cref{lem:eqandcovofconntypes} this means that $A\to S^1$ is an equivalence. 
\begin{remark}
...
More generally, if $0<m$, then $g$ gives an equivalence between $\pt_A=\pt_A$ and the multiples of $m$.\footnote{comeback181220: show that $A=S^1$}
