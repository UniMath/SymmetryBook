...
\end{lemma}
\begin{proof}
An identity between two subgroups $i_H:\Hom(H,G)$ and $i_{H'}:\Hom(H',G)$ is an identity $p:H'=_{\typegroup}H$ such that $i_{H'}=i_H\,p$ (a proposition since $\Hom(H',G)$ is a set).
...
$(H,i_H,!)=_{\typesubgroup_G}(H,i_H,!)$ is equivalent to the type of homomorphisms $f:\Hom(H,H)$ which are such that $!:i_H^==i_H^=f^=$ such that $f^=$ is an equivalence (as we see in a moment this last requirement is redundant).  Now, since $(H,i_K,!)$ is a subgroup, $i_H^=$ is an injection of sets, which forces $!:f^==\refl{\pt_H=\pt_H}$, which forces $f$ to be (identical to) the identity homomorphism. 
\end{proof}
...
\begin{lemma}
  Under the equivalence between $G$-sets and $\abstr(G)$-sets, the $G$-set $\typesubgroup_G$ corresponds to the $\abstr(G)$-set 
...
\begin{lemma}
  \label{lem:iso2}
  Let $f:Hom(G,G')$ be a surjective homomorphism with kernel $N$, and let $H$ be a subgroup of $G$.  Let $i_N$ and $i_H$ be the inclusions of $H$ and $N$ in $G$.  Let $i_{HN}:Hom(H \vee N,G)$ be the homomorphism from the sum $H\vee N$\footnote{recall that $B(H\vee H)$ is the pointed sum $BH\vee BN$} to $G$ induced by the inclusions.  Then 
the pullback $BN\times_{BG}BH$
%$$F\defequi \sum_{(x,y):BH\times BN}i_H(x)=i_N(y)$$ 
is equivalent to $\coker i_{HN}\times B(H\cap N)$.
\end{lemma}
((INSERT LEMMA A from whiteboard))
