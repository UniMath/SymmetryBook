...
Conversely, if $(\phi,!):\Hom^\abstr(\abstr(G),\abstr(H))$ we define 
$\concr(\phi,!):\Hom(G,H)$ as follows.  If $X:BG\to\Set$ is a $G$-set, let
$\phi_*X:BH\to\Set$ be given by sending $z:BH$ to the set $\phi_*X(z)\defequi(\pt_H=z)\times_{\pt_G=\pt_G}X(\pt_G)$ \footnote{here I need set-quotients: $(\pt_H=z)\times_{\pt_G=\pt_G}X(\pt_G)$ is the coequalizer of the two maps $(\pt_H=z)\times X(\pt_G)\gets (\pt_H=z)\times{(\pt_G=\pt_G)}\times X(\pt_G)$ sending $(p,q,x)$ to $(p\,f(q),x)$ and $(p,X(q) x)$.}
