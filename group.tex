...
$B\ker f_\div\defequi \sum_{z:BG}\sum_{p:\pt_{G'}=f(z)}||\pt_{\ker f}=(z,p)||$ of $\pt_{\ker f}$ in $F(f)$.  The first projection $B\ker f\to_* BG$ gives a subgroup since the preimages are equivalent to the sets $\sum_{z:BG}\sum_{p:\pt_{G'}=f(z)}||\pt_{\ker f}=(z,p)||$.
\end{definition}
