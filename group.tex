...
$$(Bf)^{-1}(\pt_{G'})\defequi\sum_{z:BG}(\pt_{G'}=Bf(z)):$$
 the cokernel is the set of components and the kernel is a component.  This point of view makes it clear that the kernel is a subgroup whereas there is no particular reason for the cokernel to be more than a ($G'$-) set.
...
\begin{xca}
  ...
  \begin{enumerate}
  \item a surjective group homomorphism (\ie the cokernel is trivial) if and only if the induced map of sets 
...
\begin{lemma}
  ...
  \begin{enumerate}
  \item $N\cap H$ is a normal subgroup of $H$.
  \item The induced homomorphism $H/N\cap H\to G'$ is injective
  \item If $H$ and $G'$ are finite with coprime cardinalities, then $H$ is a subgroup of $N$.
  \end{enumerate}
  \begin{proof}
    \begin{enumerate}
    \item ((WRITE))
    \item ((WRITE: almost the definition of intersection))
    \item Since the induced homomorphism from $H/N\cap H$ to $G'$ is injective, the cardinality of $H/N\cap H$ divides the cardinality of $G'$ ((ref)).  
Since $|H|$ and $|G'|$ are coprime this implies that $|H|$ and $|H/N\cap H|=|H|/|N\cap H|$ ((ref)) are coprime which implies that $|N\cap H|=|H|$.  Since $N\cap H$ is a subgroup of $H$ this implies that $N\cap H=H$ ((write ref)), or in other words, that $H$ is a subgroup of $N$ ((elaborate)).
    \end{enumerate}
  \end{proof}
\end{lemma}
