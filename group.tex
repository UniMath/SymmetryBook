...
\begin{remark}
  ...
  \cref{def:abstractgroup}.  In other words, the existence of inverses is not assumed.  For this reason, the unit laws and the associativity law
  are, together, known as the \emph{monoid laws}.
\end{remark}
...
\begin{remark}
  ...
  \begin{align*}
    ...
    \mathrm{Monoidlaws}(S,e,\mu) & \defequi\mathrm{Unitlaws}(S,e,\mu)\times\mathrm{Assoclaw}(S,\mu{}) \\
  ...
  \begin{align*}
    \typegroup^\abstr \defequi \sum_{S:\UU} & \sum_{e:S}\sum_{\mu{}:S\to S\to S} \sum_{\iota\colon S\to S}
    \left ( \isset{(S)}\times\mathrm{Grouplaws}(S,e,\mu,\iota) \right )
  \end{align*}

  Thus an abstract group $G$ will be a quintuple of the form $G \jdeq (S,e,\mu,\iota,!)$.  For brevity, we will usually omit the proof of the
  properties from the display, since it's unique, and write an abstract group as though it were a quadruple $G \jdeq (S,e,\mu,\iota)$.
\end{remark}
...
\begin{remark}
  ...
  the function $\abstr:\typegroup\to\typegroup^\abstr$ is an equivalence.
\end{remark}

\begin{remark}
  If $\mathcal G=(S,e,\mu,\iota)$ and $\mathcal G'=(S',e',\mu',\iota')$ are abstract groups, an element of the identity type $\mathcal
  G=\mathcal G'$ consists of quite a lot of information.  First and foremost, we need an identity $p:S=S'$ of sets, but from there on the
  information is a list of elements in propositions (this is more interesting for \inftygps).  An analysis shows that this list can be shortened
  to the equations $e'=p(e)$ and $\mu'(p(s),p(t))=p(\mu(s,t))$.  A convenient way of obtaining an identity $p$ that preserves the equations is
  to apply univalence to an equivalence $f: S \equiv S'$ that preserves the equations.  Such a function will be called an abstract
  \emph{isomorphism}.
\end{remark}
