...
by putting by $\symm{}_{a,a}(\refl{a})\defequi\refl{a}$.
\item[T] {\bf Transitivity.} After \cref{def:eq-symm} we {\color{red} suggested to 
define, for any $b,c:A$, the
function $$\trans{}_{a,b,c}:(a=b)\to((b=c)\to(a=c)),$$
we do this here by double induction, putting
$\trans{}_{a,a,a}(\refl{a})(\refl{a})\defequi \refl{a}$.}
\end{enumerate}

To emulate classical notation, recalling we have fixed $a:A$, let's write
 \begin{enumerate}
 \item $e$ instead of $\refl{a}$,
 \item $g^{-1}$ instead of $\symm_{a,a}(g)$, when $g:a=a$, and
 \item $h\cdot g$ instead of $\trans_{a,a,a}(g)(h)$ when $g,h:a=a$.
 \end{enumerate}
 What can we show about these without knowing anything about $A$ and $a$? 
For convenience, here is a list of the results we are aiming for: 
for all $g,g_1,g_2,g_3:a=a$ we will construct elements in all the following
types (was:propositions)
 \begin{enumerate}
 \item $g=g\cdot e$  \qquad(``right unit law'')
 \item $g=e\cdot g$ \qquad(``left unit law'')
 \item $g_1\cdot(g_2\cdot g_3)=(g_1\cdot g_2)\cdot g_3$ \qquad(``associativity law'')
 \item $e=g\cdot \inv g$ \qquad(``law of inverses'').
...
\footnote{\label{ftnt:equality-jdeq}This makes sense since we \emph{defined} $e\cdot e\jdeq e$ 
(or, as it was originally formulated, $\trans_{a,a,a}(\refl{a})(\refl{a})\jdeq \refl{a}$). 
Similarly for $\inv e \jdeq e$, that is, $\symm{}_{a,a}(\refl{a})\jdeq\refl{a}$.}
\end{definition}
\begin{remark}{\color{red}REMOVE?}
...
\begin{definition}\label{def:p3}
  Let $b,c,d:A$ and $g_3:a=b$, $g_2:b=c$ and $g_1:c=d$ elements.
  Then\marginnote{This definition is somewhat more complicated than the first, 
...
we need to apply the rule three times.}
  $$p_3(b,c,d,g_1,g_2,g_3):g_1\cdot(g_2\cdot g_3)=_{a=_Ad}(g_1\cdot g_2)\cdot g_3$$ is the element defined by induction by saying that $p_3(a,a,a,e,e,e)$ is $\refl e:e\cdot(e\cdot e)=(e\cdot e)\cdot e$,
using footnote \ref{ftnt:equality-jdeq}.
\end{definition}
\begin{definition}\label{def:p4}
  Let $b:A$ and $g:a=b$ be elements. Then $p_4(b,g):g\cdot\inv g=_{a=_Aa} e$ is 
the element defined by induction by saying that $p_4(a,e)$ is $\refl e:e=e\cdot e$,
using footnote \ref{ftnt:equality-jdeq}.
\end{definition}

\begin{xca}\label{xca:p2}
    Define $p_2(b,g)$ %and $p_3(a,b,g)$
by exactly the same procedure, completing the list.
\end{xca}

\begin{remark}
{\color{red}NEW:} In the general situation, where $A$ is an arbitrary type with a given element $a:A$,
...
by the additional assumption that $A$ is a set. Then all the laws are propositions
