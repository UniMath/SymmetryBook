...
However, a $G$-torsor correspond to \emph{a} universal \covering and the important step is to consider the \emph{type} of these:
...
\begin{definition}
  ...
  by $\loops X \jdeq \loops(A,a) \defeq (a =_A a)$.
\end{definition}
...
The examples Klein and Lie were interested in were of a type making it admissible to say that a group is the loops $\loops(A,a) \jdeq (a=_Aa)$ for \emph{some} type $A$ and \emph{some} element $a:A$.
...
\begin{definition}\label{def:pt-conn-groupoid}
  The type of \emph{pointed connected groupoids} is the type\marginnote{%
    ...
    \[
      \UU^{>0} \defeq \sum_{A:\UU}\Trunc{A}\times
    \prod_{x,y:A}\Trunc{x=_ay}.\]
    Similar notations with a subscript ``$*$'' indicate pointed types.}
  \[
    \UUscone \defeq \sum_{A:\UU}\sum_{a:A} \isset(a=_Aa)
    \times \prod_{x:A}\Trunc{a=_Ax}.
  \]
\end{definition}
...
We are now ready to define the \emph{type} of groups:
\begin{definition}\label{def:typegroup}
  A \emph{group} is given by pointed connected groupoid $X\jdeq(A,a)$
  via the loop type $\loops(A,a)$;
  the \emph{type of groups},
  \[
    \typegroup \defequi \Copy(\UUscone),
  \]
  is a wrapped copy (cf.~\cref{sec:unary-sum-types})
  of the type of pointed connected groupoids $\UUscone$.
  We rename the constructor $\mkgroup : \UUscone \to \typegroup$,
  so a group $G\jdeq\inc X:\typegroup$ will be referred to simply as
  $\mkgroup X$.
  We rename the destructor $\clf : \typegroup \to \UUscone$, and
  \[
    \clf G \jdeq \clf(\mkgroup X) \defequi X \jdeq (A,a)
  \]
  is referred to as the \emph{classifying type of $G$}.
  The element $\p_G\defequi \pt_{\clf G} \jdeq a$ will be referred to as the \emph{base point}.
  Informally, we may also refer to the type $\clf G_\div \jdeq A$ itself
  as the classifying type of $G$.
\end{definition}

\begin{definition}\label{def:group-symmetries}% or elements
  We define the \emph{symmetries} of a group $G \jdeq \mkgroup X$,
  to be the set $\USym G \defeq \loops X$. In this way we have defined a
  map
  \[
    \USym : \Group \to \Set, \qquad
    \mkgroup X \mapsto \loops X.
  \]
\end{definition}
\begin{remark}\label{rem:aut}
  We are emphasizing that the essential feature of a group
  is the symmetries of the base point.
  And that is why we defined $\Group$ to be a copy of $\UUscone$,
  ...
  The type most often associated to a group $G$ is the set $\USym G$,
  so we use a special notation for the classifying type $\clf G$.
  However, as noted in \cref{sec:unary-sum-types},
  the constructor and destructor pair forms an equivalence $\Group \weq \UUscone$.
  And $\UUscone$ is a subtype of $\UUp$, so in this sense,
  once you know that a pointed type $X$ is a connected groupoid,
  this pointed type gives a classifying type for a group $G\defeq\mkgroup X$,
  and $X$ carries all the information about the group $G$.
\end{remark}
\begin{remark}\label{rem:BG-convention}
  ...
  Since $X\jdeq\clf G$, we shall make a convention to
  always use the variable name $\BG$ instead of $X$,
  and similarly, for a variable $H:\Group$,
  when we use the induction principle for $\Group$,
  we use the variable $\BH:\UUscone$, etc.
  And whenever we introduce a variable $G$ of type $\Group$,
  we immediately and silently use the induction principle
  to get a variable $\BG:\UUscone$ such that $G\jdeq\mkgroup\BG$
  and $\BG \jdeq \clf(\mkgroup\BG) \jdeq \clf G$.\footnote{%
    You see immediately the typographical reason behind this convention:
    The italic letters $B$,$G$ get along nicely,
    while the roman $\clf$ distances itself a bit from its italic friend $G$.}

  Also note that identifications $G=H$ of groups are equivalent
...
\begin{remark}
  ...
  an identification $G = \Aut_{\BG_\div}(\p_G)$, of $G$ with the automorphism
  group of the base point $p_G : \BG$.
\end{remark}
...
is simple:
it is a pointed function of classifying types $\BG\ptdto\BH$, that is a function
$\Bf :\BG\to\BH$ that ``sends $\p_G$ to $\p_H$'',
\ie together with an identification $p:\p_H=_{\BH}\Bf(\p_G)$:
\begin{definition}\label{def:grouphomomorphism}
  ...
  \]
  \ie a wrapped copy of the type of pointed maps of classifying spaces.
  Just as for groups, we rename the constructor
  $\mkhom : (\BG \ptdto \BH) \to \Hom(G,H)$,
  and the destructor $\clf : \Hom(G,H) \to (\BG \ptdto \BH)$.
  We call $\clf f$ is \emph{classifying map} of the homomorphism.
\end{definition}
...
\begin{remark}\label{rem:Bf-convention}
  We shall use a parallel convention for homomorphisms
  as we use for groups. Recall, that because
  $\Hom(G,H)$ is a wrapped copy of $\BG\ptdto\BH$,
  to construct a function $\varphi : \prod_{f:\Hom(G,H)}T(f)$,
  where $T(f)$ is a family of types indexed by $f:\Hom(G,H)$,
  it suffices to consider the case $f \jdeq \mkhom\Bf$,
  where we deliberately use the variable name $\Bf : \BG\ptdto\BH$.

  Also, remember that identifications of homomorphisms $f=_{\Hom(G,H)}h$
  are equivalent to identifications of pointed maps
  $\mathit{\Bf} =_{\BG\ptdto\BH} \mathit{\Bh}$,
  and that the latter are pairs of an identification of (unpointed) maps
  $H : \Bf_\div =_{(\BG_\div \to \BH_\div)} \Bh_\div$ with an identification
  $K : H(\p_G)\Bf_0 =_{(\p_H=\Bh(\p_G))} \Bh_0$.
\end{remark}
...
\begin{definition}\label{def:classifyingspace}
  ...
  is still called the  \emph{classifying type} and $\p_G\defequi \pt_{\BG}$
...
demand
\begin{displaymath}
  \Trunc[\big]{\sum_{b:X(\pt_G)}\prod_{a:X(\pt_G)}\sum_{g:\pt_G=\pt_G}a=g\cdot b},
\end{displaymath}
...
$$\eta_y:f_*\princ G(y)=\sum_{x:\BG}(\Bf(x)=y)\times(\pt_G=x)\simeq (\Bf(x)=y)\oldequiv\princ{\Bf(x)}^H(y).$$  The resulting identity $\bar\eta:f_*\princ G=\princ{\Bf(x)}^H$ shows that for every $G$-torsor $X$ the $H$-type $f_*X$ is an $H$-torsor, defining a map 
...
Since $\princ G(\pt_G)\defequi (\pt_G=\pt_G)$ this defines a pointed function $\rho_G:\BG\to_* \BSigma_{\pt_G=\pt_G}\defequi(\Set_{(\pt_G=\pt_G)},\pt_G=\pt_G)$, \ie a homomorphism from $G$ to the permutation group
