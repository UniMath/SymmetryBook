...
\end{definition}
\begin{remark}
  Through \cref{lem:freeloopspace} we can get a different perspective on the circle which highlights it as an object classifying very simple symmetries.

A type family $S^1\to\UU$ is uniquely given by a type $X:\UU$ together with a $p:X=_\UU X$, with no further requirements on $p$.  We have seen one example, namely the one given by the set $\zet$ of integers together with the element $s:\zet=\zet$ given by the successor.\footnote{brutal honesty would dictate $s=\ua(S)$}  The importance of this example will become apparent when we eventully explain that \emph{the circle is equivalent to the component $C$ of $\sum{X:\UU}X=_{\UU}X$ containing $(\zet,s)$}. 

Heading towards this goal, we investigate this component a bit further.  Firstly, if $(X,f):\sum{X:\UU}X=_{\UU}X$, then the identity type $(\zet,s)=(X,f)$ is $\sum_{p:\zet=X}fp=_{\zet=X}ps$ (the equation $fp=ps$ comes from the fact that equality in the identity type means that the transport of $s$ along $p$ is $f$).  Since $\zet$ is a set, this identity type is a proposition and so our component $C$ is a connected groupoid.  We investigate the symmetries $(\zet,s)=_C(\zet,s)$, and see that this type is equivalent to $\sum_{p:\zet=\zet}sp=ps$.  \footnote{((comebackhere))181218}
