...
\end{lemma}
\begin{proof}
  First and foremost, $f_S$ is actually well defined in the sense that $(S,v^{-1}cy_m(\Sloop_z)v)=(\bn m,s_m)$ is inhabited which is clear in view of the function
  ...
  Note that  once we have shown that $f_S$ has connected fiber, we know that the truncation $\Trunc{f_S}_0$ is an equivalence, % and in view of the commutativity of 
 ...
 \Trunc{\Sigma_{z\colon\Sc^1}(S=cy_m(z))}_0\ar[r]^-{\Trunc{f_S}_0}& \Trunc{\Sigma_{q\colon S=S}\Trunc{(S,q)=(\bn m,s_m)}}_0 }
  $$ and the fact that $\Sigma_{q\colon S=S}\Trunc{(S,q)=(\bn m,s_m)}$ gives us (upon applying $\Sigma_{S:\fin_m}$) the desired equivalence $BC_m\we B\ZZ_m$.

  So to finish the proof whe must show that $f_S$ has connected fiber, and since $\Trunc{S=\bn m\oldequiv cy_m(\base)}$ and that $\Trunc{(cy_m(\base),q)=(\bn m,s_m)}$ implies that $q=\Sloop^k$ for some $k$ it suffices to prove that $f_{cy_m(\base)}$ has connected fibers $f_{cy_m(\base)}^{-1}(cy_m(\Sloop^k),!)=\Sigma_{z:S^1}\Sigma_{v:\bn cy_m(\base)=cy_m z}cy_m(\Sloop^k)=v^{-1}cy_m(\Sloop_z)v$ for every $k$.
...
commutes, and so $cy_m(p)v:cy_m(\base)=cy_m(\base)$ commutes with $cy_m(\Sloop)$ and so $cy_m(p)v$ must be of the form $cy_m(\Sloop^k)$ for some $k=0,1,\dots,m-1$, or alternatively, we have a $\alpha:v=cy_m(p^{-1}\Sloop^k)$.  Hence $(p^{-1}\Sloop^k,\alpha):(z,v,!)=(\base,\refl{})$.
