...
$$\abstr:\Hom(G,H)\to\Hom^\abstr(\abstr(G),\abstr(H))$$
in \cref{def:abstrisfunctor} as the function which takes a pointed map $f:BG\to_*BH$ to the induced map of identity types $f^\abstr=\ap f:(\pt_G=\pt_G)\to(\pt_H=\pt_H)$ together with the proofs that this is an abstract group homomorphism, c.f~\cref{def:abstrisfunctor}
