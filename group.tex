...
$$\coker f:BG'\to\Set,\qquad \coker f(z)\defequi  ||f^{-1}(z)||_0.\footnote{set trunctation} $$ 
...
We say that the homomorphism $f$ is \emph{surjective}\index{surjective! group homomorphism} if $f^{-1}(\pt_{G'})$ is connected (which is equivalent to $\coker f$ being contractible).
%, or equivalently if $\image f\to G'$ is an equivalence.

Consider the element $\pt_{\ker f}\defequi(\pt_G,p_f):f^{-1}(\pt_{G'})$ (where $p_f:\pt_{G'}=f(\pt_G)$ is the part of $f$ claiming it is a pointed map). 
Define the \emph{kernel}\index{kernel}  of $f$ to be the group defined by the pointed component  of $\pt_{\ker f}$ in $f^{-1}(\pt_{G'})$:
$$\ker f\defequi (f^{-1}(\pt_{G'})_{(\pt_{\ker f})},\pt_{\ker f}).
$$ 
Written out,
$B\ker f_\div\defequi \sum_{z:BG}\sum_{p:\pt_{G'}=f(z)}||\pt_{\ker f}=(z,p)||.$  

The first projection $B\ker f\to_* BG$ displays the kernel $\ker f$ as a subgroup of $G$ (\emph{sub}group since the preimages are equivalent to the sets $\sum_{p:\pt_{G'}=f(z)}||\pt_{\ker f}=(z,p)||$).  

A subgroup is said to be \emph{normal}\index{normal} if it is the kernel of a homomorphism.


The \emph{image}\index{image} of $f$ is the subgroup $\image f\defequi \sum_{z:BG'}\coker f(z)$ of $G'$.  
\end{definition}
% \begin{remark}
