...
\begin{lemma}
  Under the equivalence between $G$-sets and $\abstr(G)$-sets, the $G$-set $\typesubgroup$ corresponds to the $\abstr(G)$-set 
$$\sum_{H:\typegroup}\sum_{\phi:\Hom^\abstr(\abstr(H),\abstr(G))}\isprop(\phi^{-1}(e_G))$$ with $g\cdot(H,\phi,!)=(H,c^g\,\phi,!)$ for $g:\abstr(G)$.
\end{lemma}
