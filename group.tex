...
((Tentative section sketching that the sum (``free products'' or ``amalgamated sum'') of two groups is given by the wedge.  Write the introduction and introduce the terms used.  Has several applications that we'll explore later.  The proof below also gives a concrete description of the sum and should perchance be extracted.  Also, the definition of the wedge must be written out  -- the formulation I have chosen is not optimal.))
\label{sec:coprod}
\begin{lemma}
  \label{lem:wedgeofgpoidisgpoid}
  Let $\aut_{A_1}(\pt_{A_1})$ and $\aut_{A_2}(\pt_{A_2})$ be decidable groups, then the wedge sum $\aut_{A_1\vee A_2}(\pt_{A_1\vee A_2})$ is a decidable group.
\end{lemma}
\begin{proof}
That the wedge is connected follows by transitivity of identity, passing through the base point in the wedge.

We must prove that all identity types are sets, which we do by giving an explicit description of the universal covering.  The idea is that an identity in $\pt_{A_1\vee A_2}=x$ can be factored into a string of identities, each lying solely in $A_1$ or in $A_2$.  We define a family of sets consisting of exactly such strings of identities --  it is a set since $A_1$ and $A_2$ are groupoids -- and prove that it is equivalent to the family $P(x)\defequi(\pt_{A_1\vee A_2}=x)$ which consequently must be a family of sets.

  Let $i_1:A_1\to A_1\vee A_2$ and $i_2:A_2\to A_1\vee A_2$ be the two inclusions, let $g:i_1\pt_{A_1}=i_2\pt_{A_2}$ be the imposed identity in the (non-symmetric formulation of the) wedge sum based in $\pt_{A_1\vee A_2}\defequi i_1\pt_{A_1}$.  For ease of notation, let $a_{2k+i}$ denote $\pt_{A_i}$ for $i=1,2$, $k:\NN$.
Define families of sets
$$C_i:A_i\to\Set,\qquad i=1,2$$
by 
$$C_i(x)\defequi\sum_{n:\NN}(a_i=_{A_i}x)\times\prod_{1\leq k\leq n}\sum_{p_k:a_{i+k}=% _{A_{i+k}}
  a_{i+k}}(p_k\neq\refl {a_{i+k}})$$
when $x:A_i$.  Note that $p_k\neq\refl{a_{i+k}}$ makes sense and is a proposition since our groups are decidable.  Define $C_{12}:C_1(a_1)\to C_2(a_2)$ by
$$C_{12}(n,p_0,\dots,p_n,!)=
\begin{cases}
  (n-1,p_1,\dots,p_n,!)& \text{ if }p_0=\refl{a_1}\\
  (n+1,\refl{a_2},p_0,\dots,p_n,!)& \text{ if }p_0\neq\refl{a_1}.
\end{cases}
$$
$C_{12}$ is ((obviously or write out)) an equivalence, and so the triple $(C_1,C_2,C_{12})$ defines a family
$$C:A_1\vee A_2\to\Set.$$
We will show that $C$ is equivalent to $P\defequi P_{\pt_{A_1\vee A_2}}$, which is given by $P(x)=(\pt_{A_1\vee A_2}=x)$, and so that the identity types are sets.

One way is the ``inclusion''; more precisely, 
$$\alpha:\prod_{x:A_1\vee A_2}(P(x)\to C(x))$$ is given by letting identities be considered as strings of length zero: $\alpha_i(i_ia)(p)=(0,p):C_i(a)$.  This is well defined since $\alpha_2(i_2\pt_{A_2})(gpg^{-1})=C_{12}\alpha_1(i_1\pt_{A_1})(p)$ ((is this how you'd say this?)).
The other way, 
$$\beta:\prod_{x:A_1\vee A_2}(C(x)\to P(x))$$ is given by composing the identities, using the glue $g$ to make their ends meet: $\beta_1(n,p_0,\dots,p_n,!)\defequi i_1(p_0)g^{-1}i_2(p_1)gi_1(p_2) g^{-1}\dots $ (ending in $\dots gi_1(p_n)$ if $n$ is even and $\dots g^{-1}i_2(p_n)g$ if $n$ is odd) and likewise for $\beta_2$ and the glue ((write)).
