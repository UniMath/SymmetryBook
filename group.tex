...
See the end of the chapter \cref{sec:grouphistory} for a brief summary of the early history of groups.
\begin{remark}
...
\begin{remark}
  When considering the identity type $a=_Aa$, the elements $x:A$ with $x$ unequal to $a$ are irrelevant, and we are free to consider only the \emph{connected} case, \ie where $x=_Aa$ is always inhabited.
\end{remark}
...
We refer to an element of $\typegroup$ as a \emph{group}.  A group $(A,a,p,q):\typegroup$ will be referred to simply as $\aut_A(a)$.
\end{definition}
...
\begin{example}
  ...
  \item More generally, if $S$ is a set, is there a pointed connected $1$-type $(A,a)$ so that $a=_Aa$ models all the ``permutations'' $S=_{\Set}S$ of $S$.  The only thing wrong about the type $\Set$ (apart from it being large\footnote{how do we deal with that?}) is that it is not connected.  So we \emph{pick
