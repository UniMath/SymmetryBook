...
\begin{definition}
  ...
  \index{loop type}\glossary(Omega){$\protect\loops$}{loop type}
\end{definition}
...
\glossary(UUc){$\protect\UUscone$}{pointed, connected groupoids}
  \[
...
\begin{definition}\label{def:typegroup}
  ...
  \glossary(Omegab){$\protect\mkgroup$}{group constructor}
...
\marginnote{More precisely, but using language not yet established%,
% this is shown in ???\footnote{find reference} as applied to the map $cy_m\colon S^1\to\fin_m$ (interpreted as a ``group homomorphism'' $cy_m\colon \ZZ\to\Sigma_n$)
: $\ZZ/m$ is equivalent to the ``quotient group'' (\cf \cref{def:normalquotient}) of $\ZZ$ by the ``kernel'' (\cf \cref{def:kernel}) of $cy_m$, whereas $C_m$ is exactly the ``image'' (\cf \cref{def:image}) of $cy_m$.  Thanks to \cref{lem:kerandcoker} these are equivalent groups. However, in our special case we can give a proof using only language we know.}
...
\begin{proof}
  ...
  %and the rightmost square commutes by associativity.
...
\begin{remark}
  ...
  \marginnote{$$\xymatrix{\BG\vee\BG\ar[r]^{\text{fold}}\ar[d]_{\text{inclusion}}&\BG\\
      \BG\times\BG\ar@{.>}[ur]}$$}
  offers the alternative definition that a group $G$ is abelian if and only if the ``fold'' map $\BG\vee \BG\to \BG$ factors over the inclusion $\BG\vee\BG\to\BG\times\BG$.
\end{remark}
\begin{xca}
...
% \item $G$ instead of $a=_Aa$,
 ...
 \item $g=g\cdot e$  \qquad(``right unit law'')
   %\footnote{redundant (keep).  If you still want to reinsert the other redundant $g\cdot g^{-1}=e$ and $(g^{-1})^{-1}=g$ we have to do some renumbering.  
...
\begin{definition}\label{def:grouphomomorphism}
  ...
  \glossary(Omegac){$\protect\mkhom$}{homomorphism constructor}
...
\begin{remark}\label{def:grouphomomaxioms}
  ...
  \begin{displaymath}
    ...
    (f(\sh_G)=f(\sh_G)) \xrightarrow{\trp {\inv{p_f}}} \USym H
  \end{displaymath}
...
propositions; hence a homomorphism of abstract group is uniquely determined
...
\marginnote{%
  $f^\abstr\defequi \trp {\inv{p_f}}\ap {\Bf_\div}: \USym G =\USym H$, and so
  \begin{displaymath}
    \xymatrix{%
      \Bf_\div(\sh_G)\ar@{=}[r]^{\ap{\Bf_\div}(g)}_\to \ar@{=}[d]^\uparrow_{p_f} &
      \Bf_\div(\sh_G)\ar@{=}[d]^\uparrow_{p_f}
...
% , in a precise sense, any abstract group corresponds to a group.
...
\label{sec:torsors}
\begin{definition}\marginnote{%\begin{remark}
...
%\footnote{in the appropriate universe}.
\end{theorem}
...
%BID will remove the following comment after May 1 2020
...
\marginnote{%
  ...
  % However, %we prove in a moment that
  % the induced $H$-type of a $G$-torsor is an $H$-torsor, and so, in particular, an $H$-set.
}%\end{remark}
$f_*\BH\to\UU$ by
...
% see that we have shown the desired result if we prove instead $A$ is an equivalence.
...
\marginnote{$$\xymatrix{\USym G\ar[d]^\simeq\ar[r]^{f^\abstr}&\USym H\ar[d]^\simeq\\
...
to
  $$\prod_{x:\BG}\sum_{\phi(x):E(x)=F(x)}\phi(\sh_G)=qp^{-1},$$
  is a proposition.  
% Let us pin this down in our language.
...
We now prove that wedges of decidable groups are decidable groups.
...
\begin{definition}
  ...
  $$G_1\vee G_2\defequi \aut_{A_1\vee A_2}(a_{12}).$$ The homomorphisms $i_1:G_1\to G_1\vee G_2$ and $i_2:G_2\to G_1\vee G_2$ induced from the structure maps  $i_1:A_1\to A_1\vee A_2$ and  $i_2:A_2\to A_1\vee A_2$ are also referred to as \emph{structure maps}.
\end{definition}
\begin{lemma}
