...
but we can cannot make this pointed uniformly in $A$.\footnote{%
  Why not? A construction $p : \prod_{A:\BSG_n}(\Bsgn(A) \eqto \sh_{\SG_2})$
...
\begin{definition}\label{def:sgn-permutation}
  ...
  $\Bsgn(\sigma)$ swaps the elements of the $2$-element set $\Bsgn(A)$, or not.
...
\begin{proof}
  ...
  \ref{it:sign-cycle} now follows via~\cref{xca:perm-prod-transpositions}.
