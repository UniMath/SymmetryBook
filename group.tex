...
some authors only assume the existence of inverses as a property:

(d) for all $g:S$ there exists an $h:S$ such that $g\cdot h =e = h\cdot g$.

(Using the properties (a) and (b) one proves that such inverses are unique:
$h =  h\cdot e = h\cdot(g\cdot h') = (h\cdot g) \cdot h' = e \cdot h' = h'$.)
Note that \cref{def:abstractgroup} requires that $S$ is a set.
In a more general approach, $S$ is an object of a suitable category,
and multiplication and the inverse operation are morphisms in this category.
Property (d) instead of (3) plus (c) does not require the inverse operation to be a morphism. In the next paragraphs we explain why we prefer (3) plus (c). 

Property (d) contains the quantification ``there exists''.
The faithful translation of (d) would translate this with
$\exists$, ``there merely exists''. Then, contrary to
...
Indeed, ``the inverse of $g$'' can only be mentioned when targeting
a propositional goal. 

The following lemma proves that we can define an inverse
operation as in (3) satisfying (c) from a witness of (d).
This means that, a posteriori, we can talk about 
``the \emph{inverse of $g$}'' also if the goal is not a proposition. 
We prefer to avoid such intricacies, and
include the inverse operation in the structure right from the start.
DISCUSS
\end{remark} 
