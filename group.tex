...
In other words, the cyclic group of order $m$ is the (pointed) component of the type of \coverings of the circle containing the $m$-fold \covering.
We analyzed this in \cref{thm:coveringsofS1} and found that this (pointed) component was equivalent to 
$$(\sum_{X:\Set}\sum_{p:X=X}\Trunc{\zet/m = (X,p)},\zet/m).$$

There are other, occasionally more convenient, ways of obtaining the cyclic group $C_n$ of order $n$.
Staying close to the type $C$ above is perhaps the simplest: %\cref{def:S1toC}
$$BC_n\defequi(\sum_{X:\Set}\sum_{p:X=X}\Trunc{(\bn{n},s_n)=(X,p)},(\bn{n},s_n)),$$
where $s_n :\bn{n}=\bn{n}$ is (the identity corresponding to the equivalence given by) 
the cyclic permutation of $\bn{n}$, sending $n-1$ to $0$ and, 
for $0\leq i<n-1$, sending $i:\bn n$ to $i+1$. The role
the successor function plays in $C$ is played by the successor modulo $n$ in $C_n$. 

For yet another way of obtaining $C_n$, consider the function 
$$cy_n:S^1\to\fin_n$$ 
with $cy_n(\base)\defequi\bn n$ and 
$cy_n(\Sloop)\defis s_n$.  
Note that the symmetry $cy_n(\Sloop)$ is cyclic in the sense that 
the $n$-fold iterate $cy_n(\Sloop)^n$ is $\refl{\bn n}$.
...
\[
B_n\defequi(\sum_{S:\fin_n}\Trunc{cy_n^{-1}(S)}_0 ~,~ 
(\bn n,\trunc{(\base,\refl{\bn n})}_0))
\]
where the type can be viewed as the ``image'' of $cy_n$,
except that the truncation is one level higher than we have considered before.
Since $\fin_n$ is a groupoid, $B_n$ is a groupoid. 
  
If we focus on the point $\pt_{B_n}\defequi(\bn n,\trunc{(\base,\refl{\bn n})}_0)$ and 
write out the identity type $\pt_{B_n}=\pt_{B_n}$ of the $\Sigma$-type $B_n$, 
we see that it consists of those $p:\bn n=\bn n$ such that there is 
a $q:\base=\base$ with $p=cy_n(q)$: 
...
$$C_n\defequi\aut_{B_n}(\pt_{B_n}).$$

It is good to know that $B_n$ is connected, so that $(B_n,\pt_{B_n})$ is actually 
AAAthe classifying type [DELETE $BC_n$. NOT YET PROVED: ISOMORPHIC ABSTRACT
 GROUPS HAVE EQUIVALENT CLASSIFYING TYPES] Remember that 
$cy_n^{-1}(S)\defequi\sum_{z:S^1}(S=cy_n(z))$, and notice that the map 
$$e:\sum_{S:\fin_n}cy_n^{-1}(S)\to S^1,\qquad e(S,z,p)=z$$ 
is an equivalence fitting in a commuting diagram [CHECK LOOP?]
$$\xymatrix{
\sum_{S:\fin_n}cy_n^{-1}(S)\ar[d]^e_\simeq\ar[rr]^-{\text{set truncation}}&&
\sum_{S:\fin_n}\Trunc{cy_n^{-1}(S)}_0\ar[d]^{\text{first projection}}&\oldequiv B_n\\
S^1\ar[rr]^{cy_n}&&\,\fin_n.}$$
All preimages of of the top map are all inhabited, and since $S^1$ is connected, 
the source is connected.  
Hence \cref{lem:whenisbasespaceconnected} tells us that $B_n$ is connected too.


% .  Let $(S,z,p):B_n$ be any element; we want to show that $(S,z,p)=_{B_n}(\bn n, \base, \refl\base)$ is not empty, so that $B_n$ is connected. Since $S^1$ is connected there is a $q:z=_{S^1}\base$ so $(cy_n(q)\,p,q,!):(S,z,p)=(\bn n,\base,\refl\base)$ (using that $cy_n(\base)\defequi\bn n$ to compose $p:S=cy_n(z)$ and $cy_n:cy_n(z)=cy_n\base$), $B_n$ is connected.  
...
\begin{definition}\label{def:abstractgroup}
  ...
  \item an element $e:S$ called the \emph{unit};
  ...
  \item a function ${\blank}^{-1}: S\to S$ called the \emph{inverse operation},
    taking an element $g:S$ to its inverse denoted by $g^{-1}$.

Moreover, the above data should satisfy the following properties, for all $g,g_1,g_2,g_3:S$ :
    \begin{enumerate}
    \item %$e$ is a ``neutral element'':
      $g\cdot e=e\cdot g=g$ called the \emph{unit laws};
    \item %satisfying ``associativity'':
      $g_1\cdot(g_2\cdot g_3)=(g_1\cdot g_2)\cdot g_3$ called the \emph{associativity laws};
    \item %inverse
      $g^{-1}\cdot g =e = g\cdot g^{-1}$ called the \emph{inverse laws}.
    \end{enumerate}
%  \item %inverses:
...
Instead of including the inverse operation as part of the structure of a group,
some authors take the existence of inverses as an additional property:

(d) for all $g:S$ there exists a $h:S$ such that $g\cdot h =e = h\cdot g$.

It is then easier to generalize the concept of a group to other categories
than the category of sets. \cref{def:abstractgroup} requires that $S$ is a set.
In a more general approach, $S$ is an object of a suitable category,
and multiplication and the inverse operation are morphisms of this category.
Property (d) would not require the inverse operation to be a morphism.

The first issue in translating (d) to type theory 
is the quantification ``there exists''. 
If we translate this with a sum type, any proof t...
If $S$ is a set, the properties (a)--(c) are propositions.
Since inverses are unique, property (d) is also a proposition.
Actually, proofs of these propositions form also part of the structure,
but we have left them out since their types are propositions.
It is trivial to prove (d) from (3).
The following lemma proves that we can define an inverse
operation as in (3) above from a proof of (d).
For this reason we prefer to include (3) in the structure
right away
...
More precisely, since $BG$ is connected, a $G$-type $X\colon BG\to\UU$ factors over the component $\UU_{(X(\pt_G))}\defequi\sum_{Y:\UU}\Trunc{Y=X(\pt_G)}$ which contains the point $X(\pt_G)$.  Since $B\Sigma_{X(\pt_G)}\defequi(\UU_{(X(\pt_G))},X(\pt_G))$ the $G$-type $X$ can, without loss of information, be considered as a homomorphism 
...
$$\mathrm{isTrans}(X)\defequi\prod_{y:BG}\prod_{a,b:X(y)}\Trunc{\sum_{g:y=y}a=g\cdot b}$$
holds.
\end{definition}
Note that the mention of $y$ is redundant in the definition: by transport it is enough to demand $\prod_{a,b:X(\pt_G)}\Trunc{\sum_{g:\pt_G=\pt_G}a=g\cdot b}$, and furthermore, if the proposition holds for one $b$ it holds for all others by composition.  
...
$$\typetorsor_G\defequi\sum_{X:\Set_G}\Trunc{\princ G=X},$$
...
\end{lemma}
\begin{proof}
The composite of $\pathsp{}$ with the evaluation $\ev_{y,z}:(\pathsp y=\pathsp z)\to \pathsp z(y)$ given by $\ev(f)=f(\refl y)$ sends $q:(y=z)$ to its inverse $q^{-1}:P_z(y)\defequi (z=y)$.  Since inversion is an equivalence we only need to show that $\ev_{y,z}$ is an equivalence.  However, since inversion is an equivalence we must have that $\ev_{y,z}$ is a surjection and we must show that $\ev_{y,z}$ is injective.  

Now, $\pathsp y$ is transitive, so we \emph{do} know by \cref{lem:evisinjwhentransitive} with $X\defequi\pathsp y$ that $\ev_{y,y}$ is injective and so an equivalence.  Since being an equivalence is a proposition and $BG$ is connected, we deduce that $\ev_{y,z}$ is an equivalence.  
...
$$\xymatrix{
  ...
  \underset{E:BG\to\Set}\sum\,\underset{x:BG}\prod\Trunc{(\pt_G=x)=E(x)}\ar@{}[rr]^-{E\mapsto E(\pt_G)}&&
\,\underset{X:\Set}\sum\Trunc{(\pt_G=\pt_G)=X}.
}$$
