...
\begin{proof}%
  ...
  \end{displaymath}
  which we will prove is an equivalence afterwards. To be able to
  express $\Phi$, we need a small gadget about truncations of function
  ...
  $\lceil f_0 \rceil (\settrunc x) = \settrunc{f(x)}$, which suffices
  in order to define $\lceil f_0 \rceil$ entirely because its codomain
  $\setTrunc Y$ is a set. If we assume the axiom of
  choice\footnote{What is the status of AC in this book??}, then
  $\lceil \blank \rceil$ is injective when $Y$ is a set. Let
  ...
  contractible. For any $x:\setTrunc{a=a}$ and any $p:a=a$, one can
  ...
  prove
  $\settrunc p \cdot \settrunc q = \settrunc q \cdot \settrunc p$, or
  ...
  \begin{align*}
    ...
    &\weq \sum_{\alpha:\setTrunc{(a=a)\weq(a=a)}}(a=a) \to 
      \prod_{x:\setTrunc{a=a}}\lceil \alpha \rceil (x) =  x
    \\
    &\weq \sum_{\alpha:\setTrunc{(a=a)\weq(a=a)}} \lceil \alpha \rceil = \left\lceil {\settrunc{\id_{a=a}}} \right\rceil
    \\
...
immediately from \cref{cor:fib-vs-path}(\ref{set-fib-vs-path}) and the
fact that $BG$ is connected.
...
This means that for all $x,y:BG$, $p: pt_H = f(x)$,
$q: pt_H = f(y)$ and $r,s : (x,p) = (y,q)$, we have $r=s$.
Again it suffices to verify this for $x\jdeq y\jdeq\pt_G$.
We can view $r$ as the pair $(g,r')$ and and $s$ as $(g',s')$, 
with $g,g': pt_G = \pt_G$ and $r' : f(g)p = q$
and $s' : f(g')p = q$ (the latter two path types come from transport
in the family $\pt_H = f(\_)$). Thus we see that $f(g)=qp^{-1}=f(g')$.
We now use the assumption (\ref{it:mono}) that $f$ is a monomorphism.
Let $F$ be the free group with one generator, that is,
$BF \jdeq (\Sc,\base)$. Define $h,h': \Hom(F,G) \defeq (BF\ptdto BG)$
by circle induction, setting $h(\base)\jdeq h'(\base)\jdeq\pt_G$
and $h(\Sloop)\defis g$ and $h'(\Sloop)\defis g'$. Both $h$ and $h'$
are trivially pointed by $\refl{\pt_G}$. Using $f(g)=f(g')$ one
proves $fh=fh'$ by circle induction. Cancelling $f$ we get $h=h'$,
and so $g=g'$. Since $r'$ and $s'$ are of propositional type,
we can conclude $r=s$.
...
gives rise to several data. One is a family of paths $p_x : f(g(x)) = f(h(x))$
for every $x:BF$. The other is a path of type $p_{\pt_F} f(g_0) f_0 = f(h_0) f_0$.
The latter path ensures that the transport of the path pointing $fg$ is 
equal to the path pointing $fh$, and yields a path $p': p_{\pt_F} = f(h_0 g_0^{-1})$.
