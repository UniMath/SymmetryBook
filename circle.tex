...
note the way we formulate this so that we don't need to talk about an inverse of $\mZ$) we get an identity $\mZ:(-^mc)(\base)=(c\,-^m)(\base)$.  Likewise, transport of $\id\times s$ along $m$ is $s^m$, so that $\mZ$ verifies the proposition $(-^mc)(\Sloop)=(c\,-^m)(\Sloop)$ too.
