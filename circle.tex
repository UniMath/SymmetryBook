...
\begin{definition}
  \label{def:circle}
The circle is a type $\Sc:\UU$ has an element $\base : \Sc$ and an identification
$\Sloop: \base=\base$. The (higher) induction principle of $\Sc$
states that, in order to prove $T(x)$ for every element $x$ of $\Sc$,
it suffices to give an element $b$ of $T(\base)$ together with an
identification $l$ of type $\trp_{P,\Sloop}(b) = b$. More precisely,
giving such $b$ and $l$ defines a function $f(x):T(x)$ such that
$f(\base)\jdeq b$ and $\apd{f}(\Sloop)=l$. 
\end{definition}

In the special case of the above definition that $T(x)\defeq A$ for some type $A$,
we get a function $f:\Sc\to A$ such that $f(\base)\defeq b$ and $f(\Sloop)= l$.
Here $b : A$ and $l : b=b$, since transport in the TBC.....
...
\begin{lemma}\label{lem:freeloopspace}
  If $A$ is a type, the function $$(\Sc\to A)\to \sum_{a:A}(a=_Aa)\qquad g\mapsto (g(\base),g(\Sloop))$$
is an equivalence.  

In particular, if $a:A$, the function $((\Sc,\base)\to_* (A,a))\to (a=_Aa)$ given by sending $g:\Sc\to A$ to $g(\Sloop)$ is an equivalence. 
\end{lemma}
\begin{proof}
  ((write)) 6.2.9
\end{proof}
\begin{remark}
  A function $\gamma:\Sc\to A$ is often referred to as a \emph{loop} in $A$: the picture being that $\gamma$ throws $\Sloop:\base=\base$ as a lasso in the type $A$.
...
\end{lemma}
\begin{proof}
It suffices to show $\Trunc{\base=z}$ for all $z:\Sc$.
This we do by induction on the circle \cref{def:circle}.
For the base case we take $\refl{\base}$.
The induction step follows immediately as $\Trunc{\base=\base}$
is a proposition.
\end{proof}

In the proof above, note the propositional truncation coming 
...
\begin{lemma}\label{lem:conn-ap-cov}
Let $X$ and $Y$ be connected types and let $f:X\to Y$ be a function.  
Then $f$ is a covering if and only if each induced function 
$\ap{f}: x=x'\to f(x)=f(x')$ is an injection.
\end{lemma}
\begin{proof}
Let $y:Y$. By \cref{xca:isEq-pair=} and \cref{lem:trp-in-fx=Ygx} we have 
\[
((x,p)=(x',p'))\equiv\sum_{q:x=x'}\trp_q(p) = p'\equiv\sum_{q:x=x'}\ap{f}(q)=p^{-1}\ct p'
\]
for all $(x,p),(x',p'):f^{-1}(y)$. This proves the if-part. For the converse,
note that for any $r: f(x)=f(x')$ we have $(x,\refl{f(x)}),(x',r):f^{-1}(f(x))$.
SUSPICIOUS: CONNECTED NOT USED.
\end{proof}
...
We could be more ambitious and ask: what is the \emph{type} of decidable coverings of the circle?  Since the type of coverings is equivalent to $\Sc\to \Set$, and the fact that if $F,F':\Sc\to\Set$, then $F=_{\Sc\to\Set}F$ is the type $\prod_{z:\Sc}(F(z)=_{\Set}F'(z))$ (which is a set\footnote{reference}), we see that the type of decidable coverings of the circle is a groupoid.  We will pin this groupoid down by first identifying the components (of which there turns out to be one for each natural number), and then analyzing one component at a time.
 ...
 Recall the function $c:\Sc\to C$ of \cref{def:S1toC}.  By \cref{thm:S1bysymmetries} we know that $c$ is an equivalence, so classifying coverings of $\Sc$ is equivalent to classifying coverings of $C$.  
