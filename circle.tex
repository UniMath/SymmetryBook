...
Let $m>0$ and $$D_m\defequi\sum_{t:\Sc\equiv\Sc} -^m =_{\Sc\to\Sc} -^m t.$$
Define 
\[
...
as an equivalence. Moreover, the type $-^m =_{\Sc\to\Sc} -^m \id_\Sc$ is equivalent,
by function extensionality and the universal property of the circle for 
the type family $T(x)\defeq (-^m(x) = -^m(x))$, to the type 
$\sum_{p: \base=\base} (\Sloop^m p = p \Sloop^m)$.
The latter type is equivalent to $\base=\base$, and therefore
we can give an element of $-^m =_{\Sc\to\Sc} -^m \id_\Sc$ as $\Sloop^z$ for some $z:\zet$.
Finally, we tacitly coerce any element $k:\bn{m}$ to $k:\zet$
and we give the element as $\Sloop^k$. We use these simplifications also below.)

Claim: for all $t:\Sc\equiv\Sc$ and $Q : -^m = -^m t$ we have
\[
...
since $-^m \neq -^m (-\id_\Sc)$ (for $m=1$ this is again \cref{xca:S1=S1-components}).
In the remaining paragraphs we deal with the case $t\jdeq\id_\Sc$.
We use the equivalence $w: (\base=\base)\to\zet$ that sends ${\Sloop}^k$ to $k:\zet$.

Let $Q: -^m = -^m$, then we have $Q(\base): \base=\base$,
and we can ignore $Q(\Sloop)$ which proves a proposition. 
We have to give a center, and prove that it is a center of contraction.
...
For the second component of the center it suffices to give an inhabitant of 
$(\id_\Sc,Q(\base))=(\id_\Sc,{\Sloop}^k)\jdeq f(k)$. 
We take ${\Sloop}^{-l} : \id_\Sc = \id_\Sc$.
In the following picture we analyse transport in the family $R(t)\defeq(-^m = -^m t)$ (left)
and specialize to the situation at hand (right).

\[
\xymatrix{
t\ar@{=}[d]^{p}_\downarrow
& -^m\ar@{=}[r]^{X}_\rightarrow 
&-^m t \ar@{=}[d]^{(-^m \_)(p)}_\downarrow &&
...
Then ${\Sloop}^{-l'} : \id_\Sc = \id_\Sc$ for some $l':\zet$. By exactly
the same analysis as above we get $w(Q(\base)) = ml'+k'$. Since Euclidean Division
is unique, we get $k'=k$ and $l'=l$. Hence the pair $(k',p')$ is equal to the center. 
\end{proof}
