...
\begin{definition}
  \label{def:circle}
The circle is a type $\Sc:\UU$ with an element $\base : \Sc$ and 
an identification $\Sloop: \base=\base$. For convenience and
...
The choice of $a$ above is referred to as `the base case', and
the choice of $l$ as `the loop case'.
...
If $A$ is a type, the function 
\[
(\Sc\to A)\to \sum_{a:A}(a=_Aa)\qquad g\mapsto (g(\base),g(\Sloop))
\]
is an equivalence.  
