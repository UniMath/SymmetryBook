...
we can give any element of $-^m =_{\Sc\to\Sc} -^m \id_\Sc$ as $\Sloop^z$ for some $z:\zet$.
Finally, we tacitly coerce any element $k:\bn{m}$ to $k:\zet$.
We will frequently use such simplifications in the sequel.)

Claim: for all $t:\Sc\equiv\Sc$ and $Q : -^m =_{\Sc\to\Sc} -^m t$ we have
\[
...
Consider a $Q: -^m =_{\Sc\to\Sc} -^m$; then we have $Q(\base): \base=\base$,
...
In the following picture we analyze transport in the family $R(t)\defeq(-^m = -^m t)$ 
