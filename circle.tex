...
Given $a:A$ and $p:a=a$, we say that the function 
$f\defeq\fst(\ev_A^{-1}(a,p)): \Sc\to A$ 
is \emph{given by} $a$ and $p$. One can always freely substitute $f(\base)$
by $a$. One can also freely substitute $f(\Sloop)$ by $p$ in propositions.
However, substitution of $f(\Sloop)$ by $p$ in a type that is not a
proposition incurs a transport. 
\end{lemma}
\begin{proof}
...
Finally, we show that $(C,-^m,!) =_{\hat C} (\Sc,-^m,!) =_{\hat C} (A,f,!)$.
The first equality follows from \cref{exa:mfoldCcover}.
For the second equality we take the path from $\Sc$ to $A$ by applying UA 
to the equivalence $\check e$, using that the following diagram commutes:
\[
\xymatrix{\Sc\ar[rr]^{\check e}\ar[dr]_{-^m}&&A\ar[dl]^{f}\\
&\,C.&}
\]
...
As connected decidable \coverings, 
$(C,-^1,!) =_{\hat C} (\Sc,c,!) =_{\hat C} (\Sc,c\revers,!) =_{\hat C} (C,id_C,!)$.

it is equal to the identity on $\Sc$ (corresponding to $m=1$): Letting also $c:\Sc=\Sc$ be the identity given by univalence, we have that $\refl{\Sc}=c\cdot c$, so that $c$ itself proves that $\id_{\Sc}$ and $c$ are equal.
\end{remark}
