...
$P_\base$ defines a connected covering of the circle: if $z:S^1$ then $P_\base(z)\defequi (z=\base)$ is the set $\zet$ and the type $P_\base S^1$ is connected (even contractible).
\end{lemma}
\begin{proof}
...
\begin{align*}
  ...
  &=s(\trp_{R,\Sloop^m}0)\footnote{some references needed here}
\end{align*}
