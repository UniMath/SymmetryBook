...
where $s:\zet=\zet$ is the successor.  This may seem circular: ``the circle is equivalent to a component of the type of \coverings over the circle!'' 
...
More generally, the successor $s:\zet=\zet$ corresponds to composition with $\Sloop$
...
$$\uc{\base}:\Sc\to\UU,\qquad \uc{\base}(z)\defequi (\base=z).$$
Using function extensionality and univalence, it suffices to give
a equivalences from $\uc{\base}(z)$ to $R(z)$, for every $z:\Sc$.
...
\begin{definition}
  \label{def:fPtoR}
  The function $f:\prod_{z:\Sc}(\uc{\base}(z)\to R(z))$ is defined by transport: $f(z)(p)\defequi\trp{R,p}0$.\footnote{picture of transport!}
\end{definition}
\begin{definition}
  \label{def:gRtoP}The function $g:\prod_{z:\Sc}(R(z)\to \uc{\base}(z))$ is defined by circle induction: 
$$g(\base)\defequi \Sloop^-:\zet\to(\base=\base)$$ and 
$$g(\Sloop): Q(\Sloop)\, g(\base)\,P(\base)^{-1}=_{Q(\base)\to \uc{\base}(\base)}g(\base)$$ states that if $n:\zet$, then the two elements $Q(\Sloop)\, g(\base)\,P(\base)^{-1}(n)\defequi \Sloop\,\Sloop^{s^{-1}(n)}=\Sloop^{1+n-1}$ and $\Sloop^n$ in $\base=\base$ are equal.
\end{definition}
...
\begin{lemma}
  \label{lem:univisexp}
  The function $f:\uc{\base}\to R$ defined by transport in \cref{def:fPtoR} is an equivalence with inverse $g:R\to \uc{\base}$ defined by circle induction in \cref{def:gRtoP}.  Hence, 
$\uc{\base}$ defines a connected \covering of the circle: $\uc{\base}(z)\defequi (\base=z)$ is a set for all $z:\Sc$ and the type $\uc{\base} \Sc$ is connected (more precisely, by univalence $\uc{\base}(z)$ is equal to the set $R(z)$ and $\uc{\base} \Sc$ is contractible).
\end{lemma}
\begin{proof}
  Firstly we need to give an element $H:g\,f=\id_{\uc{\base}}$, or in other words, that for all $z:\Sc$ and $p:\uc{\base}(z)\defequi(\base=z)$ an element $H(z,p):g(z)\,f(z)(p)=p$.  By the induction principle for the identity type, it is enough to show this for the case $z\oldequiv\base$ and $p\oldequiv\refl\base$ in which case we can set $H(\base,\refl\base)\defequi\refl{\refl\base}$ since
