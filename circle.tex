...
use the concept of coverings, a concept custom-built for 
exposing symmetries (and subsymmetries) of types.  
...
A \emph{covering} of a type $B$ consist of a type $A$, a function $f:A\to B$
and a proof that for all $b:B$ the preimage $f^{-1}(b)$ is a set.
With a formula, given a type $B$, the type of coverings of $B$ is
\[
...
See \cref{fig:covering} for two examples of coverings of the circle.
\end{definition}

By \cref{lem:isEq-pair=} the identity type  
$(A,f,!)=(A',f',!)$ of two coverings of the type $B$ is equivalent to 
\[
\sum_{p_A:A=_{\UU}A'}(f'p_A=_{A\to B}f). \text{WHY NOT f LEFT?}
\]
...
of coverings of $B$, we see that we could alternatively have defined a covering to be a type family $B\to\Set$.\footnote{it sounds almost cosa nostra to claim that coverings and families are the same things}  
...
\begin{figure}
  ...
  \begin{tikzpicture}
    ...
    \end{scope}
  \end{tikzpicture}
  \caption{A visualization of two coverings of the circle}
  \label{fig:covering}
\end{figure}
