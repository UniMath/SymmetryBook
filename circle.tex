...
Note to BID: what about this phrasing, not mentioning truncation?  ((OK by me, but I'm surprised YOU are ok with it))}
: 
\[
...
The composite $c\,-^m$ is given by 
$c\,-^m(\base)=(\zet,s)$ and $c\,-^m(\Sloop)=s^m$ and the composite $-^mc$ is given by $-^mc(\base)=(\bn m\times\zet,\sqrt[m]s)$ and $-^mc(\Sloop)=\id\times s$.
...
note the way we formulate this so that we don't need to talk about an inverse of $\mZ$) we get an identity $\mZ:(-^mc)(\base)=(c\,-^m)(\base)$.  Likewise, transport of $\id\times s$ along $m$ is $s$, so that $\mZ$ verifies the proposition $(-^mc)(\Sloop)=(c\,-^m)(\Sloop)$ too.
