...
If $X$ is a set and $f:X\to X$, we define the $m$-th \emph{root}
\[
...
\[
-^m:\sum_{X:\UU}(X=X)\to\sum_{X:\UU}(X=X),\quad -^m(X,f)=(\bn m\times X,\sqrt[m]f).
\]
We now focus on  $C$, the component of $\sum_{X:\UU}(X=X)$ containing $(\zet,\etop{s})$.
Note that the function $\mZ : (\bn m\times \zet) \to \zet$ 
given by $\mZ(i,n)=i+mn$ is an equivalence and that 
\[
(\mZ\cdot\sqrt[m]s)(i,n) = i+1+mn = (s\cdot\mZ)(i,n),~~\text{for all $(i,n):\bn m\times \zet$}.
\]  
...
Note to BID: what about this explanation?} 
\[
-^m:C\to C,\qquad -^m(X,f)=(\bn m\times X,\sqrt[m]f).%,~~\text{for all $(X,f):C$}.
\]
Note that all reincarnations of the degree $m$ function are denoted the same.
...
the fact $!:(\id\times p)\sqrt[m]f=_{\bn m\times X=\bn m\times X'}\sqrt[m]{f'}(\id\times p)$\footnote{$(i,x)$ is taken to $(i+1,px)$ if $i<m-1$, and $(m-1,x)$ is taken to $(0,f'px)$
and $(0,pfx)$, respectively, which are equal by assumption}.  
In particular, letting $p\defeq \etop{s}:\zet=\zet$, 
we get that $(\id\times s)(i,n)=(i,sn)=(\sqrt[m]s)^m(i,n)$.
