...
note the way we formulate this so that we don't need to talk about an inverse of $\mZ$) we get an identification which we also call $\mZ:(\bn m\times\zet,\sqrt[m]s)=_C(\zet,s)$.  Likewise, transport of $\id\times s$ along $\mZ$ is $s^m$, so that $\mZ$ lifts to an element in
$((\bn m\times\zet,\sqrt[m]s),\id\times s)=((\zet,s),s^m)$.
\end{example}
