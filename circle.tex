...
We already defined a \covering $f:A\to B$ to be universal if $A$ and all $a=_A a$
(for $a:A$) is connected. In the situation where $B$ is a pointed groupoid this 
coincides with the path bundle we have defined more generally.  
...
$\refl{\base}:\base=\base$, $1:\zet$ to $\Sloop:\base=\base$,
and $-1:\zet$ to $\Sloop^{-1}:\base=\base$. More generally,
