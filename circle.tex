...
Since $\zet$ is a set, and being a set is a propostion,
one can prove by circle induction that $R$ is indeed a set bundle.
...
$$R: \Sc\to\Set,\qquad R(\base)\defeq\zet,\, R(\Sloop)\defis \etop{s}$$
is equal to the family
$$\uc{\base}:\Sc\to\Set,\qquad \uc{\base}(z)\defeq (\base=z).$$
What does it mean for $\uc{\base}$ and $R$ to be equal?
They are both functions from $\Sc$ to $\Set$. 
...
\begin{lemma}
  \label{lem:univisexp}
For every $z:\Sc$, the functions $f$ defined in \cref{def:fPtoR} 
and $g$ in \cref{def:gRtoP} are inverse equivalences between
$\uc{\base}(z)$ and $R(z)$.
...
The second statement rephrases that $g(\base)$ from \cref{def:gRtoP} is an equivalence.
\end{proof}
...
$(\zet,s)$ in the type $\sum_{X:\UU}(X=_{\UU}X)$}. 
...
% and so our component $C$ is a connected groupoid. FOLLOWS FROM C=S1
...
$\etop{s}\etop{s}=\etop{s}\etop{s}$ and for the 
...
since $\ap{f}$ preserves these operations.\footnote{%
Using language yet to be introduced, we actually classify the subgroups of the integers.}
We start by listing examples of connected \coverings of the circle.

\begin{example}\label{ex:univS1cover}
...
\begin{definition}\label{def:m-fold-cover}
