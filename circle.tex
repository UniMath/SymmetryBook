...
\footnote{((Let's talk about it: The red did not make sense to me.  What was here before?))\color{red}  
There are several interesting aspects. First,
...
\emph{degree $m$ function} to the component $C$\footnote{((OK by me, but I'm surprised YOU are ok with it)) \color{red}%
...
$$-^mc=c\,-^m\defequi \prod_{z:S^1} c(z)^m=_Cc(z^m),$$
which we produce by circle induction as follows.

Under the equivalence $(S^1\to C)\simeq \sum_{(X,f):C}(X,f)=(X,f)$, the composite $c\,-^m$ is given by $((\zet,s),s^m)$ 
...
$((\bn m\times\zet,\sqrt[m]s),\id\times s)$.
...
note the way we formulate this so that we don't need to talk about an inverse of $\mZ$) we get an identity $\mZ:((\zet,s),s^m)=_C((\zet,s),s^m)$.  Likewise, transport of $\id\times s$ along $m$ is $s^m$, so that $\mZ$ defines an element
$((\zet,s),s^m)=((\bn m\times\zet,\sqrt[m]s),\id\times s)$.  Transporting back via the equivalence to $S^1\to C$ we get the desired commutativity.
\footnote{The $\sqrt[m]s$ should be as they are. I've reformulated to make it more transparent. Are you happy with it now?   ({\color{red} I think $\sqrt[m]s$ should twice be $(\sqrt[m]s)^m$, and that the cicle induction requires that $\mZ$ is invariant under $\Sloop$. That appears to be the same
