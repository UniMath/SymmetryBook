...
the $m$-fold set bundle from \cref{exa:mfoldCcover}. 
Any (other) textbook will tell you that these are all the connected \coverings over 
...
Now apply LPO (\cref{LPO}) with $P(m)\defeq 1$ if $g^{-1}(m+1)$ and $0$ otherwise.
We are then left with two situations (use that $g$ is injective and decidable): either $\pt_A=_A\pt_A$ is contractible or there is a $p_0:\pt_A=_A\pt_A$ with $g(p_0)$ different from $0$. 
