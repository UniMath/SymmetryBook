...
\begin{axis}[height=100mm, axis lines=none,line width=1pt, axis equal image]

\node at (-10,13) () {$\zet$};
\node [] at (-10,12) (A) {};

\addplot[domain=0:10*pi,samples=500,black,no marks,thick] 
...
\addplot[domain=0:2*pi,samples=500,black,no marks,thick] 
({10*cos(deg(x))}, % x-coordinate
...
\end{axis}
\end{tikzpicture}
\caption{Figure in Progress (slows down typesetting)
...
\begin{definition}
  \label{def:gRtoP}The function $g:\prod_{z:\Sc}(R(z)\to \uc{\base}(z))$ is 
defined by circle induction: 
\[
g(\base)\defequi \Sloop^{-}:\zet\to(\base=\base) : n \mapsto {\Sloop^n}
\]
and 
\[
...
$$
This follows by a simple calculation: $\Sloop\,\Sloop^{n-1} = \Sloop^n$, for all $n:\zet$. \end{definition}
