...
Then $\pathpair {p_2}{q_2} \cdot \pathpair {p_1}{q_1} = \pathpair {p_2 \cdot p_1}{q_2 \cdot^o q_1} $.
\end{lemma}
\begin{proof}
...
%     Let $m$ be a positive integer.  
% Recall the $m$-fold \covering, either in the guise $\dg{m}:\Sc\to \Sc$ of \cref{exa:mfoldS1cover} or $\dg{m}:C\to C$ of \cref{exa:mfoldCcover}.
% We are going to investigate the set
% $$D_m\defequi\sum_{g:\Sc=\Sc}\dg{m}g=_{\Sc\to \Sc}\dg{m}$$  
...
% $$F_m\defequi\sum_{t:\Sc\to C}\dg{m}t=_{\Sc\to C}\dg{m}c$$
...
% Given If $t:\Sc\to C$ is given by $t(\base)\defequi (Y,g,!)$ (where $!$ asserts that $(Y,g)$ lies in the component of $(\zet,s)$)  and $t(\Sloop)=(p,!,!):(Y,g,!)=_C(Y,g,!)$, (where $p:Y=Y$ and $pg=gp$ (an identity in the set $Y=Y$)), we study the type $\dg{m}t=_{\Sc\to C}\dg{m}c$.
% Spelling out the details we see that $\dg{m}c$ is defined by 
% $$\dg{m}c(\base)=(\bn m\times\zet,\sqrt[m]s,!)$$ and 
% $$\dg{m}c(\Sloop)\defequi(\id\times s,!,!):(\bn m\times\zet,\sqrt[m]s,!)=_C(\bn m\times\zet,\sqrt[m]s,!)$$   and $\dg{m}t$ is given by $$\dg{m}t(\base)\oldequiv(\bn m\times Y,\sqrt[m]g,!)$$ and 
% $$\dg{m}t(\Sloop)\oldequiv(\id\times p,!,!):(\bn m\times Y,\sqrt[m]g,!)=_C(\bn m\times Y,\sqrt[m]g,!).$$

% An element in $Q:\dg{m}c=\dg{m}t$ is then given by a 
...
% An identity  $R:(t,Q)=_{F_m}(t',Q')$ consists of a $\rho:t=t'$ and an $s:(\dg{m}\rho)Q=_{\dg{m}c=\dg{m}t'}Q'$ (note that this is a proposition), \ie $R$ is given by an $r:Y=Y'$ with 
...
% This also shows another thing: the $t:\Sc\to C$ fitting in pairs $(t,Q):F_m$ are all equivalences, so that $F_m$ actually is equivalent to the type $\sum_{g:C=C}\dg{m}g=_{C\to C}\dg{m}$ we are trying to understand.
...
\subsection*{Marc's proof of Thm 3.7.2(2) not using the type C}
...
The latter type is equivalent to $\base=\base$, and therefore
...
Claim: for all $t:\Sc\equiv\Sc$ and $Q : \dg{m} =_{\Sc\to\Sc} \dg{m} t$ we have
\[
\iscontr(\sum_{k:\bn{m}} (t,Q) = f(k)).
\]
This claim is a proposition and means that $f$ is an equivalence.
...
\begin{xca}\label{xca:S1=S1-eqv-S1+S1}
Give an equivalence from $\Sc\amalg\Sc$ to $\Sc\equiv\Sc$.
Hint: use \cref{xca:S1=S1-components} and \cref{lem:S1-delooping}.
\end{xca}
