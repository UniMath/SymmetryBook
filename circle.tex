...
$$P_\base,R:S^1\to\UU$$ (in the end we'll see that, both families have values in sets).  
The first is the family defined by $P_\base(z)\defequi(\base=z)$ for $z:S^1$. If we knew \`a priori that $S^1$ were a groupoid, we could already say that $P_\base$ represented the universal cover.  Regardless of this we know by \cref{lem:thepathspaceiscontractible} that $P_\base S^1\defequi\sum_{z:S^1}(\base=z)$ is contractible.

The second family is defined by the induction priciple for the circle by  % $E(\Sloop)(p)=\trans{}_{z,\base,\base}(p)(\Sloop):(z=\base)$
$R(\base)\defequi\zet$ and $R(\Sloop)\defequi s:\zet=\zet$, where $s$ is the successor function.
