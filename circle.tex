...
$p: f(\base)= g(\base)$ and $q: f(\Sloop) = p\ct g(\Sloop)\ct p^{-1}$.
...
By $q$ we have $f(\Sloop)^{-1}\ct p\ct g(\Sloop)= p$.
Now apply \cref{lem:trp-in-fx=Ygx}. To finally get $\el(\ev(f))=f$,
...
\begin{definition}
  \label{def:covering}
  A function $f:A\to B$ between two types is a \emph{covering of $B$}
if for all $b:B$ the preimage $f^{-1}(b)$ is a  set.  
...
\end{lemma}
\begin{proof}
Let $y:Y$. By \cref{lem:isEq-pair=} and \cref{lem:trp-in-fx=Ygx} we have 
\[
