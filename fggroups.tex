...
\begin{xca}
  Complete the definition of $\rho_S$ by nested induction on words.
\end{xca}
...
\begin{definition}
  ...
  \begin{align*}
    ...
    \sem{wa}                    &\defeq \sem w \Sloop_a,
                                &\text{for $a:S$,}& \\
    \sem{w\bar a}\jdeq \sem{wA} &\defeq \sem w \Sloop_a^{-1},
                                &\text{for $a:S$.}&\qedhere
  \end{align*}
\end{definition}
...
\begin{theorem}
  Fix a decidable set $S$.
  The interpretation map $\llbracket\blank\rrbracket$ respects the equivalence
  relation, and induces an equivalence denoted the same way,
  $\llbracket\blank\rrbracket : \mathcal R_S \to \UFG_S$.
\end{theorem}
