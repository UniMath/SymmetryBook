...
We can follow that same strategy as in~\cref{lem:wedgeofgpoidisgpoid}
...
\begin{definition}
  ...
  taking a list $\ell$ and an element $t:T$ to the extended list
  consisting of $\ell$ with $t$ tacked on at the end.
\end{definition}
...
\]
When we interpret these as symmetries in $\BFG_S$, the words
$aA$ and $Bb$, etc., become trivial.
\begin{definition}
  A word $w : \tilde S^*$ is called \emph{reduced}
  if it doesn't contain any consequence pair of complementary letters.
  The map $\rho : \tilde S^* \to \tilde S^*$ maps a word to its \emph{reduction},
...
The kernel of $\iota$ is the \emph{2-sided Dyck language} $\mathcal D_S$.\footnote{%
  ...
  It's customary to pick $\tilde S=\set{\textup{(},\textup{)}}$.
  The 1-sided Dyck language for this alphabet consists
  of all \emph{balanced} words of opening and closing parentheses,
  like (),(()),()(), etc.}
