...
so that $\bar a \jdeq A$ and $\bar A \jdeq a$.
...
The kernel of $\iota$ is the \emph{2-sided Dyck language} $D_S$.\footnote{%
  Perhaps the \emph{1-sided Dyck language} is more familiar
  in language theory: Here, $S$ is considered as a set of
  ``opening parentheses'', while the complementary elements of $S^*$
  are ``closing parentheses''.
  It's customary to pick $S^*=\set{\textup{(},\textup{)}}$.
