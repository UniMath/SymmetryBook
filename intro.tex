...
\begin{tikzpicture}
  ...
  \end{scope}
  \draw[casblue] (0,.35) ellipse (2.4 and 0.9);
  \draw[pink] (2.4,0.35) .. controls ++(.1,-.5) and ++(.2,.4) .. (2,-1.12);
  \draw[pink] (1.2,0.1) .. controls ++(.1,.4) and ++(-.1,.5) .. (2.4,0.35);
  \draw (0,0) ellipse (3 and 1.5);
  \begin{scope}
    \clip (0,-1.8) ellipse (3 and 2.5);
    \draw (0,2.2) ellipse (3 and 2.5);
...
and pinned to $x_0$. In the picture we've drawn such a rubber band (in pink)
...
serve only as inspirations and are very helpful when trying to discover proofs.
