...
\item If $p$ and $q$ are paths, then $(p,q)$ means $(p,q)^=$.
\item If $e$ is a pair of a function and a proof, we also use $e$ for the function.
\item If $e$ is an equivalence between types $A$ and $B$, we also use $e$ for the
identification of $A$ and $B$ induced by univalence.
\item If $p: A= B$ with $A$ and $B$ types, then we also use $p$ for $\cast(p): A\equiv B$.
\item If $G$ is the group $(A,a,p,q)$, then $g:G$ means $g: a=_A a$.
\item If $X$ is $(A,a,\ldots)$ such that $(A,a)$ is a pointed type, 
then $\pt_X$ and even just $\pt$ mean $a$. 
\end{itemize}
