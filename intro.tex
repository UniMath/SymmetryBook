...
the solid, workable tool, which is the topic \MB{purpose?} of this book.
...
let alone to define mathematically.  For instance, what is the ``type of all squares'' which we discussed earlier, representing all rotational symmetries of $\square$?  While it is harder to draw, you have perhaps already visualized it as the type of all squares in the plane, with $\square$ being the shape the loop must start and stop in. \MB{Refine to exclude the reflections. E.g., make the plane two-sided with each side a different color (I was inspired by shifting
the car wheels recently :-)} In addition, we see that any symmetry can be reached by doing the $90$\textdegree-rotation a few of times, together with the fact that taking any loop four times reduces to not doing anything at all: it represents the ``cyclic group of order four''.
\MB{Check this for the colors as well. NB the square has then two colors.}
