...
$\Sloop$ by an arbitrary $p:\base\eqto z$, $z:\Sc$.\footnote{
$\varepsilon_A(p) : \refl{\pt_A}\eqto \cst{\pt_A}(p)\cdot\refl{\pt_A}$,
so $\loops(f)(\varepsilon_A(p))$ is a path from 
$\loops(f)(\refl{\pt_A})\jdeq \refl{\pt_B}$ to 
$\loops(f)(\cst{\pt_A}(p)\cdot\refl{\pt_A}) \jdeq
f(\cst{\pt_A}(p)\cdot \refl{\pt_A})\cdot \refl{\pt_B}$, since
$f_\pt \jdeq \refl{\pt_B}$. Also, $\iota(p)$ has the right type.} 
