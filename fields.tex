...
Let $\mathscr R$ be an abstract ring consisting of an abstract group 
$\agp R$ with underlying set $R$ and a monoid $(R,1_R,\cdot_R)$, 
and likewise $\mathscr S$ consisting of an abstract group $\agp S$ 
and a monoid $(S,1_S,\cdot_S)$.
An \emph{abstract ring homomorphism} from $R$ to $S$ is an abstract homomorphism
$f:\absHom(\agp R,\agp S)$ that is a monoid homomorphisme
...
\subsection{Rings with coherent left and right multiplication}
\label{sec:altring}
...
\section{Concrete rings}\label{sec:concrings}
