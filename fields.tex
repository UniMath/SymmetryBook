...
\subsection{Move to better place (Ch.\ 11 or 2)}
...
\]
Assume also $h: X\ptdto Y$ and let $(k,k_\pt): H(f,g)$ and 
$(k',k'_\pt): H(g,h)$. Define the \emph{pointwise composition}
$(k',k'_\pt)\cdot_{\ptw} (k,k_\pt)$ of type $H(f,h)$ by\footnote{%
We'll use the notation ``$\cdot_{\ptw}$'' also for $k'$ and $k$.}
\begin{align*}
\bigl((k',k'_\pt)\cdot_{\ptw} (k,k_\pt)\bigr)&\defeq
\bigl((k'\cdot_{\ptw} k ~,~ k'_\pt\cdot\ap{(k'(\pt_X)\cdot\blank))}(k_\pt)\bigr),
\quad\text{where}\\
(k'\cdot_{\ptw} k) &\defeq (x\mapsto k'(x)\cdot k(x)).
\end{align*}
In ...
\end{definition}
...
\begin{marginfigure}
  ...
  \pt_Y\ar[rr,eqr,"f_\pt"]\ar[ddrr,eql,"g_\pt"']%\ar[dd,"{\jdeq}"']
  \& \& f(\pt_X)  \ar[dd,eqr,"k(\pt_X)"]
  \\ \& \mbox{} \& \\
  \pt_Y\ar[rr,eql,"g_\pt"']\ar[ddrr,eql,"h_\pt"']    
  \& \mbox{} \& g(\pt_X)\ar[dd,eqr,"k'(\pt_X)"]
  \\\& \mbox{} \& \\
 \&\& h(\pt_X)
  \end{tikzcd}
  \caption{Tag 1: $k'(\pt_X)$, tag 2: $k'(\pt_X)$.} 
\end{marginfigure}
...
\]
%\marginnote{\MB{Aside. } Given $p:f\eqto_{A\to B}f'$, $q:g\eqto_{B\to C}g'$}
...
\MB{Goal comes out if $p'(\base)_\pt,~q'(\base)_\pt$ are
reflexivity paths. W.i.p.}
%$\inv{(pq)'(\base)_\pt} = \inv{p'(\base)_\pt}\inv{(\ap{p'(\base)}(q'(\base)_\pt))}$. %unfoldings, hopefully not needed
%\begin{align*}
%\inv{p'(\base)_\pt}\cdot\inv{(\ap{p'(\base)}(q'(\base)_\pt))}\cdot
%\ev_{\base}(p'(g)q'(g))\cdot \ap{p'(\base)}(q'(\base)_\pt)\cdot p'(\base)_\pt 
%=\\ \inv{p'(\base)_\pt} \cdot \ev_{\base}(p'(g)) \cdot {p'(\base)_\pt} \cdot
%\inv{q'(\base)_\pt} \cdot \ev_{\base}(q'(g)) \cdot {q'(\base)_\pt}.
%\end{align*}
\end{proof}
...
$$\loops(X\ptdto Y) \to \loops(\loops X\ptdto \loops Y)$$
constructed in the first approach. \MB{W.i.p.}
\end{remark}
