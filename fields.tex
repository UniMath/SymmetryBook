...
\MB{NB pointing paths inverted.} We reproduce
...
\MB{Reversed order because of path inversion.}   
}
...
type $\base\eqto\base$  %$\id_{\BG_\div}\eqto\id_{\BG_\div}$
\[
...
can be seen to be true using commutativity. \MB{W.i.p.}




%$\inv{(pq)'(\base)_\pt} = \inv{\ptoe p(\sh_H)_\pt}\inv{(\ap{\ptoe p(\sh_H)}(\ptoe q(\sh_H)_\pt))}$. %unfoldings, hopefully not needed
%\begin{align*}
%\inv{\ptoe p(\sh_H)_\pt}\cdot\inv{(\ap{\ptoe p(\sh_H)}(\ptoe q(\sh_H)_\pt))}\cdot
%\ev_{\base}(\ptoe p(g)\ptoe q(g))\cdot \ap{\ptoe p(\sh_H)}(\ptoe q(\sh_H)_\pt)\cdot \ptoe p(\sh_H)_\pt 
%=\\ \inv{\ptoe p(\sh_H)_\pt} \cdot \ev_{\base}(\ptoe p(g)) \cdot {\ptoe p(\sh_H)_\pt} \cdot
%\inv{\ptoe q(\sh_H)_\pt} \cdot \ev_{\base}(\ptoe q(g)) \cdot {\ptoe q(\sh_H)_\pt}.
%\end{align*}
\end{proof}
