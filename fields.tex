...
map from \cref{def:dgm-map} as a special case.}
...
of an untyped (curried) function of two arguments has
the following effect on the application on paths.

\begin{construction}\label{con:swap-on-paths}
Let $A,B,C$ be types and $f:A\to(B\to C)$ a map.
Define the map $g:B\to(A\to C)$ by swapping the arguments of $f$,
\ie $g(b)(a)\defeq f(a)(b)$ for all $a:A$ and $b:B$. 
Let $q:b\eqto b'$ be a path in $B$. 
Then we have an identification of the dependent functions
$f(\blank)(q)$ and $\ptw(g(q))$.\footnote{%
$f(\blank)(q)$ is short for the function mapping $a:A$ to the
path $\ap{f(a)}(q): (f(a)(b)\eqto f(a)(b'))$.
Also, $\ptw(g(q))$ is the function mapping $a:A$ to the path
$\ptw(\ap{g}(q))(a): (g(b)(a) \eqto g(b')(a))$.
By the definition of $g$, the types $f(a)(b)\eqto f(a)(b')$
and $g(b)(a) \eqto g(b')(a)$ are definitionally equal.}  
\end{construction}
\begin{implementation}{con:swap-on-paths}
We use induction of $q$, taking $b'\jdeq b$ and $q\jdeq\refl{b}$.
Using function extensionality, let $a:A$.
Then $f(a)(\refl{b})\jdeq\refl{f(a)(b)}\jdeq\refl{g(b)(a)}
\jdeq\ptw(\refl{g(b)})(a)\jdeq\ptw(g(\refl{b}))(a)$.
\end{implementation}
...
\begin{marginfigure}
  ...
  \end{tikzcd}
  \caption{\label{fig:ulrik}Fill ($i$ is path reversal)!} 
\end{marginfigure}
...
(\cst{\pt_{\BB G}},\refl{\pt_{\BB G}}).\qedhere
\]
\end{definition}
...
$(\cst{\pt_{\BB G}},\refl{\pt_{\BB G}})$ of $\grpHom(H,G)$ by $\sh$. 
The composite of the following chain of equivalences 
