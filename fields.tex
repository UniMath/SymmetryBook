...
$\loops((f:X\ptdto Y) \mapsto \loops(f))$, defined by
\[
(\loops(\loops))(q)\defeq\inv\pi\cdot\ap{f\mapsto\loops(f)}(q)\cdot\pi
\quad\text{for any $q:\loops(X\ptdto Y)$}.
\]
We can further elaborate $\ap{f\mapsto\loops(f)}(q)$:
its type is (by the above) equivalent to 
...
$\ap{f\mapsto\loops(f)}(q)$ corresponds to $\ptw(q)$ \MB{Check!}
\end{remark}
