...
\begin{marginfigure}
  \begin{tikzcd}[ampersand replacement=\&,column sep=small]
  \Sc(X\ptdto Y)\ar[rr,eqr,"{\ptw_*}"]\ar[dd,"{O\circ\blank}"']
  \& \& X\ptdto \Sc Y  \ar[dd,"{\blank\circ\wip{\blank}}"]
  \\ \& \mbox{} \& \\
  \Sc(\Scc X\ptdto \Scc Y) \ar[rr,eql,"{\ptw_*}"']   
  \& \mbox{} \& \Scc X \ptdto \Sc (\Scc Y)
  \end{tikzcd}
  \caption{\label{fig:bjørn}
  Fill! Here $\Sc A$ means $\Sc\ptdto A$ and the colors track
  related occurrences of $\Sc$. It is expected that
...
Compostion with $O$, $(O\circ\blank)\defeq(q\mapsto O\circ q)$,
gives the desired map of type\footnote{
...
\]
The right vertical arrow in \cref{fig:bjørn} composes
$f:X\ptdto(\Sc\ptdto Y)$ with $p:\Scc\ptdto X$ to
$(f\circ p):\Scc\ptdto (\Sc\ptdto Y)$. Note that the latter type
and the corresponding type in \cref{fig:bjørn} cannot be consistently
\wip{colored}: here we expect $-\id_\Sc$ to be used.
