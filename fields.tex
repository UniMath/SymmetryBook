...
by applying $\cst{\ast}^\Sc$ to the path $(f_\pt,\refl{f_\pt}')$
between $(\pt_B,\refl{\pt_B})$ and $(f(\pt_A),f_\pt)$ 
in the type $\sum_{x:B}(\pt_B\eqto x)$.\footnote{%
Here $\refl{f_\pt}' : f_\pt \cdot \refl{\pt_B} \eqto f_\pt$
is defined by induction on $f_\pt$, 
setting $\refl{\refl{\pt_B}}'\defeq\refl{\refl{\pt_B}}$.
This minor adjustment of $\refl{f_\pt}$ is needed since
$f_\pt \cdot \refl{\pt_B}$ is not definitionally $f_\pt$.}
The situation is illustrated in the diagram below and we define 
$\OO(f)_\pt\defeq\ap{\cst{\ast}}(f_\pt,\refl{f_\pt}')$.
\end{definition}
\[
\begin{tikzcd}[ampersand replacement=\&,column sep=small]
   (\pt_B,\refl{\pt_B})
    \ar[r,mapsto,"{\cst{\ast}}"]
    \ar[d,eql,"{(f_\pt,\refl{f_\pt}')}"']
  \& (\cst{\pt_B},\refl{\pt_B})\jdeq\pt_{\OO B}
    \ar[d,eqr,"{\ap{\cst{\ast}}(f_\pt,\refl{f_\pt}')}"]
\\
  (f(\pt_A),f_\pt)
    \ar[r,mapsto,"{\cst{\ast}}"] 
...
Identify $\OO(f)_\pt$ with $\inv{\ptw_*}(\cst{f_\pt},\refl{f_\pt}')$.
\end{xca}
...
Define $\Sc:\UUp\to\UUp$ by $\OO A\defeq (\Sc\ptdto A)$, $A:\UUp$.\qedhere 
\end{definition}
...
Let $A$ and $B$ be pointed types and $f :{A\ptdto B}$ a pointed map.
For every $b:B$ and $p:\pt_B\eqto b$, define the type $T(b,p)$ by
\[
T(b,p)\defeq\bigl((\cst{\pt_B}(\Sloop)\cdot\refl{\pt_B})\eqto_{\loops B}
(\inv p \cdot\cst{b}(\Sloop) \cdot p)\bigr).
\]
We have a reflexivity path $r$ in $T(\pt_B,\refl{\pt_B})$,
and $\ev(O(f)_\pt)$ is an element of $T(f(\pt_A),f_\pt)$.\footnote{%
See \cref{def:O-functor} and apply $\ev$ to the endpoints of $O(f)_\pt$.}
...
$f_\pt\jdeq\refl{\pt_B}$.\footnote{%
\MB{NB (omit?)} We use $\pt_B$ as a convenient abbreviation of $f(\pt_A)$.
A more correct view is that $\pt_B$ is a variable that gets
instantiated by $f(\pt_A)$.}

In this case the transport is along
the reflexivity path and it suffices to identify $\ev(O(f)_\pt)$ with $r$.
...
\ev(O(f)_\pt) &\eqto \ptw(\ap{\loops}(\refl{\pt_{\Sc\ptdto B}}))(\Sloop)\\
         ...
         &\jdeq \refl{\loops(\cst{\pt_B},\refl{\pt_B})(\Sloop)} 
          \jdeq \refl{(\cst{\pt_B}(\Sloop)\cdot\refl{\pt_B})} \jdeq r.\qedhere
\end{align*}
\end{implementation}

In \cref{def:O-functor} we have defined a 
...
\begin{marginfigure}
  \begin{tikzcd}[ampersand replacement=\&,column sep=small]
  \OO A\ar[rr,"O(f)"]\ar[dd,equivl,"\ev_A"']
...
Elaborating the situation in \cref{fig:Omega-O-elab}, we have to
identify (1) and (2) for all $(p,p_\pt)$ by a map $i(f)$ and
fill the following diagram:\footnote{We often leave out the ``$\ap{\blank}$''.} 
\[
\begin{tikzcd}[ampersand replacement=\&]
  \& \&\inv{f_\pt}\cdot f(\cst{\pt_A}(\Sloop)\cdot \refl{\pt_A})\cdot f_\pt  \ar[dddd,eqr,"{i(f)(\cst{\pt_A},\refl{\pt_A})}"]
\\
...
\end{tikzcd}
\]
Our task can be simplified by applying path induction on $f_\pt$,
that is, setting $\pt_B\jdeq f(\pt_A)$ (also used as shorthand)
and $f_\pt\jdeq\refl{\pt_B}$. This yields the following diagram,
with $\iota(\Sloop)$ explained below:
\[
\begin{tikzcd}[ampersand replacement=\&]
  \& \&f(\cst{\pt_A}(\Sloop)\cdot \refl{\pt_A})\cdot \refl{\pt_B}  
   \ar[dddd,eqr,"{\iota(\Sloop)}"]
\\
  \& \refl{\pt_B}
   \ar[ru,eqr,"{\loops(f)((\ev_A)_\pt)}"]
\\
...
\end{tikzcd}
\]
By \cref{con:ev(O(f)pt)-refl} we can identify $\ev(O(f)_\pt)$ with
a reflexivity path. Thus, recalling \cref{rem:pointing-ev} for the pointing 
paths $(\ev_A)_\pt$, $(\ev_B)_\pt$, we have to fill the following diagram:

%$\varepsilon_B(\Sloop)\eqto\iota(\Sloop)\cdot\loops(f)(\varepsilon_A(\Sloop))$,

\[
\begin{tikzcd}[ampersand replacement=\&]
  \&f(\cst{\pt_A}(\Sloop)\cdot \refl{\pt_A})\cdot \refl{\pt_B}  
  \ar[dd,eqr,"{\iota(\Sloop)}"]
 \\
\refl{\pt_B}
...
Note that $\iota(\Sloop)$ comes from $i(f)$, the identification of 
(1) and (2) in \cref{fig:Omega-O-elab}, which can be obtained by easy
path algebra.\footnote{\label{ft:path-alg-refl}%
In evidence: in (1), distribute the application of $f$ on paths over the
composite $(\inv{p_\pt}\cdot p(\Sloop)\cdot p_\pt)$ and reassociate.
Then use $\inv r\inv q\eqto\inv{(qr)}$ and 
$\ap{f}(\ap{p}(\Sloop)) \eqto \ap{f\circ p}(\Sloop)$ to obtain (2).
All constructions involved are defined by path induction,
which means that they preserve reflexivity. E.g., for $g:\base\eqto z$,
$c(g):(\ap{f\circ p}(g)\eqto \ap{f}(\ap{p}(g)))$ is defined
by setting $c(\refl{\base})\defeq \refl{f(p(\base))}$.}
Moreover, the above diagram stays well typed when we replace
...
f(\cst{\pt_A}(g)\cdot \refl{\pt_A})\cdot \refl{\pt_B}$, since
$f_\pt \jdeq \refl{\pt_B}$. Also, $\iota(g)$ has the right type.} 
Now apply induction on $g$, setting $g\jdeq\refl{\base}$,
