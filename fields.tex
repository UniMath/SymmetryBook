...
%\begin{lemma}\label{lem:BG->*B2G-connected} % Could not prove it,
%For every \MB{?} group $G$ the type $\BG\ptdto\BB G$ is connected.
%\end{lemma} % not even for abelian groups
...
by $\B\grpHom(G,G) \defeq \Aut_{\BG\ptdto\BB G}((z\mapsto \pt_{\BB G}),\refl{})$.
\end{definition}
