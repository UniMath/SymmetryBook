...
In \cref{def:O-functor} we will why $\cst{\ast}^A$ is useful.}
\end{definition}
...
by applying $\cst{\ast}^\Sc$ to the unique path $(f_\pt,\refl{f_\pt}')$
...
The above identification stays well typed when we replace
$\Sloop$ by an arbitrary $g:\base\eqto z$, $z:\Sc$. We define
\[
\iota(f,p,g) :
...
a reflexivity path, we get that $O(f)_\pt$ and $\ev_B(O(f)_\pt)$ are
...
as also $\iota(f,\cst{\pt_A},\refl{\base})$ is reflexivity, and we are done.
\end{implementation}
