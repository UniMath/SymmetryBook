...
The task can be simplified by applying path induction on $f_\pt$,
that is, setting $\pt_B\jdeq f(\pt_A)$ (also used as shorthand)
and $f_\pt\jdeq\refl{\pt_B}$. This yields the following diagram:

\begin{tikzcd}[ampersand replacement=\&]
  \& \&f(\cst{\pt_A}(\Sloop)\cdot \rfl)\cdot \refl{\pt_B}  \ar[dddd,eqr,"{i(\cst{\pt_A},\refl{\pt_A})}"]
\\
  \& \refl{\pt_B}
   \ar[ru,eqr,"{\loops(f)((\ev_A)_\pt)}"]
\\
