...
\subsection{Lagrange's theorem, counting version}
...
\begin{definition}
  \label{def:finitegrd}
A \emph{finite group}\index{finite group} is a group such that the set $\pt_G=\pt_G$ is finite.   If $G$ is a finite group, then the \emph{\gporder}\index{\gporder} $|G|$ is the cardinality of the finite set $\pt_G=\pt_G$ (\ie $(\pt_G=\pt_G):\fin_{|G|}$).
...
\end{definition}
\begin{example}
  The trivial group has \gporder $1$, the cyclic group $C_n$ of order $n$ has \gporder $n$ (which is good) and the permutation group $\Sigma_n$ has \gporder $n!$.
\end{example}
