...
but in this case we can reorient that $g$s to point in the same direction as the $f$s,
...
\begin{proof}
  ...
  \begin{align*}
    \varphi^{-1}(p)
    &\equiv
      \sum_{\substack{h_+ : \prod_{n:\NN}P(n+1) \\ h_- : \prod_{n:\NN}P(-n-1)}}
    \bigl(h_+(0) = f_0(p)\bigr)
    \times \bigl(h_0(0) = f_{-1}^{-1}(p)\bigr) \\
    &\qquad\qquad\qquad
      \times \biggl(\prod_{n:\NN}h_+(n+1) = f_n(h_+(n))\biggr) \\
    &\qquad\qquad\qquad
      \times \biggl(\prod_{n:\NN}h_-(n+1) = f_{-n-2}^{-1}(h_-(n))\biggr).
  \end{align*}
  This type is contractible, because it is the product of two contractible types,
...
Fix $P : \TorZ \to \UU$, $p_\pt : P(\pt)$ and $p_\Zloop : p_\pt =^P_\Zloop p_\pt$.
...
\begin{proof}
  ...
  We leave this for the reader, who may consult our 
  formalization as needed.\footnote{%
     A useful transport lemma in this context is:
     if $q:a_1=a_2$, $h_i: \zet\to a=a_i$, $r(n): q_*(h_1(n))=h_2(n)$,
     $hh_i(n): h_i(n+1)=p\ct h_i(n)$, then $(q,r)_*(hh_1(n)): h_2(n+1)=p\ct h_2(n)$
     is equal to the composite of $r(n+1)^{-1}$, $\ap{q_*}(hh_1(n))$, 
     use of $q_*(p)=p$, and $\ap{p\ct\_}(r(n))$.} 
\end{proof}
