...
in \cref{sec:TorZ-recursion}, but $A$ is now not constant
and $p$ is a pathover.
...
The following lemma uses that $\Zloop\jdeq(ua(s),!,!)$.
...
\begin{proof}
Prefixing $s^Z_x$ with $\Zloop$ amounts to
...
technique as for the recursion principle. 
Recall that $A:\TorZ\to\UU$, $a:A(\pt)$ and $p: a=^A_\Zloop a$.
%Fix $P : \TorZ \to \UU$, $p_\pt : P(\pt)$, $p_\Zloop : p_\pt =^A_\Zloop p_\pt$.
\begin{lemma}\label{lem:guided-null-hmtps-dep}
  For every $Z\defeq(X,f,t):\TorZ$, the type
  \[
    \sum_{a' : A(Z)} \sum_{h : \prod_{x:X} a =^A_{s^Z_x} a'} \prod_{x:X} h(f(x)) = g_x(p\cto h(x))
  \]
...
\begin{proof}
  ...
  \[
    R(a') \defeq \sum_{h : \prod_{x:X} a =^A_{s^Z_x} a'} \prod_{x:X} h(f(x)) =  g_x(p \cto h(x)).
  \]
  Thus, to show that $Q_p(\zet,s)$ is contractible,
  it suffices to define an equivalence
  \[
    \varphi_p : R(p) \xrightarrow{\sim}{} (a = p)
  \]
  for each $p : P(\pt)$.
  We again invoke \cref{thm:integers-univ-symm},
  this time with the family $S_p : \zet \to \UU$ given by $S_p(z) \defeq (a =^A_{s^\pt_z} p)$
...
\begin{tikzpicture} %\large
   ...
   [matrix of math nodes, row sep=2em, column sep=2em, ampersand replacement=\&]
    {
\sum\limits_{a' : P(Z)}
\sum\limits_{h : \prod\limits_{x:X} a =^A_{s^Z_x} a'} 
\prod\limits_{x:X} h(f(x)) = g_x(p \cto h(x))\&       \& A(X)\\
\\
   \& \TorZ \& \\};
...
\end{tikzpicture}
\caption{\label{fig:TorZ-induction}Factoring through a contractible type}
\end{figure}
