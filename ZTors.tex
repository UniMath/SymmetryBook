...
One can prove $\pi_1(c'_p(X,f,e)) = a$ for all $e$,
and $\Trunc{\pi_1(c_p(X,f,t)) = a}$ for all $t$.
One can in general \emph{not} prove $\pi_1(c_p(X,f,t)) = a$ for all $t$,
...
We proceed as follows. From $\varepsilon(n): s^\pt_n = \Zloop^n$ we get
$\ap{\trp_{P,\varepsilon(n)}}: (s^\pt_n)_* = (\Zloop^n)_*$.
Applying to $\pt$ gives an inhabitant of $(s^\pt_n)_*(\pt) = (\Zloop^n)_*(\pt)$
that we denote by $\varepsilon_*(n)$. Thus we get for all $n:\zet$:

\[
\varepsilon_*(n) \ct p_\Zloop^n : \pt =^P_{s^\pt_n} \pt
\] 
