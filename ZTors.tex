...
We use this principle in \cref{lem:dep-elim-TorZ}, and the last paragraph
of its proof gives this some extra attention to the computational aspect.
...
Applying UA gives thus the first result.
...
so $e_x\circ s = e_{x+1}$. Applying UA we get:
...
We only have to check that the general statement of the lemma is well-typed.
...
In total, we have proved the induction principle of the circle for $\TorZ$:

\begin{definition}\label{def:TorZelim}
The function ${\ind}_A$ mapping $(a,p)$ to $c_p$ as defined in 
\cref{lem:guided-null-hmtps-dep} has type
\[
{\ind}_A : \sum_{a:A(\pt)}(a=_\Zloop a) ~~\to~~ \prod_{Z:\TorZ} A(Z)),
\]
and satisfies ${\ind}_A(a,p)(\pt) \jdeq a$ and 
$\apd{{\ind}_A(a,p)}(\Zloop) = p$.
\end{definition}
