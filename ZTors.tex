...
\begin{lemma}\label{lem:TBN}
The following equation is well-typed and true:
\[
\Zloop_*\, (a,n\mapsto p^n) = (a,n\mapsto p^n).
\]
\end{lemma}
\begin{proof}
We have $\apd{\hat{p}}\Zloop : \Zloop_*\,\hat{p}(\pt) = \hat{p}(\pt)$,
and $\hat{p}(\pt) \jdeq (a,n\mapsto p^n)$.
\end{proof}

Another aspect of the universal property that we are after is
