...
The following lemma uses that $\Zloop\defeq(ua(s),!,!)$.
...
\begin{proof}
  ...
  We have $Q_p(\pt)\jdeq \sum_{a':A(\pt)} R(a')$ for $R : P(\pt) \to \UU$ defined by
  \[
    R_p(a') \defeq \sum_{h : \prod_{x:X} a =^A_{s^\pt_x} a'}~\prod_{x:X} h(f(x)) =  g_x(p \cto h(x)).
  \]
  ...
  this time with the family $S_{a'} : \zet \to \UU$ given by $S_{a'}(z) \defeq (a =^A_{s^\pt_z} {a'})$
  and the equivalences $g'_{a'} : \prod_{z:\zet} S_{a'}(z)\equiv S_{a'}(z+1)$ given by
  $g'_{a'}(z) \defeq g_z(p \cto ({-}))$.
\end{proof}
