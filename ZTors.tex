...
\section{Abstract}
We define a type $\TorZ$ of $\ZZ$-torsors and show that it has the dependent
universal property of the circle. We use Voevodsky's Univalence Axiom
as well as propositional truncation. We do not use the circle itself.

\section{Introduction}
As a prerequisite we require working knowledge of homotopy type theory
as described in, for example, the first four chapter of \cite{hottbook}.
One crucial extra piece of knowledge is the \emph{dependent} elimination
principle for propositional truncation, not mentioned in \cite[3.7]{hottbook},
and qualified as `not really useful' in \cite[6.9]{hottbook}.
\footnote{The dependent elimination principle appears to 
be used later in \cite{hottbook}, though. ULRIK HAD SPOTTED A PLACE}
In \cref{sec:XXX} we give this dependent elimination property some extra
attention, as it is crucial for the computational properties of our proof.

One simple extension of the abovementioned prerequisites is the inductive
definition of the set of integers. We give that definition below
in \cref{sec:integers}. For now it suffices to have a set
of integers $\zet$, a constant $0:\zet$,
and a successor function $s:\zet\to\zet$,
which actually is an equivalence. We can then define \emph{the 
pointed type of $\zet$-torsors} by
\[
\TorZ\defeq \sum_{(X,f)}\Trunc{(\zet,s)=(X,f)} \text{~with~}
\pt\defeq ((\zet,s),\trunc{\refl{(\zet,s)}}).
\]
Here and below, pairs $(X,f)$ have type $\sum_{X:\UU}(X\to X)$
without explicitly mentioning this. It is not difficult to see 
that $\pt =_\TorZ \pt$ is equivalent to $\zet$. First, observe that
every bijection $f:\zet\to\zet$ commuting with $s$
is extensionally equal to $s^{f(0)}$. Then, the first projection
acting on any $p : \pt =_\TorZ \pt$ gives rise to such a bijection,
which we can evaluate in $0$. Let's denote the composite as
$\ev_0 : (\pt =_\TorZ \pt) \to \zet$. Combining these observations,
using UA, one easily proves that $\ev_0$ is an equivalence.

The preimage of $s(0)$ under the equivalence $\ev_0$ is a
natural generator path of $\pt =_\TorZ \pt$, so we define
...
In this context we can formulate a recursion principle for $\TorZ$.
It states that, given a type $A$, an element $a$ of $A$ and a
path $l:a=_A a$, one can construct a function $f:\TorZ\to A$
such that $f(\pt)\jdeq a$ and $f(\Zloop)=l$.
This construction has been formalised by Grayson 
in \cite{circlerec-Dan}. We call the resulting function
\[
...
This induction principle has resisted some attempts at proving it. 
On the basis of Grayson's proof of the recursion principle,
Shulman sketched a possible proof in \cite{circleind-Mike}. 
Independently of this, but also departing from Grayson's proof of the 
recursion principle, Buchholtz and Bezem found the proof that
we present in this paper. This proof has subsequently been formalized
by Grayson \cite{circleind-Dan}. The precise result that has been
proved is
...
\sum_{r: f(\pt)= a}~
{f(\Zloop) = r\cdot l \cdot r^{-1}}.
\]

This results implies that $\TorZ$ has the same universal property
as the circle, for example, the higher inductive type $\Sc$ 
in \cite[Ch. 6.1]{hottbook}, and is thus equivalent to $\Sc$.
If the underlying type theory has propositional truncation with an 
eliminator that computes judgmentally on the point constructor, 
then the same is true for our `circle', i.e., the '$r$' above is a reflexivity path.

It is important to stress that our proof does not make use
of $\Sc$, such that $\TorZ$ can serve as a circle in Unimath.
In the presence of $\Sc$ it is easy to prove $\TorZ\equiv\Sc$.
One can simply observe that both $\TorZ$ and $\Sc$ are pointed connected types
with the loop space both equivalent to $\zet$. From this $\TorZ\equiv\Sc$
follows easily. COULD REFER TO SYMMETRY BOOK COR. 2.17.6 (3).
