...
The proof uses Voevodsky's Univalence Axiom and propositional truncation, 
...
such that $\mathsf{pr}_1(p)_* = s$.
\end{definition}

%Note that $\Trunc{(\zet,s)=(X,f)}$ has propositional consequences
%such as $\isset(X)$ and $\isEq(f)$. PERHAPS INFORMATIVE, BUT NOT NEEDED HERE
