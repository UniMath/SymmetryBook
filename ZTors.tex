...
\begin{abstract}
We show that the type $\TorZ$ of $\ZZ$-torsors has the dependent universal property of the circle, which characterizes it up to a unique
homotopy equivalence.  The proof uses Voevodsky's Univalence Axiom and propositional truncation, yielding a stand-alone construction of the
...
that $\pt =_\TorZ \pt$ is equivalent to $\zet$.  To see that, begin by observing that
every function $f:\zet\to\zet$ commuting with $s$
is propositionally equal to $s^{f(0)}$.  The first projection
acting on any $p : \pt =_\TorZ \pt$ gives rise to such a bijection,
which we can evaluate at $0$.  We let $\ev_0$ denote the composite function of
type $(\pt =_\TorZ \pt) \to \zet$.  Combining these observations,
...
The precise result that has been
proved and formalized is
...
that computes judgmentally on the point constructor, then
the same is true for our circle, i.e., the path $r : f(\pt)= a$ above is a reflexivity path.  
The \emph{UniMath} project lacks dependent elimination for
propositional truncation, so the formalized proof in \cite{circleind-Dan} is 
somewhat different from the proof presented below.

Our construction does not use higher inductive types, nor does it depend on the previous existence of a type satisfying the induction principle
of the circle.  Indeed, the convention in {\em UniMath} is to avoid higher inductive types.  However, in a type theory where they are admitted,
one can introduce a circle $\Sc$ as a higher inductive type, as in \cite[Ch. 6.1]{hottbook}, and it will be equivalent to $\TorZ$.  The
equivalence can be established by observing that $\TorZ$ and $\Sc$ are pointed connected types with loop spaces equivalent to $\zet$.  (TODO: REFER TO SYMMETRY BOOK COR. 2.17.6 (3).)
...
From now on we will denote $i(n):\zet$ by $n$
and $-i(n)$ by $-n$, for all $n:\NN$.
...
Given $P : \zet \to \UU$, to construct an elements $f(z) : P(z)$ for every $z : \zet$,
it suffices to give $f(0): P(0)$ and two functions
$g : \prod_{n:\NN}(P(n) \to P(n+1))$ and
$h : \prod_{n:\NN}(P(-n) \to P(-n-1))$.
The function $f$ thus defined satisfies
$f(n+1)\jdeq g(n,f(n))$ and $f(-n-1)\jdeq h(n,f(-n))$ for all $n:\NN$.
...
and therefore is pointed out early.
