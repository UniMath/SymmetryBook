...
explicit.
\begin{marginfigure}\label{fig:fibersofcomposites}
$$
\xymatrix{
      F_1^{-1}(x_1,p_2)\ar[r]^H_\simeq\ar[d]_{\fst}&f_1^{-1}(x_1)\ar[d]^{\fst}\ar[dl]_{F_2}&\\
      (f_2f_1)^{-1}(x_2)\ar[r]^{\fst}\ar[d]^{F_1}&X_0\ar[r]^{f_2f_1}\ar[d]^{f_1}&X_2\ar[d]^{\id}\\
...
From the universal property of the pullback (\cref{def:pullback}),\footnote{The fiber $(f_2f_2)^{-1}$ is the pullback of
$X_0\xrightarrow{f_2f_1} X_2 \xleftarrow{x_2}\bn1$.} it furthermore follows that $F_2$ is the unique map such that $(\fst F_2)\eqto _{f_1^{-1}(x_1)\to X_0}\fst$ and $H^{-1}$ is similarly unique with respect to $(\fst H^{-1})\eqto F_2$.
\begin{figure}
\[
...
The following corollary analyzes how kernel and cokernel work on
the composition of two homomorphisms.

\begin{corollary}
  \label{cor:cokermaps}
  \marginnote{$$\xymatrix{\Ker(f_1) \ar[r]^{\id_{\Ker(f_1)}}\ar[d]^{F_2}&\Ker(f_1)  \ar[d]^{\kermap{f_1}}\\
      \Ker(f_2 f_1)\ar[d]^{F_1}\ar[r]^-{\kermap{f_2f_1}}&G_0\ar[d]^{f_1}\ar[r]^{f_2f_1}&G_2\ar[d]^{\id_{G_2}}\\
      \Ker(f_2)  \ar[r]^-{\kermap{f_2}}&G_1\ar[r]^{f_2}&G_2% \\
   % \coker(f_2) &\coker(f_1) 
 }
  $$}
  ...
  Furthermore,\footnote{\wip{Was: $F_2\eq_{\typemono(G_1)}\kermap{F_1}$}}
	$$(\Ker(f_1),F_2)\eq_{\typemono(\Ker(f_2f_1))}(\BKer(F_1),\kermap{F_1})$$ 
and the restriction of the cokernel of $f_1$ to the kernel map of $f_2$ is 
the cokernel of $F_1$:
	$$\coker(f_1)\B\!\kermap{f_2}\eq_{\BKer(f_2)\to\Set}\coker (F_1).$$
