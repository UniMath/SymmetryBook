...
\section{Epimorphisms, and a new take on monomorphims}
\label{sec:epis}
...
map $f:B\to C$ with $B$ and $C$ types.}  In particular, if $g,h:C\to D$ are two functions into a set $D$ the proposition $\prod_{c:C}(g(c)=h(c))$ is equivalent to $\prod_{b:B}(gf(b)=hf(b))$.\footnote{\wip{For the converse:
take $\Prop$ for $D$ and define $g(c)\defeq\Trunc{f^{-1}(c)}$
and $h(c)\defeq\true$ for all $c:C$, then $gf=hf$, whereas $g=h$
expresses surjectivity of $f$. Add? Cf. 6.3.6-8. Caveat in 6.3.6,
where injection is not in $\Set$.}}
...
\item\label{it:surjepi} The function $f$ is an surjection if and only if for any set $D$ and functions $g,h:C\to D$, if
...
This observation about sets translates fruitfully to other contexts and in particular to groups.  To make it clear that we talk about group homomorphisms (and not about the underlying unpointed functions of connected groupoids) we resort to categorical terminology as introduced in \cref{ch:cats}.\footnote{%
In fact we defined monomorphisms already in \cref{def:typeofmono}.
The definition we give here is soon proved to be equivalent and therefore
we don't bother to give it a new name.} 
...
\glossary(EpiG){$\protect{\typeepi_G}$}{type of epimorphisms from the group $G$} is\footnote{\wip{Discuss overlap with 5.3. 
Good example of epi: For groups $G_1$ and $G_2$, then the first projection from $G_1\times G_2$ is an epimorphism.}}
  \[
  \typeepi_G\defequi\sum_{H:\typegroup}\sum_{f:\Hom(G,H)}\isepi(f).\qedhere
  \]
\end{definition}
...
The following propositions are equivalent:\label{lem:eq-epi-conn}\footnote{%
\label{ft:eq-mono-cover}Consequently, the notions of monomorphisms in
\cref{def:typeofmono} and \cref{def:monomorphism} are equivalent.} 
\begin{enumerate}
...
$$\xymatrix{\Hom(F,G)\ar[d]_{g\mapsto(g\circ\blank)}\ar[r]^{f\circ\blank}
           ...
           &(\Hom(\ZZ,F)\to\Hom(\ZZ,H)),}
$$
where the vertical maps are injections by \cref{fig:Znatural}
using that $\USym$ acts injectively.
...
\begin{construction}\label{con:monos-are-equalizers}
  Given groups $G$ and $H$ and mmonomorphism $i : H \to G$,
