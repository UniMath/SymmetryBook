...
instantiate to particular morphisms in the category of groups,\footnote{%
As opposed to, \eg their underlying unpointed functions of connected 
groupoids in their own (wild) category.} 
\ie to group homomorphisms having a particular property.

In fact we defined monomorphisms of groups already in \cref{def:typeofmono}.
...
\index{type! of monomorphisms into a group}%
  ...
  is
  \[
  \typemono(H)\defequi\sum_{G:\typegroup}\sum_{i:\Hom(G,H)}\ismono(i).
  \] 


The \emph{type of epimorphisms from $G$}
\index{type! of epimorphisms from a groups}%
\glossary(EpiG){$\protect{\typeepi(G)}$}{type of epimorphisms from the group $G$}%
 is\footnote{\wip{Discuss overlap with 5.3. 
Good example of epi: For groups $G_1$ and $G_2$, then the first projection from $G_1\times G_2$ is an epimorphism.}}
  \[
  \typeepi(G)\defequi\sum_{H:\typegroup}\sum_{f:\Hom(G,H)}\isepi(f).\qedhere
  \]
\end{definition}
...
\USymG\ar[r]^{\US f}&\USymH
}$$
\caption{\label{fig:Znatural}Naturality of $\protect\ev$.}
\end{marginfigure}
We've seen in \cref{ex:Zinitial} that for any group $G$, 
the underlying set $\USymG\defequi(\sh_G\eqto\sh_G)$ of $\abstr(G)$ is equivalent to the set of homomorphisms $\Hom(\ZZ,G)$. Moreover,
...
$\US f$ is an injection is equivalent to saying that 
...
\wip{at the shape of $G'$},
whereas \wip{(the underlying set of)} the cokernel is the set of components 
of that fiber.  We spell out the details.
...
\begin{definition}
  ...
  \[
    \sh_{\Ker(f)}\defequi(\sh_G,f_\pt):(\Bf)^{-1}(\sh_{G'}),
  \]
  where $f_\pt:\sh_{G'}\eqto{}\Bf(\sh_G)$ is the pointing path of $\Bf$.%
\footnote{That is:
    \[
      \Ker(f)\defeq \Aut_{(\Bf)^{-1}(\sh_{G'})}(\sh_G,f_\pt)
    \]}
...
\wip{Use $\princ{G'}\circ\Bf$, relation to \ref{sec:fixpts-orbits}?}}
\[
  \sum_{z:\BG}\sum_{p:\sh_{G'}\eqto f(z)}\Trunc{(\sh_G,f_\pt)\eqto(z,p)}
\]
pointed at $(\sh_G,f_\pt,!)$, 
...
a $g:\USymG'$ such that $g\cdot\settrunc{(\sh_G,\Bf_\pt)}=u$.
...
for $z$, and then $g\jdeq \MB{p^{-1} \Bf_\pt}$ will do.\footnote{%
...
$$\xymatrix{
  \sh_{G}\ar[dd]_\id&\sh_{G'}\ar[dr]^{\Bf_\pt}\ar[dd]_g\\
                                                       &&\Bf(\sh_{G})\\
...
\marginnote{The subgroup of $G'$ associated with the cokernel is the ``image'' of the next section.}
  ...
  The obvious point to choose is $\settrunc{(\sh_G,f_\pt)}$. 
...
to say that, since $BG$ is connected and equivalent to 
$\sum_{w:\B G'}\Bf^{-1}(w)$, which maps surjectively onto 
$\sum_{w:\BG'}\Trunc{\Bf^{-1}(w)}_0$, the latter type is connected. 
Moreove, when pointed at $(\sh_{G'},\settrunc{(\sh_G,f_\pt)})$, 
just another name for $E(\coker(f)):\typemono(G')$.
\end{remark}
...
\begin{xca}
  ...
  \item if $h:\Hom(L,G)$ is a homomorphism such that $fh:\Hom(L,G')$ is the
    ...
    $fh$ factors through the trivial group $1$),
...
\begin{lemma}
  ...
  Consider pointed functions $(f_1,p_1):(X_0,x_0)\ptdto (X_1,x_1)$ and $(f_2,p_2):(X_1,x_1)\ptdto (X_2,x_2)$ and the resulting functions
...
\begin{xca}
  ...
  $$\sum_{p:\sh_{G'}\eqto{}\Bf(x)}\Trunc{\sum_{\beta:\sh_G\eqto{}x}p\eqto{}\USymf(\beta)f_\pt}.$$
...
In particular, we choose the base point $(\sh_{G'},\trunc{(\sh_G,f_\pt)}_0)$, so that the \emph{image group} is  %type $\sum_{b:B}\Trunc{f^{-1}(b)}$ is simply replaces by
$$\Img f\defequi\Aut_{\sum_{z:\BG'}\Trunc{(\Bf)^{-1}(z)}_0}((\sh_{G'},\trunc{(\sh_G,f_\pt)}_0)).$$
...
\begin{implementation}{con:im}
  ...
  $b:f_\pt\eqto{}a_{x_0}^{-1}j(p_q)p_j$, 
  where $a:\prod_{x:X}f(x)\eqto{}j(q(x))$ witnesses that we have a factorization, 
  then we point $\sum_{z:Z}\Trunc{f^{-1}(z)}_n$ in $(z_0,\trunc{(x_0,f_\pt)}_n)$ 
  ...
  $$b:\xymatrix{
    ...
    z_0\ar@{=}[r]^{f_\pt}_\to&f(x_0)\ar@{=}[r]_\to^{a_{x_0}}&j(q(x_0)).}$$
\end{implementation}
...
\end{align*}
    ...
    $$\Img f\defequi \Aut_{\sum_{z:\BG'}\coker(f)(z)}(\sh_{G'},\trunc{(\sh_G,f_\pt)}_0),$$
...
the epimorphism
 ...
 \begin{align*}
    \img:\Hom(G,G')\to\typemono(G'),\qquad&\img(f)\defequi(\Img f,\incl_{\img f},!)\\
 \prjim:\Hom(G,G')\to\typeepi(G), \qquad&\prjim f\defequi(\Img f,\prj_{\img f},!)
  \end{align*}
...
%   $$\sh_{\Img f}\defequi (\sh_{G'},|\sh_G,f_\pt|):\sum_{z:\BG'}\coker(f)(z)$$ and define the image group of $f$ to be
...
% %   If $f:\Hom(G,G')$  is a homomorphism we must specify the ingredients in $\img f\defequi(\Img f,\incl_{\img f},!):\typemono(G)$.
% %   The \emph{image group}\index{image!group} (or most often just the image) of $f$ is the group
% %   $$\Img f\defequi \aut_{\sum_{z:\BG'}\coker(f)(z)}(\sh_{G'},|\sh_G,f_\pt|).$$
% %   The first projection $\fst: B\image f\to \BG'$ is a \covering
...
\begin{definition}
  ...
  \typemono(G)(y)\defequi \sum_{H:\typegroup}\sum_{f:\Hom(H,G)(y)}\isset(\Bf^{-1}(\sh_G))$$
  for $y:\BG$,
where  -- as in \cref{ex:HomHGasGset} --
$$\Hom(H,G)(y)\defequi\sum_{F:\BH_\div\to \BG_\div}(y\eqto{}F(\sh_H))$$
...
$$
E(y):\typemono(G)(y)\to\typesubgroup_G(y),\qquad E(H,F,f_\pt,!)\defequi(F^{-1}, (\sh_H,f_\pt),!)
$$
for $y:\BG$ (where $H$ is a group, $F:\BH_\div\to \BG_\div$ is a map and $f_\pt:y\eqto{}F(\sh_H)$ an identity in $\BG$; and $F^{-1}:\BG\to\Set$ is $G$-set given by the preimages of $F$ and $(\sh_H,f_\pt):F^{-1}(y)\defequi \sum_{x:\BH}y\eqto{}F(x)$ is the base point).  If $y$ is $\sh_G$ we follow our earlier convention of dropping it from the notation.
\end{definition}
...
%   are called \emph{kernels} with inclusion $i:\typekernel_G\to \typemono(G)$ again given by $i(M)\defequi M(\sh_G)$.  We let $E^G:\typekernel_G\to\typenormal_G$ denote the equivalence given by the equivalence of $G$-sets $E:\typemono(G)\to\typesubgroup_G$.
% \begin{remark}
...
% $$E(y):\typesubgroup_G(y)\to\typesubgroup_G'(y),\qquad E(H,F,f_\pt,!)\eqto{}(F^{-1}, (\sh_H,f_\pt),!)$$
% (where $H$ is a group, $F:\BH_\div\to \BG_\div$ is a map and $f_\pt:y\eqto{}F(\sh_H)$ an identity in $\BG$; and $F^{-1}:\BG\to\Set$ is $G$-set given by the preimages of $F$ and $(\sh_H,f_\pt):F^{-1}(y)\defequi \sum_{x:\BH}y\eqto{}F(x)$ is the base point).  If $y$ is $\sh_G$ we follow our earlier convention of dropping it from the notation.
...
% $$\typeepi(G)\defequi\sum_{G':\typegroup}\sum_{f:\Hom(G,G')}\mathrm{issurj}(f).$$
% \end{definition}
...
\end{lemma}
\begin{proof}
  Following $(G',f,!):\typeepi(G)$ around the top to $\typesubgroup_G$ yields the transitive $G$-set sending $y:\BG$ to the set $\sh_{G'}\eqto{}f(y)$ together with the point $f_\pt:\sh_{G'}\eqto{}f(\sh_G)$ while around the bottom we get the transitive $G$-set sending $y:\BG$ to the set $f(\sh_G)\eqto{}f(y)$ together with the point $\refl{f(\sh_G)}:f(\sh_G)\eqto{}f(\sh_G)$.  Hence, precomposition by $f_\pt$ gives the identity proving that the diagram commutes.
\end{proof}
...
$$q:\typenormal_G\to\typeepi(G),\qquad q(N)\defequi(G/N,q_N,!).$$
\end{definition}
...
$$
We note that in the case $\princ G(y)\defequi (\sh_G\eqto{}y)$ we get
that
$
\princ G /N(y)\defequi\sum_{z:\BG}(\sh_G\eqto{}z)\times X_z(y)
$
...
% Observe that by using the identification of the basepoint $Q^f_{f(\sh_G)}$ of $\BG/\nor f$ with $Q^f_{\sh_{G'}}$ given by $f_\pt:\sh_{G'}\eqto{}f(\sh_G)$ we have defined a homomorphism $Q^f:\Hom(G',G/\nor f$) such that
...
Then
$$\Ker q(N)\defequi\Aut_{\sum_{x:\BG}(X_x\eqto{}X_{\sh_G})}(\sh_G,\refl{X_{\sh_G}})
$$
...
% $$(B\ker q_N)_\div\defequi\sum_{z:\BG}(X_z\eqto{}X_{\sh_G})$$
% pointed at $(\sh_G,\refl{X_{\sh_G}})$ and with $i_{\ker(f)}:B\ker q_N\ptdto \BG$ given by the first projection.

However, going the other way around the pentagon of \cref{lem:diagfornormal}, we see that $\mathrm{ass}(N)\defequi E^{-1}i(N):\typemono(G)$ consists of the group
...
%   \label{lem:normalsarekernels}
% The map
% $$\mathrm{ev}:B\ker q_N\to B\mathrm{ass}(N),\qquad \mathrm{ev}(z,f)\defequi(z,f(\pt_z))$$ is an equivalence and furthermore commutes with the first projections to $\BG$.  The pointed map $\mathrm{ev}_*\defequi(\mathrm{ev},\refl{(\sh_G,\pt_{\sh_G})}):B\ker q_N\ptdto  B\mathrm{ass}(N)$
...
% $\preinv(f_\pt):(\sh_{G'}=f(z))=(f(\sh_G)=f(z))$ and
% $$\preinv:(f(\sh_G)=f(z))=
...
% $$\pt_{H\times_GH'}\defequi(\sh_H,\pt_{H'},p_{f'}f_\pt^{-1})$$ of the pullback $\BH\times_{\BG}\BH'$ (where $f_\pt:\sh_G=f(\sh_H)$ is the name we chose for the data displaying $f$ as a pointed map, so that $p_{f'}f_\pt^{-1}:f(\sh_H)=f'(\pt_{H'})$).
...
% %   If $H,K:\typemono(G)$ with $X,Y:\BG\to\Set$ being the corresponding transitive $G$-sets under the equivalence $E$, then the intersection of $H$ and $K$ corresponds to the $G$-set $X\times Y:\BG\to\Set$ (with $(X\times Y)(x)\defequi X(x)\times Y(x)$).
% % \end{example}
...
$$(\Bf)^{-1}(\sh_{G'})\defequi\sum_{y:\BG}(\sh_{G'}\eqto{}\Bf(y)).$$
...
$$B(fi)^{-1}(\sh_{G'})\defequi\sum_{x:\BH}\sh_{G'}\eqto{}B(fi)x,$$
the preimage of $\sh_{G'}$ of the composite $B(fi):\BH\to \BG'$.
 By definition, the intersection $B(N\cap H)$ is the pointed component of the pullback containing $(\pt_N,\sh_H)$.  Under the equivalence with $B(fi)^{-1}(\sh_{G'})$ the intersection corresponds to the component of $(\sh_H,\Bf(p_i)\,f_\pt)% :\sum_{x:\BH}\sh_{G'}\eqto{}B(fi)x
 $.
Since (by definition of the composite of pointed maps) $p_{fi}\defequi \Bf(p_i)\,f_\pt$ we get that the intersection $N\cap H$ is identified with the kernel of the composite $fi:\Hom(H,G')$.
%    \item
...
This can be somewhat confusing when it comes to fixed points: the fixed points of $F^*X$ are given by $\prod_{v:\BH}XF(v)$ which evaluates nicely to $XF(\sh_H)$, but in order to considered  these as elements in $X(\sh_G)$ we need to apply $X(f_\pt^{-1}):X(F(\sh_H))\eqto{}X(\sh_G)$.

Consequently, we'll say that $x:X(\sh_G)$ is an \emph{$H$-fixed point} if there is an $f:\prod_{v:\BH}XF(v)$ such that $x\eqto{}X(f_\pt^{-1})f(\sh_H)$.
...
Then $g\,x$ is a fixed point for the $H$-action on $X$ if and only if $x$ is a fixed point for the action  of the conjugate subgroup $g\,H\defequi(H,F,g^{-1}f_\pt,!)$ on $X$.
\end{lemma}
\begin{proof}
  Consider an $f:\prod_{v:\BH}XF(v)$.  Then $g\cdot x\eqto{}X(f_\pt^{-1})(f(\sh_H))$ if and only if $x\eqto{}g^{-1}\cdot X(f_\pt^{-1})(f(\sh_H))\oldequiv X((g^{-1}f_\pt)^{-1}(f(\sh_H))$.
\end{proof}
...
\begin{displaymath}
  \Ker(\inn) \defequi \Aut_{\inv{(\Binn)} (G)}(\sh_G, \refl{\BG})
...
$$ together with the first projection from $\sum_{y:\BG}X(y)$ to $\BG$.  Conversely, if $(H,i_H,!):\typemono(G)$, then the corresponding transitive $G$-set is $G/H\defequi\coker i_H$ pointed at $|\sh_H,p_{i_H}|:\coker i_H(\sh_G)\defequi\Trunc{\sum_{x:\BH}\sh_G\eqto{}\Bi_H(x)}_0$.
...
\begin{definition}
  % Let $G$ be a group and consider the subgroup represented by the transitive $G$-set $X:\BG\to\Set$ together with the point $\pt_X:X(\sh_G)$ % (so that $\BH_\div\defequi\sum_{y:\BG}X(y)$ pointed at $\sh_H\defequi (\sh_G,\pt_X)$ together with the first projection $\Bi_H:\BH\ptdto \BG$ defines an element in $(H,i_H,!):\typesubgroup_G$)
  % .
...
% $$\Bj_H:\sum_{y:\BG}X(y)\ptdto \sum_{(y,Y):\BG\times \BW_GH}Y(y),\qquad \Bi_H(y,v)\defequi ((y,X),v).$$
% \end{definition}
