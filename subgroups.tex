...
\marginnote{For those familiar with the traditional notion, the following summary may guide the intuition.}
...
$\Bfpt:f^*\princ{G'}(\sh_G)$.}}
\[
  \sum_{z:\BG}\sum_{p:\sh_{G'}\eqto f(z)}\Trunc{(\sh_G,\Bfpt)\eqto(z,p)}
\]
...
$\sum_{w:\B G'}\Bf^{-1}(w)$, which maps surjectively onto 
...
From the universal property of the preimage (\cref{lem:typefamiliesandfibrations}) it furthermore follows that $F_2$ is the unique map such that $(\fst F_2)\eqto _{f_1^{-1}(x_1)\to X_0}\fst$ and $H^{-1}$ is similarly unique with respect to $(\fst H^{-1})\eqto F_2$.
\begin{figure}
\[
...
the cokernel of $F_1$:
  ...
  \end{enumerate}
	Likewise, the set truncations $F'_1$ and $F'_2$ (see \cref{def:settrunc-functor}) of the maps $F_2$ and $F_1$ constructed in \cref{lem:fibersofcomposites} give maps of $G_1$- and $G_2$-sets,\footnote{%
...
respectively:
$$F_2':\Hom_{G_1}(\coker(f_1),\coker(f_2f_1)\,\Bf_2),\quad F_1':\Hom_{G_2}(\coker(f_2f_1),\coker(f_2)) $$
  such that
  \begin{enumerate}
  \item if $f_2$ is an epimorphism then $F_2'$ is an equivalence, and
  \item if $f_1$ is an epimorphism then $F_1'$  is an equivalence.
  \end{enumerate}
\end{corollary}
\begin{xca}
  Let $f:\Hom(G,G')$.  Give an equivalence from the subgroup $E(\ker(f)):\Sub(G)$ associated with the kernel to the $G$-set sending $x:\BG$ to
  $$\sum_{p:\sh_{G'}\eqto \Bf(x)}\Trunc{\sum_{g:\sh_G\eqto x}p\eqto \USymf(g)\Bfpt}.$$
  If $f$ is an epimorphism this is furthermore equivalent to
  $$x\mapsto(\sh_{G'}\eqto \Bf(x)).$$
\end{xca}
...
\marginnote{The formula for the image in group theory is the same as the one for sets, except that the propositional truncation we have for the set factorization is replaced by the set truncation present in our formulation of the cokernel $\coker(f)\defeq\Trunc{(\Bf)^{-1}(z)}_0$.}
This factorization
...
In particular, we choose the base point $(\sh_{G'},\trunc{(\sh_G,\Bfpt)}_0)$, so that the \emph{image group} is  %type $\sum_{b:B}\Trunc{f^{-1}(b)}$ is simply replaces by
$$\Img f\defeq\Aut_{\sum_{z:\BG'}\Trunc{(\Bf)^{-1}(z)}_0}((\sh_{G'},\trunc{(\sh_G,\Bfpt)}_0)).$$
