...
We also noted that the resulting pointed map from $S^1$ to $\B C_m$ was intimately tied up with the degree $m$ map $\dg{m}:S^1\ptdto S^1$ from \cref{def:mfoldS1cover}, pointed by reflexivity. 
...
\begin{corollary}
  ...
  Furthermore, 
	$$F_2\eq_{\typemono(G_1)}\kermap{F_1}$$ 
...
\begin{lemma}
  ...
  Under the equivalence of \cref{lem:actionsconcreteandabstract} between $G$-sets and $\abstr(G)$-sets, the $G$-set $\typemono(G)$ corresponds to the $\abstr(G)$-set
