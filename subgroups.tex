...
\begin{definition}
  ...
  $\ker(f)\defeq(\Ker(f),\incl_{\ker(f)},!):\typemono(G)$.
...
\footnote{That is:
  ...
  together defining $\ker(f)\defeq(\Ker(f),\incl_{\ker(f)},!):\typemono(G)$.
\end{definition}
...
\begin{xca}
  ...
  \item if $h:\Hom(L,G)$ is a homomorphism such that $fh:\Hom(L,G')$ is the
    ...
    $h\eqto \incl_{\ker(f)}k$.\footnote{Hint: consider the corresponding property of the preimage of $\Bf$.
...
\xymatrix{
      ...
      (f_2f_1)^{-1}(x_2)\ar[r]^{\fst}\ar[d]^{F_2}&X_0\ar[r]^{f_2f_1}\ar[d]^{f_1}&X_2\ar[d]^{\id}\\
...
\begin{lemma}
  ...
  $$F_1:f_1^{-1}(x_1)\to(f_2f_1)^{-1}(x_2),\qquad F_1(x,p)\defeq(x,f_2(p)p_2),$$
  $$F_2:(f_2f_1)^{-1}(x_2)\to f_2^{-1}(x_2),\qquad F_2(x,q)\defeq(f_1x,q)$$
  $$H:F_2^{-1}(x_1,p_2)%\defeq\sum_{(x,q)\in (f_2f_1)^{-1}(x_2)}((x_1,p_2)\eqto (f_1x,q))\to \\
...
$F_1H \eqto\fst$, can be filled using function extensionality as follows:
if $(x,q,\pathpair p r):F_2^{-1}(x_1,p_2)$, then $\fst(x,q,\pathpair p r)\jdeq (x,q)$, whereas $F_1H(x,q,\pathpair p r)\jdeq(x,f_2(p)p_2)$, and the latter
two pairs can be identified using $r:f_2(p)p_2\eqto q$.\footnote{%
\wip{Hmm, this is a construction within the statement of a lemma.}} 
\end{enumerate}
\end{lemma}
\begin{proof}
  That $H$ is an equivalence is seen by noting that $F_2^{-1}(x_1,p_2)$ 
  ...
  codomain $f_1^{-1}(x_1)\jdeq\sum_{x:X_0}(x_1\eqto f_1(x))$.\footnote{%
\wip{Note that $f_1^{-1}(x_1)$ only depends on $p_2$ and is otherwise
independent of $f_2$.}} 
\end{proof}
...
Hence, through univalence, $H$ provides an identification
  $$\etop H:(F_2^{-1}(x_1,p_2),\fst)\eqto (f_1^{-1}(x_1),F_1)$$ in the type $\sum_{X:\UU}(X\to(f_2f_1)^{-1}(x_2))$.
...
From the universal property of the preimage (\cref{lem:typefamiliesandfibrations}) it furthermore follows that $F_1$ is the unique map such that $(\fst F_1)\eqto _{f_1^{-1}(x_1)\to X_0}\fst$ and $H^{-1}$ is similarly unique with respect to $(\fst H^{-1})\eqto F_1$.
\begin{figure}
\[
...
\&\&\&\&
\\
\ar[rrrrd]\ar[<->,casred,bend right=30,"{f_1^{-1}(x_1)}"']{dd}\&\&\&\&
\ar[rrrrd]\ar[d,no head]\ar[<->,bend left=30,"{f_2^{-1}(x_2)}"]{dd}\&\&\&\&
\\
x_0\&\&\&\&
x_1\&\&\&\&
x_2\\
...
\\
\ar[rrrru]
\ar[uu,no head]\ar[<->,casblue,bend left=60,"{(f_2f_1)^{-1}(x_2)}"]{uuuu}
\&\&\&\&
\&\&\&\&
\\
\ar[u,dotted,no head]\&\&\&\&
...
$F_1(x,!)=(x,!)$, $F_2(x,!)=(f_1(x),!)$, and $H$ is an equivalence
...
\end{tikzcd}
\]
where $\Bsgn: \BSG_4\ptdto\BSG_2$ is
the classifying map of the sign homomorphim from \cref{def:sgn}.
Further, the \covering $R_4$ is from \cref{def:RmtoS1},\footnote{%
\wip{Confusing terminology: no mono.}} 
restricted to $\BSG_4$ and pointed by reflexivity.
That is, $R_4$ maps $\base$ to $\bn4$ and 
$\Sloop$ to the successor $\zs$. Note that $\zs$ is here
the odd cyclic permutation $(3 2)(2 1)(1 0) = (0 1 2 3)$ 
(\cref{def:sgn-permutation}). Note that $\base\mapsto\id_{\bn2}$
gives rise to an identification $\Bsgn R_4\eqto R_2$ that
we (mis)treat as definitional.

The fibers involved are $\sum_{z:\Sc}(\bn4\eqto R_4(z))$,
$\sum_{X:\BSG_4}(\bn2\eqto \Bsgn(X))$, $\sum_{z:\Sc}(\bn2\eqto R_2(z))$.
These are action types, groupoids that need not be connected,
but here the latter two are. \wip{Does this have to do with epi?}
This means that the latter two classify subgroups consisting of, 
abstractly speaking,
the even permutations of $\bn4$ and the even integers, respectively. 
In this case $F_2$ (pointed by reflexivity) is a homomorphism,
and $F_2^{\abstr}$ maps $\Sloop^{2k}$ to $(0 1 2 3)^{2k} = ((0 2)(1 3))^k$.
The lemma now states that contracting away $q$ in
\[
F_2^{-1}(x_1,p_2) \jdeq \sum_{(z,q):\sum_{z:\Sc}(\bn2\eqto R_2(z))}
(\bn4,\refl{\bn2})\eqto(R_4(z),q)
\]
identifies the latter type
with the action type $\sum_{z:\Sc}(\bn4\eqto R_4(z))$. You are asked to
explore the orbit set of this action type in \cref{xca:fibersofcomposites}.
\end{example}

\begin{xca}\label{xca:fibersofcomposites}
Elaborate the details of \cref{exa:fibersofcomposites}.
\end{xca}
...
\begin{corollary}
  \label{cor:cokermaps}
  \marginnote{$$\xymatrix{\Ker(f_1) \ar[r]^{\id_{\Ker(f_1)}}\ar[d]^{F_1}&\Ker(f_1)  \ar[d]^{\incl_{\ker(f_1)  }}\\
      \Ker(f_2 f_1)\ar[d]^{F_2}\ar[r]^-{\incl_{\ker(f_2 f_1)}}&G_0\ar[d]^{f_1}\ar[r]^{f_2f_1}&G_2\ar[d]^{\id_{G_2}}\\
      \Ker(f_2)  \ar[r]^-{\incl_{\ker(f_2)  }}&G_1\ar[r]^{f_2}&G_2% \\
   % \coker(f_2) &\coker(f_1) 
 }
  $$}
  Consider two composable homomorphisms $f_1:\Hom(G_0,G_1)$ and $f_2:\Hom(G_1,G_2)$.
  There is a unique monomorphism $F_1$ from $\Ker(f_1)  $ to $\Ker(f_2f_1)$
  and a unique homomorphism $F_2$ from $\Ker(f_2f_1)$ to $\Ker(f_2)  $ such that $\incl_{\ker(f_1)  }\eqto \incl_{\ker(f_2 f_1)}F_1$ and $f_1\incl_{\ker(f_2 f_1)}\eqto \incl_{\ker(f_2)  }F_2$.
  Furthermore, $$F_1\eqto _{\typemono(G_1)}\incl_{\ker(f_2)  }$$ and $$(\coker(f_1) )\,\B\incl_{\ker(f_2)  }\eqto _{\B\Ker(f_2)  \to\Set}\coker (F_2).$$

  Consequently,
  \begin{enumerate}
  \item if $f_2$ is a monomorphism then $F_1:\Ker(f_1)  \to\Ker(f_2 f_1)$ is an isomorphism and
  \item if $f_1$ is a monomorphism then $F_2:\Ker(f_2 f_1)\to\Ker(f_2)  $ is an isomorphism.
  \end{enumerate}
Likewise, the set truncations $F'_1$ and $F'_2$ of the maps $F_1$ and $F_2$ constructed in \cref{lem:fibersofcomposites} give maps of $G_1$- and $G_2$-sets,\footnote{%
...
respectively:
$$F_1':\Hom_{G_1}(\coker(f_1),\coker(f_2f_1)\,\Bf_2),\quad F_2':\Hom_{G_2}(\coker(f_2f_1),\coker(f_2)) $$
  such that
  \begin{enumerate}
  \item if $f_2$ is an epimorphism then $F_1'$ is an equivalence, and
  \item if $f_1$ is an epimorphism then $F_2'$  is an equivalence.
  \end{enumerate}
\end{corollary}
\begin{xca}
