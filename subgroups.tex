...
\begin{definition}
  ...
  A monomorphism $(H,i,!)$ is
      \begin{enumerate}
      \item \emph{trivial}\index{trivial monomorphism} if $\BH$ is contractible
      \item \emph{proper}\index{proper monomorphism} if $Bi$ is not an equivalence.
      \end{enumerate}
\end{definition}
...
A monomorphism into $G$ consists of a group $H$ (pointed connected groupoid  $\BH$) and a pointed function $Bi:\BH\to_* \BG$ whose fibers are sets (a pointed \covering).  There is really no need to specify that $\BH_\div$ is a groupoid: if $F:T\to \BG$ is a \covering, then $T$ is automatically a groupoid.  
...
\begin{definition}
  ...
  $$\typesubgroup_G\defequi\sum_{X:\BG\to\Set}{\,}\sum_{\pt:X(\sh_G)}\mathrm{isTrans}(X).$$
...
\marginnote{%
  Which of the equivalent sets is allowed to be called ``the set of subgroups of $G$'' is, of course, a choice.  It could easily have been the other way around and we informally refer to elements in either sets as ``subgroups'' and use the given equivalence as needed.
  }%
  \begin{example}
  ...
  Translating the concepts in \cref{def:typeofmono} through the equivalence $E$ we say that a subgroup $(X,\pt,!):\typesubgroup_G$ is
      ...
      \subsection{The action on the set of subgroups}
