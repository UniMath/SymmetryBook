...
For a function of sets the notion of the ``image'' gives us a factorization through a surjection followed by an injection.  For homomorphism of groups we similarly have a factorization through an epimorphism followed by a monomorphism.  The formula for the image in group theory is essentially the same as the one for sets, except that the propositional truncation we have for the set factorization is replaced by the set truncation present in our formulation of the cokernel: 
\begin{definition}
  \label{def:image}
  Let $f:\Hom(G,G')$  be a homomorphism.
...
\begin{xca}
  ...
  \item the following are equivalent
    ...
    \item the inclusion of the image (first projection) $\image_f:\Hom(\image f,G')$ is an isomorphism,
    \item the induced map of sets $\USym f:\USym G\to\USym {G'}$ is a surjection
    \end{enumerate}
  \item the following are equivalent
    \begin{enumerate}
...
\item the induced map $\tilde f:\Hom(G,\image f)$ is an isomorphism.
    \end{enumerate}
  \end{enumerate}
\end{xca}
