...
\end{definition}
\marginnote{We've seen that there is a close connection between monomorphisms into $G'$ and $G'$-sets, and it is natural to ask if the $G'$-set $\coker f$ gives rise to a monomorphism into $G'$.  The answer to this question is occasionally ``yes'' and is intimately connected to the notion of ``normal subgroups'' which we'll explore shortly.}
\begin{remark}
...
The first projection $B\ker f\to \BG$ is a \covering (the preimages are equivalent to the sets $\sum_{p:\sh_{G'}=\Bf(z)}\Trunc{\sh_{\ker f}=(z,p)}$) giving a monomorphism $\ker_f$ of $\ker f$ into $G$; together defining a function
$$\ker:\Hom(G,G')\to\typemono_G$$
\marginnote{There is an inherent abiguity in our notation: is the kernel of $f$ a group or a monomorphism into $G$?  This is common usage and is only resolved by a typecheck.}
which we will also refer to as the \emph{kernel}.
\end{definition}
\marginnote{\color{blue}THIS WILL BE CHANGED A subgroup is said to be \emph{normal}\index{normal} if it is the kernel of a surjective homomorphism. {clarify the relation between the surjective homomorphism and the subgroup}}
...
\marginnote{The formula for the image in group theory is essentially the same as the one for sets, except that the propositional truncation we have for the set factorization is replaced by the set truncation present in our formulation of the cokernel.}
\begin{definition}
  \label{def:image}
  Let $f:\Hom(G,G')$  be a homomorphism.
  The \emph{image}\index{image} $\image f$ of $f$ is the group
...
The kernel, cokernel and image constructions satisfy a lot of important relations which we will review in a moment, but in our setup they are really complicated ways of interpreting the following fact about preimages
\begin{lemma}
  Consider pointed functions $(f_1,p_1):(X_0,x_0)\to_*(X_1,x_1)$ and $(f_2,p_2):(X_1,x_1)\to_*(X_2,x_2)$ and the resulting functions
  $$F:f_1^{-1}(x_1)\to(f_2f_1)^{-1}(x_2),\qquad F(x,p)\defequi(x,p_1f_2p),$$
  $$G:(f_2f_1)^{-1}(x_2)\to f_2^{-1}(x_2),\qquad G(x,q)\defequi(f_1x,q)$$
  $$H:G^{-1}(x_1,p_2)%\oldequiv\sum_{(x,q)\in (f_2f_1)^{-1}(x_2)}((x_1,p_2)=(f_1x,q))\to \\
  ...
  % \end{multline*}
  % $$H:F^{-1}(x_1,p_1)%\oldequiv\sum_{(x,q)\in (f_2f_1)^{-1}(x_2)}((x_1,p_1)=(x,p_1f_2q))\to \sum_{x:X_0}(x_1=f_1(x))\oldequiv
  % \to f_1^{-1}(x_1),\qquad H(x,q,t)\defequi (x,\fst(t))$$
  \marginnote{$$\xymatrix{G^{-1}(x_1,p_2)\ar[r]^H_\simeq\ar[d]^F&f_1^{-1}(x_1)\ar[d]^{\fst}&\\
      (f_2f_1)^{-1}(x_2)\ar[r]^{\fst}\ar[d]^G&X_0\ar[r]^{f_2f_1}\ar[d]^{f_1}&X_2\ar@{=}[d]\\
    f_2^{-1}(x_2)\ar[r]^{\fst}&X_1\ar[r]^{f_2}&X_2}
  $$}
  (where the function $\xymatrix{((x_1,p_2)=(f_1x,q))\ar[rr]^-{(\overline{p,r})\mapsto p}&&(x_1=f_1(x))}$ is the ``first projection'' explained in the discussion of the interpretation of pairs following \cref{def:pairtopath}).  Then $H$ is an equivalence and $FH$ is definitionally equal to the first projection $\fst:{G^{-1}(x_1,p_2)\to(f_2f_1)^{-1}(x_2)}$ and so through univalence provides an identification
...
\begin{xca}\footnote{proofread}
  \marginnote{$$\xymatrix{\ker f_1\ar@{=}[r]\ar[d]^{F_1}&\ker f_1\ar[d]^{\ker_{f_1}}\\
      \ker f_2f_1\ar[d]^{F_2}\ar[r]^{\ker_{f_2f_1}}&G_0\ar[d]^{f_1}\ar[r]^{f_2f_1}&G_2\ar@{=}[d]\\
      \ker f_2\ar[r]^{\ker_{f_2}}&G_1\ar[r]^{f_2}&G_2% \\
   % \coker F_2&\coker f_1
 }
  $$}
...
\begin{lemma}
  ...
  \item By definition, $f=_{\Hom(G,G')}\image_f\,\tilde f$. %factors as $\tilde f$ followed by the inclusion of the image of $f$ in $G'$.  
...
\begin{definition}
  ...
  The corresponding fixed point in the $G$-set of monomorphisms are called \emph{kernels}\index{type! of kernels}
