...
map $f:B\to C$ with $B$ and $C$ types.}  In particular, if $g,h:C\to D$ are two functions into a set $D$ the proposition $\prod_{c:C}(g(c)=h(c))$ is equivalent to $\prod_{b:B}(gf(b)=hf(b))$.\footnote{\wip{Cf. 6.3.6-8. Caveat in 6.3.6,
...
\begin{lemma}
  \label{lem:injofsetsaremono}Let $f:B\to C$ be a function of sets.
  \begin{enumerate}
  \item The function $f$ is an injection if and only if for any set $A$ and functions $g,h:A\to B$, if
$$\xymatrix{A\ar@<1ex>[r]^g\ar@<-1ex>[r]_h&B\ar[r]^f&C}$$
commutes, \ie $fg=_{A\to C}fh$, then $g=h$.
\item The function $f$ is an surjection if and only if for any set $D$ and functions $g,h:C\to D$, if
...
In this observation, the integers $\ZZ$ plays no more of a role than $\bn 1$ does in \cref{lem:injofsetsaremono}; we can let the source vary over any group $F$:
