...
\wip{In the vocabulary of \cref{ch:actions}, $\BKer(f)$ is the component of
...
$$\xymatrix{
  \sh_{G}\ar[dd]_\id&\sh_{G'}\ar[dr]^{\B\Bfpt}\ar[dd]_g\\
                                                       &&\Bf(\sh_{G})\\
...
\xymatrix{
      ...
      (f_2f_1)^{-1}(x_2)\ar[r]^{\fst}\ar[d]^{F_2}&X_0\ar[rd]^{f_2f_1}\ar[d]^{f_1}\\
    f_2^{-1}(x_2)\ar[r]^{\fst}&X_1\ar[r]^{f_2}&X_2.}
% $$
\caption{\label{fig:fibersofcomposites}\wip{Mapping one vertical fiber sequence
to another.}}
\end{marginfigure} 
...
\begin{lemma}
  ...
  $$F_1:f_1^{-1}(x_1)\to(f_2f_1)^{-1}(x_2),\qquad F_1(x,p)\defeq(x,p_2f_2p\wip{/f_2(p)p_2\text{OFTEN}}),$$
...
\begin{enumerate}
\item $H$ is an equivalence with inverse\footnote{\wip{Use $p$ for $q$,
and swap $p_2f_2(q)$ as $q:x_1\eqto f_1(x)$, $p_2: x_2\eqto f_2(x_1)$.}}
$$H^{-1}(x,q)\defeq((x,p_2f_2(q)),\pathpair q {\refl{p_2f_2(q)}});$$
...
if $(x,q,\pathpair p r):F_2^{-1}(x_1,p_2)$, then $\fst(x,q,\pathpair p r)\jdeq (x,q)$, whereas $F_1H(x,q,\pathpair p r)\jdeq(x,p_2f_2p)$, and the latter
two pairs can be identified using $r:p_2f_2p\eqto q$.\footnote{%
...
\begin{proof}
  ...
  is equivalent to $\sum_{x:X_0}\sum_{q:x_2\eqto (f_2f_1)(x)}\sum_{p:x_1\eqto f_1(x)} (q\eqto p_2f_2p)$ and that $H$ contracts away the $q$ as in \cref{lem:contract-away}, resulting in values in the (equivalent) 
...
From the universal property of the preimage\footnote{\wip{\cref{lem:typefamiliesandfibrations}. Also, $F$ should be $F_1$.}} it furthermore follows that $F$ is the unique map such that $\fst F\eqto _{f_1^{-1}(x_1)\to X_0}\fst$ and $H^{-1}$ is similarly unique with respect to $\fst H^{-1}\eqto F$. \wip{The following corollary states
universal properties of kernel and cokernal, respectively.}\footnote{%
\wip{A bit hard to grasp what is going on here. We could mention the
trivial case of the above lemma, where the types are sets and, \eg
$F_1(x,\refl{x_1})=(x,\refl{x_2})$.}}

\begin{corollary}
  \label{cor:cokermaps}
  \marginnote{$$\xymatrix{\Ker(f_1)  \ar@{=}[r]\ar[d]^{F_1}&\Ker(f_1)  \ar[d]^{\incl_{\ker(f_1)  }}\\
      \Ker(f_2 f_1)\ar[d]^{F_2}\ar[r]^-{\incl_{\ker(f_2 f_1)}}&G_0\ar[d]^{f_1}\ar[r]^{f_2f_1}&G_2\ar@{=}[d]\\
