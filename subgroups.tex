...
\begin{xca}
  ...
  %solution: enough to show that for all $|x,p|\in\coker(\shape_{G'})$ there is a $g:\USym G$ s.t. $g\cdot |\shape_G,p_f|= |x,p|$.  Enough to do this for $x$ being $\shape_G$, and then $g\defequi p_f^{-1}p$ will do
\end{xca}
...
\begin{corollary}
  ...
  \end{enumerate}
\marginnote{If $f,g:A\to\Set$ are two $A$-sets, then $f\to g$ is defined to be the set $$\prod_{a:A}(f(a)\to g(a))$$ and we say that $\phi:f\to g$ is an equivalence if $\prod_{a:A}\isEq\phi(a)$.}
...
To get a geometric idea, think of the circle as the unit circle in the complex numbers so that the $a$-fold cover is simply taking the $a$-fold power.  With this setup, the pullback should consist of pairs $(z_1,z_2)$ of unit length complex numbers with the property that $z_1^a=z_2^b$.  Let $a=a'G$ and $b=b'G$ where $G=\gcd(a,b)$. Taking an arbitrary unit length complex number $z$, then the pair $(z^{b'},z^{a'})$ is in the pull back (since $a'b=ab'$).  But so is $(\zeta z^{b'},z^{a'})$, where $\zeta$ is any $G$\th root of unity.  Each of the $G$-choices of $\zeta$ contributes in this way to a component of the pullback.  In more detail: identifying the cyclic group $C_G$ of order $G$ with the group of $g$\th roots of unity, the top horizontal map $S^1\times C_G\to S^1$ sends $(z,\zeta)$ to $z^{a'}$ and the left vertical map sends $(z,\zeta)$ to the product $\zeta z^{b'}$.
...
\begin{construction}\label{cons:simpler-version-Out}
  ...
  \begin{displaymath}
    \Phi : \Aut_{\grpdTrunc{\conncomp{\UU}{\BG_\div}}}(\grpdtrunc{\BG_\div,!}) \eqto \Out(G) 
  \end{displaymath}
\end{construction}
Before going through the construction of $\Phi$, let us describe its domain in
more details. The goal og this construction is to have a alternative version of
$\Out(G)$ with a more ``hands-on'' classifying type. Because $\out(G)$ is a
transitive $\Aut(G)$-set, we already know that, because the group $\Inn(G)$
associated to the action $\out(G)$ is normal, then its type of symmetries should
be equivalent to $\out(G)(G)$, which we know can be identified with
$\setTrunc{\BG_\div \eqto \BG_\div}$.

{\color{red} Needs to be rewritten. False as it is}
\begin{implementation}[cons:simpler-version-Out]
  % We know that there is a path
  % \begin{displaymath}
  %   \out(G) \eqto \left\{
  %   \begin{aligned}
  %     \BAut(G) \to \Set
  ...
  %   \right\}
  % \end{displaymath}
  % Hence, we will define $\Phi$ as an identification from $\Aut_{\UU}(\BG_\div)$
  % to the group of automorphisms of the $\Aut(G)$-set on the right above, that we
  % denote $\out'(G)$ for the rest of the proof. We do so
  We define $\Phi$ by setting the pointed map $\B\Phi$ to be
  \begin{align*}
    \B\Phi
  ...
  First, we will show that $\Phi$ is well-defined, in the sense that each
  $\Phi(X,\omega)$ is indeed equal to $\out(G)$. Given $(X,\omega)$ is the
  domain, we are trying to prove the proposition $\out(G) = \Phi(X,\omega)$, so
  we can lift the propositional truncation of $\omega$ and assume that we have
  ...
  As both the domain and codomain of $\B\Phi$ are connected, to prove that
  $\Phi$ is an equivalence, it is enough to show that
  $\ap {\B\Phi}: (a \eqto a) \to (\B\Phi a \eqto \B\Phi a)$ for a chosen $a$ in
  the domain. We consider of course
  $a \jdeq (\BG_\div, \trunc{\refl{\BG_\div}})$. Then, for the type
  $(\BG_\div, \trunc{\refl{\BG_\div}})\eqto (\BG_\div, \trunc{\refl{\BG_\div}})$
  is equivalent to simply $\BG_\div \eqto \BG_\div$, and under this
  identification one want to understand the element
  \begin{displaymath}
    \ap{\B\Phi} (p) :
    \prod_{G':\BAut(G)} \left(
      \setTrunc{\BG'_\div \eqto \BG_\div} \eqto \setTrunc{\BG'_\div \eqto \BG_\div}
    \right)
  \end{displaymath}
  However, by induction, we show that for each $p:\BG_\div \eqto \BG_\div$, we
  get paths of type
  \begin{displaymath}
    \ap{\B\Phi} (p) \eqto \left\{
      G' \mapsto \settrunc {p} \circ \blank
    \right\}
  \end{displaymath}
  We craft an inverse for $\ap{\B\Phi}$ (actually for the function of $p$ on the
  right-hand side above that have been identified with $\ap{\B\Phi}$) as
  follows: given a dependent function
  \begin{displaymath}
    e: \prod_{G':\BAut(G)} \left(
      \setTrunc{\BG'_\div \eqto \BG_\div} \eqto \setTrunc{\BG'_\div \eqto \BG_\div}
    \right)
  \end{displaymath}
  take the element $e(G)(\settrunc{\refl{\BG_\div}}): \setTrunc{\BG_\div \eqto \BG_\div}$
\end{implementation}
