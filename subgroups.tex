...
Another thing is that for group the ``sub'' refers to the associated abstract groups, so that ``$\BH$ is a subgroup of $\BG$'' should \emph{not} mean that ``$\BH$ is a subtype of $\BG$'', but that we have a group homomorphism $f:\Hom(H,G)$ so that the induced function $(\pt_H=\pt_H)\to\USym G$ is an ``inclusion of a subset''.
...
Now, as we saw in \cref{lem:injofsetsaremono}, that   $f:\Hom(H,G)$ is a monomorphism is equivalent to saying that $f:(\pt_H=\pt_H)\to\USym G$ is an injection (preimages are propositions) which again is equivalent to the preimages of $\Bf:\BH\to \BG$ being sets.
...
% \begin{remark}
 ...
 %  It is worthwhile to study this action a bit further.  Let $(H,f,!)$ be a subgroup of $G$ and let $g:\USym G$.  We can trade $f$ for $\abstr(f):\Hom^\abstr(\abstr(G),\abstr(H))$ whose underlying set map is the injection $\Bf^=:(\pt_H=\pt_H)\to\USym G$. %We allow ourselves to write ``$f$'' instead of $\Bf^=$. 
...
\begin{definition}
\label{def:image}
The \emph{image}\index{image} of $f$ is the subgroup of $G'$ given by $B\image f\defequi \sum_{z:\BG'}\coker f(z)$ pointed at $\pt_{\image f}\defequi (\pt_{G'},|\sh_G,p_f|)$ together with the first projection $B\image f\to \BG'$ (plus the fact that $\coker f(z)$ is a set).\label{def:surjective}
