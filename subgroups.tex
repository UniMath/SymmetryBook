...
\item\label{it:cover} $B\!f_\div:\BG_\div\to \BH_\div$ is a \covering.
\end{enumerate}
\end{lemma}
...
\item\label{it:connfib} $B\!f_\div:\BG_\div\to \BH_\div$ has connected fibers.
\end{enumerate}
\end{lemma}
...
From the universal property of the preimage (\cref{lem:xxx}) it furthermore follows that $F_2$ is the unique map such that $(\fst F_2)\eqto _{f_1^{-1}(x_1)\to X_0}\fst$ and $H^{-1}$ is similarly unique with respect to $(\fst H^{-1})\eqto F_2$.
\begin{figure}
\[
...
is given by
$$\xymatrix{
  {\BG}\ar[rrr]^-{x\mapsto(\Bf(x),\settrunc{(x, \refl {\Bf(x)})})}&&&
  \sum_{z:{BG'}}\setTrunc{({\Bf})^{-1}(z)}\ar[rr]^-{\fst} &&BG',
}$$
together with base point information.\marginnote{The formula for the image of a group homomorphism $f$
is essentially the same as the one for types, but uses the set image from \cref{sec:higher-image} of the classifying map $\Bf$.}
In particular, we choose the base point $(\sh_{G'},\settrunc{(\sh_G,\Bfpt)})$, so that the \emph{image group} is  %type $\sum_{b:B}\Trunc{f^{-1}(b)}$ is simply replaces by
$$\Img f\defeq\Aut_{\sum_{z:\BG'}\setTrunc{(\Bf)^{-1}(z)}}((\sh_{G'},\settrunc{(\sh_G,\Bfpt)})).$$
...
${x\mapsto(\Bf(x),\settrunc{(x, \refl {\Bf(x)})})}$ and ${\fst}$
