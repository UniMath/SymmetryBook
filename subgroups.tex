...
$\BG$). Let us now use this equivalence to define an homomorphism\footnote{%
	\label{ft:conjugation-iso} \MB{COULD GO IN CH. 4.
Let $G$ be a group, $y$ an element of $\BG$, and $p$ a path of type $y\eqto\shape_G$.
Then $(\id_\BG,p)$ is a pointed equivalence of type $\BG\ptdto(\BG_\div , y))$
...
$\USym(\mkgroup(\id_\BG,p))\jdeq\loops(\id_\BG,p)$ of type $(\sh_G\eqto\sh_G)\equivto(y\eqto y)$
can be identified with $\inv p\blank p$, conjugation.}}
$\inn: G \to \Aut(G)$ by setting
\begin{displaymath}
  \Binn : \BG \ptdto \BAut(G), \quad y \mapsto \mkgroup(\BG_\div , y)
\end{displaymath}
where the path pointing $\Binn$ is \MB{what about $p_\text{inn}\defeq \refl{G} :  G \eqto \Binn (\shape_G)$?}
$\Binn_\ast \defequi \refl G : G \eqto \Binn (\shape_G)$. Notice that for
this map $\Binn$ to be defined properly, we need to show that, for all $y:\BG$, the proposition
$\Trunc{G \eqto \mkgroup((\BG_\div , y))}$ holds. \MB{Easy with \cref{ft:conjugation-iso}} 
...
\begin{remark}
  ...
  definition, \MB{Easier with \cref{ft:conjugation-iso}, I think again $\refl \BG$ should be $\refl G$.} 
