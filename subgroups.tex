...
\marginnote{%
  ...
  \begin{example}
    \cref{ex:sigma2inSigma3} can be displayed as a subgroup of $\Sigma_3$ through
    $$X:\fin_3\to\Set
    $$
...
In \cref{ex:Cm} we studied the map $cy_m:S^1\to\fin_m$  $cy_m(\base)\defequi\bn m$ and 
$cy_m(\Sloop)\defis s_m$, picking out exactly the cyclic permutation $s_m\colon\bn m=\bn m$ (and its iterates) among all permutations.  The reason we studied $cy_m$ was that we wanted to isolate the ``cyclic permutations'' within all symmetries in the group of all permutations $\Sigma_m$. \footnote{BIDNBNB return 4.5}
...
If $\phi:\Hom^\abstr(\mathcal G,\mathcal G')$ is an abstract group homomorphism, the preimage $\phi^{-1}(e_G)$ is an abstract subgroup which is classically called the kernel of $\phi$.
...
$f:\Hom(G,G')$ being given by a pointed function $\Bf:\BG\to_*\BG'$, the kernel and cokernel are just two aspects of the preimage 
...
The first projection $B\ker f\to \BG$ is a \covering (the preimages are equivalent to the sets $\sum_{p:\sh_{G'}=\Bf(z)}\Trunc{\sh_{\ker f}=(z,p)}$) giving a monomorphism into $G$; defining a function
$$\ker:\Hom(G,G')\to\typemono_G$$
\marginnote{There is an inherent abiguity in our notation: is the kernel of $f$ a group or a monomrophism into $G$?  This is common usage and is only resolved by a typecheck.}
which we will also refer to as the \emph{kernel}.
\end{definition}
{\color{blue}THIS WILL BE CHANGED A subgroup is said to be \emph{normal}\index{normal} if it is the kernel of a surjective homomorphism.\footnote{clarify the relation between the surjective homomorphism and the subgroup}}
Written out,
...
\begin{definition}
  ...
  together with the monomorphism of $\image f$ into $G'$ given by the first projection $B\image f\to \BG'$ (a monomorphism since $\coker f(z)$ is a set); resulting in a function
