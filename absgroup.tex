...
from $\agp G$ to the abstract permutation group of $T$.\footnote{\wip{New: }
This homomorphism is also called the \emph{action} of the
$\agp G$-set and denoted as $\cdot_T$.
In case $\agp G$ is $\abstr(G)$ and $T$ is $\USymG$ for some group $G$,
this notation complies with the notation for the group action $\cdot_X$
of a $G$-set $X$ in \cref{def:Gset}.}
...
\subsection{The adjoint triple in the case of abstract groups}
...
up is: what are the abstract versions of the adjoint triple 
$f_!\dashv f^*\dashv f_*$ 
defined for concrete groups in 
...
Recall from \cref{def:restrictandinduce} the action map called restriction map
\[
f^*: \GSet[H]\to\GSet, \qquad f^*(Y) \defeq Y\circ \Bf.
\]
The abstract version of the restriction map is no surprise:
...
Also recall from \cref{def:restrictandinduce} the induced action map 
\[
...
\def\ags{\agp} %experimental
\begin{definition}\label{def:Hom-absG}
  Let $\agp G$ be an abstract group with underlying set $S$, 
  and let $\ags X \jdeq (X,\cdot_X)$ and $\ags Y \jdeq (Y,\cdot_Y)$ be
  $\agp G$-sets. Then the type of maps from  $\ags X$ to $\ags Y$
  is defined as\footnote{\wip{Do we like this new notation?}}   
  \[
    \Hom_{\agp G}(\ags X,\ags Y) \defeq 
  ...
  \glossary(HomG){$\protect\Hom_{\agp G}(\ags X,\ags Y)$}%
...
$\Hom_{\agp H}(\varphi_!\ags X,\ags Y)$ and 
$\Hom_{\agp G}(\ags X,\varphi^*\ags Y)$ for all
$\agp G$-sets $\ags X$ and $\agp H$-sets $\ags Y$. 
\end{xca}

For the other adjunction, $\varphi^*\dashv\varphi_*$, we must
first find a suitable definition of $\varphi_*{\ags X}$. To get an idea,
...
the clue for the definition of $\varphi_*{\ags X}$.
...
by mapping every $\agp G$-set $\ags X \jdeq (X,\cdot_X)$ to
\[
  \varphi_*(\ags X) \defeq 
...
and $\varphi : \absHom(\agp G, \agp H)$ an abstract homomorphism.
Give an equivalence between
$\Hom_{\agp G}(\varphi^*\ags Y,\ags X)$ and 
$\Hom_{\agp H}(\ags Y,\varphi_*\ags X)$
for all $\agp G$-sets $\ags X$ and $\agp H$-sets $\ags Y$.
\end{xca}
