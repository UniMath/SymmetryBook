...
$\exists_{g:\USymG}((p= q \USymf(g))\times(g\cdot_X x = y))$.
...
\begin{definition}\label{def:abshom_*}
  ...
  \[
    \phi_*X(w)\defequi \bigl((\sh_H \eqto w) \times_G X(\sh_G)\bigr)
  \]
...
For all $g$ in the domain of $A$, we have
${\loops g}\circ \loops\pathsp{\blank}^G=
\loops\pathsp{\blank}^H \circ A(g)$.
...
\]%end{marginfigure}
It follows that $A(g)$ is an abstract group homomorphism. 
...
\[%begin{marginfigure}
   \begin{tikzcd}[ampersand replacement=\&]
     \USymG \ar[r,"AB(\phi)"]\ar[d,eql,"{\loops\pathsp{\blank}^G}"'] \& 
     \USymH \ar[d,eqr,"{\loops\pathsp{\blank}^H}"] \\
     \princ G\eqto\princ G \ar[r,"{\loops B(\phi)}"'] \& \princ H\eqto\princ H
   \end{tikzcd}
     % \caption{\label{fig:mapA}caption}
\]%end{marginfigure}
We have to prove $A(B(\phi))=\phi$. When we start with a $g:\USymG$, 
then $\loops\pathsp{\blank}^G$ sends $g$ to 
\[
...
\begin{definition}\label{def:f-upper*}
Let $G$ and $G'$ be groups. 
For any homomorphism $f:\Hom(G,G')$,
we define by precomposition with $\Bf$ the map
\[
f^*: \GSet[G']\to\GSet, \qquad f^*(X) \defeq X\circ \Bf.\qedhere
\]
\end{definition}
...
Then the map $f^*$ from \cref{def:f-upper*} is an injection.
\end{lemma}
...
we can conclude that $f^*:(X\eqto Y)\to(f^*X\eqto  f^*Y)$ is injective.
...
$p_f:\sh_{G'}\eqto f(\sh_G)$) an $F:X\eqto Y$ in the preimage of $a$ 
...
requirements coincide, showing that $f^*$ is an equivalence.\footnote{%
\MB{Typo???}}
\qedhere
