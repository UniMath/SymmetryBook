...
$$\B q_G:\BG\ptdto (\typeabstor[\abstr(G)],\absprtor[\abstr(G)]),\quad q_G(z)=(\princ G(z),\preinv,!).$$
\end{definition}
...
$$
((\sh_G\eqto x),\preinv)=_{\abstr(G)\text{-set}}((\sh_G\eqto y),\preinv)$$
which in turn is equivalent to
$$\sum_{f:(\sh_G\eqto x)=(\sh_G\eqto y)}f\preinv=\preinv f
$$
...
$$(\post,!):(x=y)\to \sum_{f:(\sh_G\eqto x)=(\sh_G\eqto y)}f\preinv=\preinv f.$$
Given an element
$(f,!):\sum_{f:(\sh_G\eqto x)=(\sh_G\eqto y)}f\preinv=\preinv f$, the preimage
$(\post,!)^{-1}(f,!)$ is equivalent to the set
$\sum_{r:x=y}(f=\post_r)$.  But if $(r,!),(s,!): \sum_{r:x=y}(f=\post_r)$, then for all $p:\sh_G\eqto x$ we get that $r\,p=f(p)=s\,p$, that is $r=s$, so that the preimage is in fact a proposition.
To show that the preimage is contractible, it is enough to construct a function $(\sh_G\eqto x)\to \sum_{r:x=y}(f=\post_r)$, and sending $p$ to $f(p)p^{-1}$ will do.
\end{proof}
...
$\absprtor =_{\typetorsor^{\abstr}_{\agp G}}\absprtor$.
...
The abstract group structure of $\abstr(\concr({\agp G}))$ is given by it being the symmetries of $\absprtor$; translated to $\sum_{p:S=S}\prod_{q,s:S}(p(s\,q^{-1})=p(s)\,q^{-1})$ this corresponds via the first projection to the symmetries of $S$.
