...
\]
where $\absprtor[\agp H] \times_{\agp G} X$ is the set quotient $T \times X/\sim$
...
\begin{proof}
  ...
  \begin{align*}
    ...
    &\jdeq \absprtor[\abstr(H)] \times_{\abstr(G)} \princ G(z) \\
    &\jdeq (\sh_H \eqto \sh_H) \times_{\abstr(G)} (\sh_G \eqto z)
  \end{align*}
  and
  \[
    \Bq_H(\Bf(z)) \jdeq \princ H(\Bf(z)) \jdeq (\sh_H \eqto \Bf(z))
  \]
  Concatenation $(h,p)\mapsto (\Bf(p) \pt_\Bf h)$
  descends to the quotient as an isomorphism of $\abstr(H)$-torsors:
  \[
    c_z : (\sh_H \eqto \sh_H) \times_{\abstr(G)} (\sh_G \eqto z)
    \isoto (\sh_H \eqto \Bf(z))
  \]
