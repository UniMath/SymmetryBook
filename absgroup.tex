...
\begin{example}\label{ex:BqG}
  ...
  \cref{xca:absprtorsor}. \wip{This exercise, however, introduces some
  unnecessary technicalities that are easily avoided by complying 
  with the choice made in \cref{def:abstrGtorsors}, which here
  means reversing the direction of the path. 
   
  For any $z:\BG$, the set $\pathsp{z}(\sh_G)\jdeq (z\eqto\sh_G)$
  is a $\abstr(G)$-set by means of 
  $$\post_z:\USymG\to ((z\eqto\sh_G)\eqto (z\eqto\sh_G)),$$
  which is an abstract homomorphism mapping $g$ to $(g\blank)$,
  left multiplication by $g$.
  For $z\jdeq\sh_G$, this is definitionally equal to the principal
  $\abstr(G)$-torsor. We give this construction a short name by defining, 
  for all $z:\BG$, the map
  \[
...
of $\concr$ on objects via two identifications
...
$\varphi : \absHom(\agp G, \agp H)$ to the group homomorphism
...
\footnote{The notation $gy$ in the definition stands for
...
\wip{New:} The definition of $\Bconcr$ can be understood as a version
of \cref{def:restrictandinduce} for abstract torsors.
...
gives an equivalent of the orbit set.
}
...
by applying \cref{lem:equiv-precat-is-ff-split-eso} to the above theorem.}
...
In \cref{def:epimorphism} we will call such an $f$ an \emph{epimorphism},
...
\wip{NB This seems to be only place with $\abstr(\_)$ in this lemma.
