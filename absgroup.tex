...
and it will then be a relative simple matter to show that the constructions of
\begin{enumerate}
...
function $S\to (S=S)$ given by sending $g:S$ to $(s\mapsto s\cdot g^{-1})$.

The type of \emph{${\agp G}$-torsors} is
$$\typeabstor\defequi\sum_{\absSetvar:\absGSet}
  \Trunc{\absprtor \eqto \absSetvar}.$$
\end{definition}
\begin{example}
...
\begin{example}\label{ex:qG}
  ...
  \emph{evaluated at $x$}, \ie the set $\princ G(x)\jdeq (\shape_G\eqto x)$,
  has a natural structure of an $\abstr(G)$-set by means of
  $$\preinv:\USymG\to ((\shape_G\eqto x)\eqto (\shape_G\eqto x)).$$
  Indeed, $\preinv$ is an abstract homomorphism 
  since $\preinv(e_G)\jdeq\refl{\shape_G\eqto x}$ and 
  for $p,q:\USymG$ we have that $\preinv(p\,q)=\preinv(p)\preinv(q)$.% 
  \footnote{For any $r\colon \shape_G\eqto x$ we have that
  ...
  $(\shape_G\eqto x,\preinv,!)$ is an $\abstr(G)$-torsor.
