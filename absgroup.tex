...
\wip{We may have to reconsider this}.}
...
relation.\footnote{\wip{Yes, but why?} The notation $gy$ in the definition stands for
...
\begin{proof}
  ...
  \[
    \begin{tikzcd}
      ...
      \BH \ar[r,"\Bq_H"'] & \absGTor[\abstr(H)]
    \end{tikzcd}
  \]
  commutes. For $z:\BG$, we have
  \begin{align*}
  ...
  Indeed, $\Bq(\Bfpt)$ is right multiplication by $\inv\Bfpt$,
  ...
  sets of $\agp G$ and $\agp H$, respectively. It suffices to check that
  square of underlying sets commutes:
  \[
    \begin{tikzcd}
    ...
    \end{tikzcd}
  \]
  For $g:S$, this amounts to showing that the following square commutes,
  where the left isomorphism is $r_{\agp H}(\varphi(g))$, and the right--down--left
  detour is $\abstr(\concr(\varphi))(r_{\agp G}(g))$:
  \[
    \begin{tikzcd}
      \absprtor[\agp H] \ar[r,equivl]\ar[d,equivl,"\preinv(\varphi(g))"']
      & \absprtor[\agp H] \times_{\agp G} \absprtor[\agp G]
      \ar[d,equivr,"\id \times_{\agp G} \preinv(g)"] \\
      \absprtor[\agp H] \ar[r,equivr]
      & \absprtor[\agp H] \times_{\agp G} \absprtor[\agp G]
    \end{tikzcd}
  \]
  The horizontal isomorphisms map $h$ to $[(h,e)]$, so the square commutes
  since $(h,\inv g) \sim (h\inv{\varphi(g)},e)$.
\end{proof}
\begin{figure}[h]\small
   \[
     ...
     \end{tikzcd}
   \]
\caption{\label{fig:q-natural}The lower right triangle commutes.}
\end{figure}
\wip{
