...
Cayley's Theorem for abstract groups states that every abstract
group $\agp G$ is isomorphic to a subgroup of the abstract permutation
group of the underlying set of $\agp G$. We did not prove this theorem,
but instead proved \cref{lem:allgpsarepermutationgps}, in which
the monomorphism $\rho_G$ singles out the subset of permutations of
$\USymG$ that correspond to the symmetries of $\sh_G$ in the
group $G$. The proof builds on \cref{def:BG2TorsG} and \cref{lem:BGbytorsor}
and identifies the principal $G$-torsor $\princ G$ as the shape 
corresponding to $\sh_G$.

From this we draw our inspiration for constructing a group from an 
abstract group.
...
\[
\sum_{p:S\equivto S}\prod_{q,s:S}\bigl(p(\mu(q,s))=\mu(q,p(s)\bigr).
\]
Setting $s\defequi e$ in the last equation, we see that $p(q)=\mu(q,p(e))$,
that is, $p$ is simply multiplication with an element $p(e):S$.
In other words,\footnote{Indeed, conversely, $\mu(u,\blank)$
satisfies the condition for $p$. Prove this!}
the function
\[
r_{\agp G}:S\to  \sum_{p:S\equivto S}\prod_{q,s:S}
\bigl(p(\mu(q,s))=\mu(q,p(s)\bigr),
\qquad r_{\agp G}(u)\defequi(\mu(u,\blank),!)
\]
is an equivalence of sets, and hence an identification.

The goal is an identification of $\agp G$ and $\abstr(\concr({\agp G}))$,
not just an identification of the underlying set.
The abstract group structure of $\abstr(\concr({\agp G}))$ is given by 
the symmetries of $\absprtor$; translated to 
$\sum_{p:S\equivto S}\prod_{q,s:S}\bigl(p(\mu(q,s))=\mu(q,p(s)\bigr)$ 
...
In view of \cref{def:abstrisfunctor} it suffices for the goal above
that $r_{\agp G}$ preserves multiplication:
...
an isomorphism of abstract groups,\footnote{%
\label{ft:abstract-Cayley}
See: Cayley's Theorem for abstract groups.} \ie
\[
...
\cref{ex:abstrconcrG} shows that every abstract group encodes the symmetries 
of something essentially unique.  Summing up the information we get
...
\begin{lemma}\label{lem:abshom_*}
  ...
  $\eta_\phi:\phi_*\princ G \eqto \princ H$ sending, for all $y:\BH$,
  $[(p,g)]: (\sh_H \eqto y) \times_G \USymG$ to $p\phi(g):(\sh_H\eqto y)$
  is a well defined (fiberwise) equivalence.
\end{lemma}
\begin{proof}
...
\[%begin{marginfigure}
      \begin{tikzcd}[ampersand replacement=\&]
        ...
        \princ G=\princ G \ar[r,"{\loops f}"'] \& \princ H=\princ H
      \end{tikzcd}
...
If $(\phi,!):\absHom(\abstr(G),\abstr(H))$ and $X:\BG\to\Set$ is a $G$-torsor, recall the induced $H$-torsor $\phi_*X$ from \cref{def:abshom_*} and the identity $\eta_\phi:\phi_*\princ G=\princ H$.
Let
