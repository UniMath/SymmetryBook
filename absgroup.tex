...
More precisely, we give an equivalence of categories between the 
categories of concrete and of abstract groups.
...
\glossary(conj){$\protect\conj^g$}{conjugation by $g$}
\end{xca}
...
$\agp X$ is a pair $(X,\cdot_X)$ where $X$ is a set and $\cdot_X$
an abstract homomorphism from $\agp G$ to 
to $\abstr(\Sigma_X)$, the abstract permutation group of $X$.\footnote{%
For any $s:S$, the application of $\cdot_X$ to $s$ is denoted as $s\cdot_X$
of type $X\eqto X$; by univalence $s\cdot_X$ can be applied
to $x:X$ which is denoted as $s\cdot_X x : X$.
 
The abstract homomorphism $\cdot_X$ is also called the \emph{action} of the
$\agp G$-set, similarly to the action of a $G$-set $X$ in \cref{def:Gset}.
In particular when $\agp G$ is $\abstr(G)$ for some group $G$,
the notations $\cdot_X$ for group action and abstract group action comply,
see \cref{rem:GSet=SetHomG}.}
Thus the type of $\agp G$-sets is defined as
\[
  \absGSet\defequi \sum_{X:\Set}\absHom({\agp G},\abstr(\SG_X)).
\]
...
\subsection{The left and right adjoint of restriction, abstractly}
\label{sec:phi-functors}
In \cref{def:abstrGtorsors} we defined the notion of $\agp G$-set
for any abstract group $\agp G$. A natural question that comes
up is: what are the abstract versions of the string of adjunctions
$f_!\dashv f^*\dashv f_*$ defined for concrete groups in 
\cref{def:restrictandinduce,rem:coinduced-Hset}?\footnote{%
See also \cref{ex:action-functors,ex:Sigma-Pi-adj}
for examples of these functors in the categorical setting.}
We answer this question here; the precise correspondence of the 
abstract and the concrete versions will come in \cref{sec:Gsetsabstrconcr}.

In the following paragraphs, $G$ and $H$ are groups and $f:\Hom(G,H)$ 
is a homomorphism.

Recall from \cref{def:restrictandinduce} the restriction map
\[
f^*: \GSet[H]\to\GSet, \qquad f^*(Y) \defeq Y\circ \Bf.
\]
The abstract version of restriction, \ie precompostion, is no surprise:

\begin{definition}\label{def:phi^*}
Given abstract groups $\agp G$ and $\agp H$
and an abstract group homomorphism $\varphi : \absHom(\agp G, \agp H)$,
we define  the map $\varphi^*: \absGSet[\agp H] \to \absGSet[\agp G]$ sending
any $\agp H$-set $(S,\psi)$ to the $\agp G$-set $(S,\psi\circ\varphi)$.
\end{definition}

Also recall from \cref{def:restrictandinduce} the left adjoint of $f^*$: 
\[
f_!: \GSet\to\GSet[H], \qquad f_!X(w)\defeq
\setTrunc{\sum_{z:\BG}\bigl((\Bf(z) \eqto w)\times X(z)\bigr)},
\] 
for all $X:\BG\to\Set$, and $w:\BH$.
Unlike in the case of $f^*$, 
the abstract version of $f_!$ is not immediately clear,
...
Define (and justify by doing \cref{xca:phi_!-OK}!)
\[
  \varphi_! : \absGSet \to \absGSet[\agp H], \quad
  \varphi_!(\agp X) \defeq \absprtor[\agp H] \times_{\agp G} {\agp X}
  \text{~~for all $\agp G$-sets $\agp X$.}
\]
Here $\absprtor[\agp H] \times_{\agp G} {\agp X}$, with
${\agp X}\jdeq(X,\cdot_X)$, is the $\agp H$-set
consisting of the set quotient $T \times X/\sim$
for the equivalence relation on $T\times X$ defined by
\[
  (t,x) \sim (u,y) \defeq 
  \exists_{g:S} (t\varphi(g) = u) \times (x = g\cdot_X y),
\]
paired with the abstract homomorphism that
maps any $h:T$ to the permutation
of $T \times X/\sim$ induced by $(t,x)\mapsto(ht,x)$.
\end{definition}

\begin{xca}\label{xca:phi_!-OK}
With conditions as in \cref{def:phi_!}, show the following:
\begin{enumerate}
\item $\sim$ is an equivalence relation;
\item $(t,x)\mapsto(ht,x)$ respects $\sim$;
\item $(t,x)\mapsto(ht,x)$ induces an abstract homomorphism.\qedhere
\end{enumerate}
\end{xca}

Like maps of $G$-sets in \cref{def:map-of-Gsets}, 
we have a notion of map of $\agp G$-sets:

\begin{definition}\label{def:Hom-absG}
  Let $\agp G$ be an abstract group with underlying set $S$, 
  and let $\agp X \jdeq (X,\cdot_X)$ and $\agp Y \jdeq (Y,\cdot_Y)$ be
  $\agp G$-sets. Then the type of maps from  $\agp X$ to $\agp Y$
  is defined as   
  \[
    \Hom_{\agp G}(\agp X,\agp Y) \defeq 
    \sum_{f:X\to Y}\prod_{s:S}
      \bigl(f\circ(s\cdot_X\blank)=(s\cdot_Y\blank)\circ f\bigr).\qedhere
  \]
  \index{map!of $\agp G$-sets}\index{Gsubset@$\agp G$-subset}
  \glossary(HomG){$\protect\Hom_{\agp G}(\agp X,\agp Y)$}%
  {type of maps of $\agp G$-sets}
\end{definition}
The following exercise asks you to prove $\varphi_! \dashv \varphi^*$,
which is the abstract version of the adjunction $f_! \dashv f^*$ 
from \cref{ex:action-functors}.\footnote{See \cref{sec:cat-equiv} for how 
to obtain this result once the equivalence between the categories 
$\typegroup$ and $\typeabsgp$ has been established.}
\begin{xca}\label{xca:phi_!-|phi^*}
Let $\agp G$ and $\agp H$ be abstract groups
and $\varphi : \absHom(\agp G, \agp H)$ an abstract homomorphism.
Give an equivalence between 
$\Hom_{\agp H}(\varphi_!\agp X,\agp Y)$ and 
$\Hom_{\agp G}(\agp X,\varphi^*\agp Y)$ for all
$\agp G$-sets $\agp X$ and $\agp H$-sets $\agp Y$. 
\end{xca}

For the other adjunction, $\varphi^*\dashv\varphi_*$, we must
first find a suitable definition of $\varphi_*{\agp X}$. To get an idea,
we recall $f_*X$ defined in \cref{rem:coinduced-Hset}:
\[
f_*: \GSet\to\GSet[H], \quad
f_*X(w)\jdeq\prod_{z:\BG}\bigl((\Bf(z)\eqto w) \to X(z)\bigr), 
\text{for $w:\BH$.}
\]
Following \cref{rem:GSet=SetHomG}, we look at the underlying set
$f_*X(\sh_H)$ of the $H$-set $f_*X$. This underlying set
$\prod_{z:\BG}\bigl((\Bf(z)\eqto\sh_H) \to X(z)\bigr)$
is a set of invariant maps for the $G$-set sending $z:\BG$ to
$(\Bf(z)\eqto\sh_H) \to X(z)$. \cref{xca:HomGsets-ev} gives
an equivalence between $f_*X(\sh_H)$ and the subset of fixed
elements of $(\Bf(\sh_G)\eqto\sh_H) \to X(\sh_G)$. This provides
the clue for the definition of $\varphi_*{\agp X}$.
However, since $\varphi$ corresponds to $\USymf\jdeq\abstr(f)$,
we must transport $\Bf(\sh_G)\eqto\sh_H$ to $\USymH$ by precomposition
with $\Bfpt$. The following diagram\footnote{Here $g:\USymG$ is
inverted in $\Bf(\inv g),\USymf(\inv g)$ as $\Bf(\sh_G)$ occurs
on the left in $\Bf(\sh_G)\eqto\sh_H$.}
shows how to transport a fixed $a:(\Bf(\sh_G)\eqto\sh_H) \to X(\sh_G)$ 
to an element $a(\blank\inv{\Bfpt}):\USymH\to X(\sh_G)$ and why
the latter function is also fixed:
\[
\begin{tikzcd}[ampersand replacement=\&, column sep=1cm]
\USymH\ar[d,equivl,"{\blank\USymf(\inv g)}"']
 \&\&(\Bf(\sh_G)\ar[ll,equivl,"{\blank\Bfpt}"']
 =\ar[d,equivr,"{\blank\Bf(\inv g)}"]
 \sh_H)\ar[rr,"a"]
 \&\&X(\sh_G)\ar[d,equivr,"X(g)"]\\
\USymH
 \&\&(\Bf(\sh_G) \ar[ll,equivr,"{\blank\Bfpt}"]
 =
 \sh_H)\ar[rr,"a"']
 \&\&X(\sh_G)
\end{tikzcd}
\]
It remains to understand the action of $f_*X$ under the above equivalences.
Taking into account how $w$ occurs in $f_*X(w)$,\footnote{%
Namely, as the endpoint of a path that is the domain of a function type,
\ie a negative or contravariant position.}
one sees that the action of $h$ maps any $a:(\Bf(\sh_G)\eqto\sh_H)\to X(\sh_G)$
to $a(\inv h \blank)$.\footnote{%
We don't have to worry about the latter being fixed whenever $a$ is,
since everything comes from the $H$-set $f_*X$.
However, you may check this as an exercise.}
The same action works for functions $\USymH\to X(\sh_G)$,
since $(\inv h\blank)\Bfpt = \inv h(\blank\Bfpt)$.
We are now ready to define $\varphi_*$.

\begin{definition}\label{def:phi_*}
Let $\agp G$ and $\agp H$ be abstract groups
with underlying sets $S$ and $T$, respectively.
Let $\varphi : \absHom(\agp G, \agp H)$ be an abstract homomorphism.
Define $\varphi_* : \absGSet \to \absGSet[\agp H]$
by mapping every $\agp G$-set $\agp X \jdeq (X,\cdot_X)$ to
\[
  \varphi_*(\agp X) \defeq 
  \{ b:T\to X \mid \prod_{s:S}\bigl(b\circ(\blank\varphi(\inv s)) =
                                   (s\cdot_X\blank)\circ b\bigr)\},
\]
with action $h\cdot_{T\to X} b$ defined as $b(\inv h \cdot_X \blank)$.
\end{definition}
The following exercise asks you to prove $\varphi^* \dashv \varphi_*$,
which is the abstract version of the adjunction $f^* \dashv f_*$ 
from \cref{ex:action-functors}.\footnote{Again, see \cref{sec:cat-equiv}
for a categorical approach.}
\begin{xca}\label{xca:phi^*-|phi_*}
Let $\agp G$ and $\agp H$ be abstract groups
and $\varphi : \absHom(\agp G, \agp H)$ an abstract homomorphism.
Give an equivalence between
$\Hom_{\agp G}(\varphi^*\agp Y,\agp X)$ and 
$\Hom_{\agp H}(\agp Y,\varphi_*\agp X)$
for all $\agp G$-sets $\agp X$ and $\agp H$-sets $\agp Y$.
\end{xca}
...
$\varphi : \absHom(\agp G, \agp H)$ to the concrete group homomorphism
$\concr(\varphi):\Hom(\concr(\agp G),\concr(\agp H))$
classified by the pointed map
\[
  \Bconcr(\varphi) : \absGTor[\agp G] \ptdto \absGTor[\agp H]
\]
defined as follows. \wip{Changed} First, recall $\varphi_!$ from \cref{def:phi_!}
as unpointed map from $\agp G$-sets to $\agp H$-sets. Second, do 
\cref{xca:Bconcr-OK}\ref{it:Bconcr_pt} to justify the restriction of $\varphi_!$
to the components at the respective abstract principal torsors.
Define the unpointed $\Bconcr(\varphi)$ as the restriction of $\varphi_!$
to these components. Finally, define $\Bconcr(\varphi)_\pt$ as the
inverse of the path given in \cref{xca:Bconcr-OK}\ref{it:Bconcr_pt}.\wip{End changed} 

\begin{xca}\label{xca:Bconcr-OK}
Show the following:
\begin{enumerate}
\item\label{it:Bconcr_pt} $(t,x)\mapsto t\varphi(x)$ induces a path from
$\varphi_!\absprtor[\agp G]$ to $\absprtor[\agp H]$;
...
\end{xca}
  ...
  \cref{xca:phi_!-|phi^*,xca:phi^*-|phi_*} follow from
  \cref{xca:adjunction-_!-^*,xca:adjunction-^*-_*}, resp.
...
Recall from \cref{sec:phi-functors} the adjoint triple
$\varphi_!\dashv\varphi^*\dashv\varphi_*$ 
for a given abstract homomorphism $\varphi$.
Let $G$ and $H$ be groups and let $f:\Hom(G,H)$ be a homomorphism.
Also recall the adjoint triple $f_!\dashv f^*\dashv f_*$
from \cref{def:restrictandinduce,rem:coinduced-Hset}. 
In the following we address how $f^*$ and $\abstr(f)^*$ correspond,
and how $f_!$ and $\abstr(f)_!$ correspond.
Recall the equivalences $\ev_\blank$ from \cref{lem:actionsconcr2abstr}.

Since $f^*$ and $\abstr(f)^*$ are both defined by precomposition,
the following diagram can easily be filled, which we leave as
an exercise to the reader.
\[
  \begin{tikzcd}[column sep=large,row sep=large,ampersand replacement=\&]
      \GSet[H] \ar[r,equivr,"{\ev_{\sh_H}}"] \ar[d,"f^*"'] \& 
...
For $f_!$ and $\abstr(f)_!$ we have a similar diagram. Filling this requires 
the equivalences preceding \cref{def:phi_!} motivating the definition
of $\varphi_!$ in view of the definition of $f_!$. 
\[
  \begin{tikzcd}[column sep=large,row sep=large,ampersand replacement=\&]
