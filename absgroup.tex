...
is an equivalence, where the group $\Sigma_{\absSetvar}$ (as a pointed connected groupoid) is the component of the groupoid $\Set$, pointed at $\absSetvar$.  The component information is moot since we're talking about pointed maps from $\BG$ and we see that $\Hom(G,\Sigma_{\absSetvar})$ is equivalent to $\sum_{F:\BG_\div\to\Set}(\absSetvar=F(\shape_G))$.  Finally,
$$\mathrm{pr}:\sum_{\absSetvar}\sum_{F:\BG_\div\to\Set}(\absSetvar=F(\shape_G))\we
(\BG_\div\to\Set),\quad \mathrm{pr}(\absSetvar,F,p)\defequi F
$$
is an equivalence (since $\sum_{\absSetvar}(\absSetvar=F(\shape_G))$ is contractible).
