...
abstract version of \cref{def:restrictandinduce}.
There we defined, for all $X:\BG\to\Set$, $f:\Hom(G,H)$ and $w:\BH$,
$f_!X(w)\defeq
\setTrunc{\sum_{z:\BG}\bigl((\Bf(z) \eqto w)\times X(z)\bigr)}$.
By \cref{lem:X/G=setTruncX_hG}, this set is equivalent to the set of orbits
...
$\exists_{g:\USymG}((p\USymf(g)=q)\times(x = g\cdot_X y))$
gives an equivalent of the orbit set. \wip{New:} 
There is a clear similarity between this equivalence relation $\sim$
and the one defined in the main text. In \cref{lem:actionsconcr2abstr} 
we'll give equivalences $\ev_{\sh_G}$ between $\GSet$ and $\absGSet[\abstr(G)]$,
for any group $G$, such that the following diagram can be filled:
\[
  \begin{tikzcd}[column sep=large,row sep=large,ampersand replacement=\&]
      \GSet \ar[r,equivr,"{\ev_{\sh_G}}"] \ar[d,"f_!"'] \& 
      \absGSet[\abstr(G)] \ar[d,"{\Bconcr(\abstr(f))}"] \\
      \GSet[H] \ar[r,equivl,"{\ev_{\sh_H}}"']        \& \absGSet[\abstr(H)].
  \end{tikzcd}
\]
\wip{TODO in \cref{sec:Gsetsabstrconcr}, also for $f^*$?}
}
\[
  (t,x) \sim (u,y) \defeq 
...
\end{figure}
  ...
  $e:X\eqto X'$ can be analyzed step for step. 
...
\wip{As a corollary, we get a different proof of \cref{lem:homomabstrconcr}
...
\]
\wip{New: }Similarly, given abstract groups $\agp G$ and $\agp H$
...
\footnote{\wip{Is there a natural way to point this map?}}
...
\begin{exercise}\label{xca:concr2abstrGset}
...
\wip{Can this perhaps be simplified by using
\cref{lem:fixpts-are-fixed} ($\Set$ is a groupoid so that the identity types
involved are sets) and \cref{xca:concr2abstrGset}?}

Since $\BG$ is connected,\footnote{%
\wip{Uses already here that $\Set$ is a groupoid?}} 
 evaluation at $\sh_G$  yields an injection
\[
\ev_{\sh_G}:(f^*X\eqto f^*Y)\to(X(\Bf(\sh_G)\eqto Y(\Bf(\sh_G))),
\]
For the same reason the composite
\[
\ev_{\sh_G}f^*\jdeq \ev_{f(\sh_G)}:(X\eqto Y)\to(X(\Bf(\sh_G)\eqto Y(\Bf(\sh_G)))
\]
...
By the equivalence between $G$-sets and $\abstr(G)$-sets,
$F'$ is uniquely pinned down by $a$ and the requirement that 
for all $h=\Bf(g)$ with $g:\USymG$ the diagram
$$\xymatrix{X(\Bf(\sh_G))\ar[r]^{X(h)}_{=}\ar[d]_{a}^{=}&
  X(\Bf(\sh_G))\ar[d]_{=}^{a}\\
  Y(\Bf(\sh_G))\ar[r]^{=}_{Y({h})}&Y(\Bf(\sh_G))}
$$
commutes.  Likewise, (using transport along the identification
$f_\pt:\sh_{H}\eqto f(\sh_G)$) an $F:X\eqto Y$ in the preimage of $a$ 
is pinned down by the commutativity of the same diagram, 
but with $h:\Bf(\sh_G)\eqto \Bf(\sh_G)$ arbitrary (an a priori more severe
requirement, again reflecting injectivity).
However, when $f:\USymG\to\USymH$ is surjective these 
requirements coincide, showing that the induced $f^*$ is an equivalence.
\qedhere
% Fix for the moment an  $a:X(f(\sh_G)=Y(f(\sh_G))$

% Now, by transport along the identity $f_\pt:\sh_{H}=f(\sh_G)$ and the equivalence between $H$-sets and $\abstr(H)$-sets, an identity $F':X=Y$ of $H$-sets is uniquely pinned down by an identity $F'_{f(\sh_G)}:X(f(\sh_G)=Y(f(\sh_G))$ together with the proposition that for all $h:f(\sh_G)=f(\sh_G)$ the diagram $$\xymatrix{X(f(\sh_G))\ar@{=}[r]^{X_{h}}\ar@{=}[d]_{F'_{f(\sh_G)}}&
%   X(f(\sh_G))\ar@{=}[d]_{F'_{f(\sh_G)}}\\
%   Y(f(\sh_G))\ar@{=}[r]^{Y_{h}}&Y(f(\sh_G))}
% $$
% commutes.  Likewise, an identity $F:f^*X=f^*Y$ is given by exactly the same data, except that the diagram is only required to commute for $h=f(g)$ for all $g:\USymG$.  But when $f:\USymG\to\USymH$ these requirements coincide.


% ; $F:X=Y$ is in the preimage of $a:X(f(\sh_G)=Y(f(\sh_G))$ if and only if $a=F_{f(\sh_G)}$ and for all $h:f(\sh_G)=f(\sh_G)$ the diagram
% $$\xymatrix{X(f(\sh_G))\ar@{=}[r]^{X_{h}}\ar@{=}[d]_{F_{f(\sh_G)}}&
%   X(f(\sh_G))\ar@{=}[d]_{F_{f(\sh_G)}}\\
%   Y(f(\sh_G))\ar@{=}[r]^{Y_{h}}&Y(f(\sh_G))}
% $$
% commutes.  However, since $f$ is surjective there is a $g:\USymG$ so that $h=f(g)$.  Therefore, anything in $f^*X=f^*Y$ which is in the preimage of $a$ is in the image of $f^*:X=Y$ and we have shown that $f^*$ is also a surjection.
\end{proof}
