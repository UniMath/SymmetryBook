...
\begin{example}\label{ex:BqG}
  ...
  \wip{$$\post_z:\USymG\to ((z\eqto\sh_G)\eqto (z\eqto\sh_G)).$$}
  ...
  \]
  pointed by \cref{xca:absprtorsor} \wip{reflexivity}. The name $\Bq_G$ anticipates
...
\]
where $\absprtor[\agp H] \times_{\agp G} X$\footnote{%
  \wip{Better with $\times_{\varphi}$??}} 
...
In \cref{xca:Bconcr-OK} we have collected the technicalities
needed to prove that $\Bconcr(\varphi)$ is well defined.
In particular \ref{xca:Bconcr-OK}\ref{it:Bconcr_pt} gives the 
inverse of the pointing path that we choose.
Note that the $g$ is uniquely determined, since $X$ is a $\agp G$-torsor;
this makes it particularly easy to check that $\sim$ is an equivalence
relation.\wip{Hardly, works for $\agp G$-sets too?} \footnote{The notation $gy$ in the definition stands for
...
$\Bconcr(\varphi)(\absprtor[\agp G])$ to $\absprtor[\agp H]$.\qedhere
\end{enumerate}
\end{xca}
...
\begin{figure}[h]%\small
   \[
     ...
     \absprtor[\abstr(H)] \times_{\abstr(G)} (\USymG,...) 
       \ar[ld,eqr,"c_{\sh_G}"] \\
...
\end{figure}
  ...
  We need to show that the inner square of pointed maps 
  ...
  function defining $c_{\sh_G}$. The dotted second components, the
  ...
  \footnote{The application of $\B\concr(\varphi)$ on an identification
  ...
  \[
    \begin{tikzcd}
      \absprtor[\agp H] \ar[rr,eqr,"{\B\concr(\varphi)_{\pt}}"]\ar[d,equivl,"{\mu_{\agp H}(\blank,\inv{\varphi(s)})}"']
      && \absprtor[\agp H] \times_{\agp G} \absprtor[\agp G]
      \ar[d,equivr,"{\id \times_{\agp G} \mu_{\agp G}(\blank,\inv s)}"] \\
      \absprtor[\agp H] \ar[rr,eql,"{\B\concr(\varphi)_{\pt}}"']
      && \absprtor[\agp H] \times_{\agp G} \absprtor[\agp G].
    \end{tikzcd}
  \]
  The pointing path of $\B\concr(\varphi)$ corresponds with
