...
\[
  (t,x) \sim (u,y) \defeq \exists_{g:S} (t\varphi(g) = u) \times (x = gy).
\]
...
\begin{proof}
  ...
  \]
  Concatenation $(h,p)\mapsto (\Bf(p) \pt_\Bf h)$
  descends to the quotient as an isomorphism of $\abstr(H)$-torsors:
  \[
    c_z : \bigl((\sh_H \eqto \sh_H) \times_{\abstr(G)} (\sh_G \eqto z)\bigr)
...
\begin{figure}[h]\small
   \[
     ...
     (\USymH,h\mapsto(h\blank))
       \ar[d,"\Bq_H(\Bfpt)"]
       \ar[r,eql,"{\Bconcr(\abstr(f))_\pt}"']&
     \absprtor[\abstr(H)] \times_{\abstr(G)} (\USymG,...) 
       \ar[ld,"c_{\sh_G}"] \\
     \Bf(\sh_G)\ar[rrr,mapsto] &&& ((\Bf(\sh_G)\eqto\sh_H),h\mapsto(h\blank))
     \end{tikzcd}
   \]
\caption{\wip{TBD}The lower right triangle commutes.}
\end{figure}
\wip{
